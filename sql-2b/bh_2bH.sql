create or replace procedure PBH_2BH_CT(
...
select json_object(so_hd,ma_kh,so_dt,kieu_hd),nv into dt_ct,b_nv from bh_2b where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_nv <> 'H' then
  b_loi:='loi:Sai kieu hop dong:loi'; raise PROGRAM_ERROR;
end if;
select txt into dt_ds from bh_2b_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ds';
...
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
  from bh_2b_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
...
/
create or replace procedure PBH_2BH_TESTr(
    ...
    ds_nam_sx out pht_type.a_num,ds_gia out pht_type.a_num,
...
for ds_lp in 1..a_ds_ct.count loop
    ...
    b_lenh:=b_lenh||'loai_xe,nhom_xe,dong,dco,ttai,so_cn,nam_sx,gia,';
    ...
    EXECUTE IMMEDIATE b_lenh into
        ...
        ds_loai_xe(ds_lp),ds_nhom_xe(ds_lp),ds_dong(ds_lp),ds_dco(ds_lp),ds_ttai(ds_lp),ds_so_cn(ds_lp),ds_nam_sx(ds_lp),ds_gia(ds_lp),
    ...
    if ds_ma_sp(ds_lp)<>' ' and FBH_2B_SP_HAN(ds_ma_sp(ds_lp))<>'C' then b_loi:='loi:Sai ma san pham xe '||a_bien(ds_lp)||':loi'; return; end if;
    if ds_cdich(ds_lp)<>' ' and FBH_MA_CDICH_HAN(ds_cdich(ds_lp))<>'C' then b_loi:='loi:Het chien dich xe '||a_bien(ds_lp)||':loi'; return; end if;
    if ds_goi(ds_lp)<>' ' and FBH_2B_GOI_HAN(ds_goi(ds_lp))<>'C' then b_loi:='loi:Ma goi het han xe '||a_bien(ds_lp)||':loi'; return; end if;
    ...
    if ds_hang(ds_lp)<>' ' and FBH_2B_HANG_HAN(ds_hang(ds_lp))<>'C' then b_loi:='loi:Sai hang xe '||a_bien(ds_lp)||':loi'; return; end if;
    if ds_hang(ds_lp)<>' ' and ds_hieu(ds_lp)<>' ' and FBH_2B_HIEU_HAN(ds_hang(ds_lp),ds_hieu(ds_lp))<>'C' then
        b_loi:='loi:Sai hieu xe '||a_bien(ds_lp)||':loi'; return;
    end if;
    if ds_hang(ds_lp)<>' ' and ds_hieu(ds_lp)<>' ' and ds_pban(ds_lp)<>' ' and FBH_2B_PB_HAN(ds_hang(ds_lp),ds_hieu(ds_lp),ds_pban(ds_lp))<>'C' then
        b_loi:='loi:Sai phien ban xe '||a_bien(ds_lp)||':loi'; return;
    end if;
    if ds_dong(ds_lp)<>' ' and FBH_2B_DONG_HAN(ds_dong(ds_lp))<>'C' then b_loi:='loi:Sai dong xe '||a_bien(ds_lp)||':loi'; return; end if;
    if ds_dco(ds_lp) not in(' ','X','D','E','H') then b_loi:='loi:Sai loai dong co:loi'; return; end if;
    if ds_loai_xe(ds_lp)<>' ' and FBH_2B_LOAI_HAN(ds_loai_xe(ds_lp))<>'C' then b_loi:='loi:Sai loai xe '||a_bien(ds_lp)||':loi'; return; end if;
    if ds_nhom_xe(ds_lp)<>' ' and FBH_2B_NHOM_HAN(ds_nhom_xe(ds_lp))<>'C' then b_loi:='loi:Sai nhom xe '||a_bien(ds_lp)||':loi'; return; end if;
    if ds_md_sd(ds_lp)<>' ' and FBH_2B_MDSD_HAN(ds_md_sd(ds_lp))<>'C' then b_loi:='loi:Sai muc dich su dung xe '||a_bien(ds_lp)||':loi'; return; end if;
    ...
    if b_bao_gia<>'BG' then
      if ds_gcn(ds_lp)=' ' or instr(ds_gcn(ds_lp),'.')=2 then
          if b_so_hdL='P' then b_loi:='loi:Nhap so GCN xe '||a_bien(ds_lp)||':loi'; return;
          elsif b_so_hdL='E' and b_kieu_hd<>'S' and instr(ds_nv_bh(ds_lp),'B')<>0 then
            if b_ttrang<>'D' then ds_gcn(ds_lp):=nvl(trim(ds_gcn(ds_lp)),' '); -- nam -- khi ttrang ='D' thi moi sinh so gcn cho ds xe moi
              ...
              if ds_kieu_gcn(ds_lp)<>'G' then
                  ...
                  if(b_i1>0) then
                     select max(REGEXP_SUBSTR(gcn, 'B([0-9]+)', 1, 1, NULL, 1)) into b_i1 from bh_2b_ds where ma_dvi=b_ma_dvi and so_id_dt=ds_so_id(ds_lp) and kieu_gcn=b_kieu_hd;
                  end if;
         ...
         select nvl(max(ngay_cap),b_ngay_capH) into b_ngay_cap from bh_2b_ds where ma_dvi=b_ma_dvi and so_id_dt=ds_so_id(ds_lp) and gcn=ds_gcn(ds_lp);
      end if;
    end if;
...
if b_kieu_hd in('B','S') and b_so_idG<>0 then
    for r_lp in (select so_id_dt,gcn from bh_2b_ds where ma_dvi=b_ma_dvi and so_id=b_so_idG) loop
...
for b_lp in 1..ds_so_id.count loop
    ds_xe_id(b_lp):=FBH_2Btso_SO_ID(ds_bien_xe(b_lp),ds_so_khung(b_lp));
    if ds_xe_id(b_lp)<>0 then
        for r_lp in(select distinct nv_bh from bh_2b_ds where xe_id=ds_xe_id(b_lp) and so_id_dt<>ds_so_id(b_lp) and
            FKH_GIAO(ds_ngay_hl(b_lp),ds_ngay_kt(b_lp),ngay_hl,ngay_kt)='C' and FBH_2B_TTRANG(ma_dvi,so_id,'C')='D') loop
...
for b_lp in 1..ds_so_id.count loop
    if ds_ktru(b_lp)<>0 then
        PBH_2BG_KBT(ds_nv_bh(b_lp),ds_so_idP(b_lp),ds_ktru(b_lp),a_ds_kbt(b_lp),b_loi);
        if b_loi is not null then return; end if;
    end if;
...
/
create or replace procedure PBH_2BH_NH_NH(
...
begin
-- Dan - Nhap
b_loi:='loi:Loi nhap Table BH_2B:loi';
b_so_dt:=ds_so_id.count;
if b_ttrang='D' then b_so_id_kt:=0; end if;
for b_lp in 1..ds_so_id.count loop
    insert into bh_2b_ds values(b_ma_dvi,b_so_id,
...
for b_lp in 1..dk_so_id.count loop
    ...
    insert into bh_2b_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),dk_bt(b_lp),dk_ma(b_lp),dk_ten(b_lp),dk_tc(b_lp),
        dk_ma_ct(b_lp),dk_kieu(b_lp),dk_tien(b_lp),dk_pt(b_lp),dk_phi(b_lp),dk_cap(b_lp),dk_ma_dk(b_lp),dk_lh_nv(b_lp),
        dk_t_suat(b_lp),dk_thue(b_lp),dk_ttoan(b_lp),dk_ptB(b_lp),dk_ptG(b_lp),dk_phiG(b_lp),dk_lkeP(b_lp),dk_lkeB(b_lp),dk_lh_bh(b_lp));
end loop;
...
for b_lp in 1..ds_so_id.count loop
    delete bh_2b_ID where xe_id=ds_xe_id(b_lp);
    insert into bh_2b_ID values(ds_xe_id(b_lp),ds_tenC(b_lp),ds_cmtC(b_lp),ds_mobiC(b_lp),ds_emailC(b_lp),ds_dchiC(b_lp),
...
insert into bh_2b_txt values(b_ma_dvi,b_so_id,'dt_ds',dt_ds);
insert into bh_2b_txt values(b_ma_dvi,b_so_id,'dt_ds_txt',dt_ds_txt);
if b_ttrang in('T','D') then
    for b_lp in 1..lt_so_id.count loop
        insert into bh_2b_kbt values(b_ma_dvi,b_so_id,lt_so_id(b_lp),lt_dk(b_lp),lt_lt(b_lp),lt_kbt(b_lp));
    end loop;
...
if b_ttrang='D' then
    ...
    for b_lp in 1..ds_so_id.count loop
        ...
        if b_loi is not null then return; end if;
        insert into bh_hd_goc_ttindt values(b_ma_dvi,b_so_idD,ds_xe_id(b_lp),'2B',
...
/
create or replace procedure PBH_2BH_NH(
...
if b_so_id<>0 then
    ...
    if b_i1<>0 then
        select ngay_ht,kieu_hd,ttrang into b_ngay_htC,b_kieu_hd,b_ttrang from bh_2b where so_id=b_so_id for update nowait;
