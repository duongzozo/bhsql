create or replace function FBH_2BB_MA_DT(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return varchar2
AS
    b_kq varchar2(20);
begin
-- Dan - Tra bien
select nvl(min(ma_dt),' ') into b_kq from
    bh_2bB_ds where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_2BB_TEN(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return nvarchar2
AS
    b_kq nvarchar2(500);
begin
-- Dan - Tra dvi rui ro
select min(ten) into b_kq from bh_2bB_ds where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
return b_kq;
end;

create or replace function FBH_2BB_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Dan - Tra so bao gia
select min(so_hd) into b_kq from bh_2bB where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_2BB_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2, b_nv varchar2:='') return number
as
    b_kq number;
begin
-- Dan - Tra so id
if nvl(b_nv,' ')=' ' then
    select nvl(min(so_id),0) into b_kq from bh_2bB where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
else
    select nvl(min(so_id),0) into b_kq from bh_2bB where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv;
end if;
return b_kq;
end;
/
create or replace procedure PBH_2BB_LKE(
...
if b_klk='N' then
    select count(*) into b_dong from bh_2bB where ma_dvi=b_ma_dvi and 
        ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj returning clob) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_2bB where
    ...
    select count(*) into b_dong from bh_2bB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj returning clob) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_2bB  where
...
/
create or replace procedure PBH_2BB_LKE_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hd:=FBH_2BB_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_2bB where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
...
/
create or replace procedure PBH_2BB_SO_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_id:=FBH_2BB_SO_ID(b_ma_dvi,b_so_hd,b_nv);
...
/
create or replace procedure PBH_2BB_XOA_XOA(
...
begin
-- Dan - Xoa
b_loi:='loi:Loi xu ly PBH_2BB_XOA_XOA:loi';
select count(*) into b_i1 from bh_2bB where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1>0 then
  select nsd into b_nsdC from bh_2bB where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
b_loi:='loi:Loi xoa Table bh_2bB:loi';
if b_lan = 0 then
    delete bh_2bB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
else
    delete bh_2bB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan;
end if;
delete bh_2bB_ds where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_2bB_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_2bB where ma_dvi=b_ma_dvi and so_id=b_so_id;
PBH_BAO_NV_XOA(b_ma_dvi,b_nsd,b_so_id,b_nh,b_loi);
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_2BB_XOA(
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_2BB_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',0,b_loi);
-- chua xoa lich su
-- delete bh_2bB_ls where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_loi is not null then raise PROGRAM_ERROR; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_2BB_HDBS(
...
if b_so_id<>0 then
    select JSON_ARRAYAGG(json_object( ma_dvi,so_id,lan,'ngay_ht' value FKH_JS_GTRIn(txt,'ngay_ht') , 'ttoan' value FKH_JS_GTRIn(txt,'ttoan'),
                              ...
                              from bh_2bB_ls where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
end if;
select json_object('dong' value b_dong,'cs_lke' value cs_lke returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_2BBH_CTbg(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select count(1) into b_i1 from bh_2bB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct' and lan = b_lan;
if b_i1 > 0 then
...
/
create or replace procedure PBH_2BB_CHUYEN_HD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_id<>0 and b_lan>0 then
    select so_id,max(lan),ttrang into b_so_id,b_i1,b_ttrang from bh_2bB where ma_dvi=b_ma_dvi and so_id=b_so_id group by so_id,ttrang;
