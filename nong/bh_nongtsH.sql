create or replace procedure PBH_NONGTSH_MO(
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_nongts_sp a,(select distinct ma_sp from bh_nongts_phi where nhom in('H') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
    where a.ma=b.ma_sp and FBH_NONGTS_MA_SP_HAN(a.ma)='C';
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_cdich from
    bh_ma_cdich a,(select distinct cdich from bh_nongts_phi where nhom in('H') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_goi from
    bh_nongts_goi a,(select distinct goi from bh_nongts_phi where nhom in('H') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
    where a.ma=b.goi and FBH_NONGTS_GOI_HAN(a.ma)='C';
...
/
create or replace procedure PBH_NONGTSH_MOd(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraOut out clob)
AS
...
/
create or replace procedure PBH_NONGTSH_CT(
...
select json_object(so_hd,ma_kh,so_dt) into dt_ct from bh_nongts where ma_dvi=b_ma_dvi and so_id=b_so_id;
select txt into ds_ct from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ct';
select count(*) into b_i1 from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hk';
if b_i1=1 then
    select txt into dt_hk from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hk';
end if;
select txt into ds_dk from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dk';
select txt into ds_pvi from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_pvi';
select count(*) into b_i1 from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
if b_i1=1 then
    select txt into ds_dkbs from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
end if;
select count(*) into b_i1 from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
if b_i1=1 then
    select txt into ds_lt from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
end if;
select count(*) into b_i1 from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
if b_i1=1 then
    select txt into ds_kbt from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
end if;
select count(*) into b_i1 from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
if b_i1=1 then
    select txt into ds_ttt from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
end if;
select JSON_ARRAYAGG(json_object(ngay,tien) order by ngay) into dt_kytt from bh_nongts_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_nongts_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
...
/
create or replace procedure PBH_NONGTSH_TESTd(
    ...
    b_loai out varchar2,b_kvuc out varchar2,b_hthuc out varchar2,b_tuoi_t out number,b_tuoi_d out number,
...
begin
-- Nam - Nhap
b_loi:='loi:Loi xu ly PBH_NONGTSH_TESTd:loi';
...
PBH_NONGTS_BPHI_TSO(dt_ct,b_nhom,b_ma_sp,b_cdich,b_goi,b_loai,b_kvuc,b_hthuc,b_ngay_hl);
...
if b_so_id_dt<100000 then
    ...
    select count(*) into b_i1 from bh_nongts_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcnG;
...
if b_gcn=' ' or instr(b_gcn,'.')=2 then
    ...
    if b_kieu_gcn<>'G' then
        select count(*) into b_i1 from bh_nongts_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        if(b_i1>0) then
           select max(REGEXP_SUBSTR(gcn, 'B([0-9]+)', 1, 1, NULL, 1)) into b_i1 from bh_nongts_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        end if;
        b_gcn:=b_gcn||'/'||b_kieu_hd||to_char(b_i1+1);
    end if;
else
    select nvl(max(ngay_cap),b_ngay_capH) into b_ngay_cap from bh_nongts_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcn;
end if;
...
FBH_NONGTSH_PHI(b_ma_dvi,b_txt,dt_dk,dt_dkbs,dt_pvi,b_so_idP,
...
if b_ttrang in('T','D') then
    select count(*) into b_i1 from bh_nongts_phi_txt where so_id=b_so_idP and loai='dt_khd';
    if b_i1<>0 then
        select txt into dt_khd from bh_nongts_phi_txt where so_id=b_so_idP and loai='dt_khd';
        b_i1:=length(dt_khd)-2;
        dt_khd:=substr(dt_khd,2,b_i1);
        PBH_NONGTSH_KHD(dt_ctH,dt_ct,dt_dk,dt_dkbs,dt_pvi,dt_khd,b_loi);
        if b_loi is not null then return; end if;
    end if;
end if;
b_loi:='';
...
/
create or replace procedure PBH_NONGTSH_TESTr(
    ...
    dvi_loai out pht_type.a_var,dvi_kvuc out pht_type.a_var,dvi_hthuc out pht_type.a_var,
...
AS
    ...
    b_loai varchar2(500); b_kvuc varchar2(500); b_hthuc varchar2(500);
...
begin
-- Nam - Nhap
b_loi:='loi:Loi xu ly PBH_NONGTSH_TESTr:loi';
...
for ds_lp in 1..a_ds_ct.count loop
    ...
    b_loai,b_kvuc,b_hthuc,b_tuoi_t,b_tuoi_d,b_ngay_hl,b_ngay_kt,b_ngay_cap,b_so_idP,
    ...
    dvi_loai(b_kt_ds):=b_loai; dvi_kvuc(b_kt_ds):=b_kvuc; dvi_hthuc(b_kt_ds):=b_hthuc;
...
if b_kieu_hd in('B','S') and b_so_idG<>0 then
    for r_lp in (select so_id_dt,dvi from bh_nongts_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idG) loop
...
/
create or replace procedure PBH_NONGTSH_NH_NH(
...
-- Rieng
    ...
    dvi_loai pht_type.a_var,dvi_kvuc pht_type.a_var,dvi_hthuc pht_type.a_var,
...
begin
-- Nam - Nhap
b_loi:='loi:Loi Table bh_nongts:loi';
b_so_dt:=dvi_so_id.count;
if b_ttrang='D' then b_so_id_kt:=0; end if;
for b_lp in 1..dvi_so_id.count loop
    insert into bh_nongts_dvi values(b_ma_dvi,b_so_id,dvi_so_id(b_lp),b_lp,dvi_kieu_gcn(b_lp),dvi_gcn(b_lp),dvi_gcnG(b_lp),
        dvi_dvi(b_lp),dvi_ddiem(b_lp),dvi_cdt(b_lp),dvi_tdx(b_lp),dvi_tdy(b_lp),dvi_bk(b_lp),
        dvi_loai(b_lp),dvi_hthuc(b_lp),dvi_kvuc(b_lp),b_so_dt,
...
for b_lp in 1..dk_so_id.count loop
    ...
    insert into bh_nongts_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),b_lp,
...
insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
if dt_hk is not null then
    insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_ct',ds_ct);
insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_dk',ds_dk);
insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_pvi',ds_pvi);
if ds_dkbs is not null then
    insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_dkbs',ds_dkbs);
end if;
if ds_lt is not null then
    insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_lt',ds_lt);
end if;
if ds_kbt is not null then
    insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_kbt',ds_kbt);
end if;
if ds_ttt is not null then
    insert into bh_nongts_txt values(b_ma_dvi,b_so_id,'ds_ttt',ds_ttt);
end if;
for b_lp in 1..tt_ngay.count loop
    insert into bh_nongts_tt values(b_ma_dvi,b_so_id,tt_ngay(b_lp),tt_tien(b_lp));
end loop;
if b_ttrang in('T','D') then
    insert into bh_nong values(
        b_ma_dvi,b_so_id,b_so_hd,b_ngay_ht,'TS',b_ttrang,b_kieu_hd,b_so_hd_g,b_kieu_kt,b_ma_kt,b_kieu_gt,b_ma_gt,b_ma_cb,
    ...
    for b_lp in 1..lt_so_id.count loop
        select JSON_ARRAYAGG(json_object(
            ...
            from bh_nongts_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=lt_so_id(b_lp);
        insert into bh_nongts_kbt values(b_ma_dvi,b_so_id,lt_so_id(b_lp),b_txt,lt_lt(b_lp),lt_kbt(b_lp));
...
/
create or replace procedure PBH_NONGTSH_NH(
...
if b_so_id<>0 then
    select count(*) into b_i1 from bh_nongts where so_id=b_so_id;
    if b_i1<>0 then
        select ngay_ht,kieu_hd,ttrang into b_ngay_htC,b_kieu_hd,b_ttrang from bh_nongts where so_id=b_so_id for update nowait;
        ...
        PBH_NONGTS_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,b_kieu_hd,b_loi);
        if b_loi is not null then raise PROGRAM_ERROR; end if;
    end if;
else
...
PBH_HD_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,b_ngay_htC,'bh_nongts',dt_ct,dt_kytt,
...
PBH_NONGTSH_TESTr(
    ...
    dvi_loai,dvi_kvuc,dvi_hthuc,dvi_tuoi_t,dvi_tuoi_d,
...
/
create or replace procedure FBH_NONGTSH_PHIb(b_tp number,
...
/
create or replace procedure PBH_NONGTSH_PHI (
...
FKH_JS_NULL(dt_ct); FKH_JSa_NULL(dt_dk); FKH_JS_NULL(dt_dkbs); FKH_JSa_NULL(dt_pvi);
FBH_NONGTSH_PHI(b_ma_dvi,dt_ct,dt_dk,dt_dkbs,dt_pvi,b_so_idP,
...
/
create or replace procedure FBH_NONGTSH_PHI(
...
AS
    ...
    b_nhom varchar2(1); b_ma_sp varchar2(10); b_cdich varchar2(10); b_goi varchar2(10); 
    b_loai varchar2(500); b_kvuc varchar2(500); b_hthuc varchar2(500); b_so_idG number:=0;
    b_kieu_hd  varchar2(1); b_so_hdG varchar2(20);
...
begin
-- Nam - Tinh phi
b_loi:='loi:Loi xu ly FBH_NONGTSH_PHI:loi';
...
PBH_NONGTS_BPHI_TSO(dt_ct,b_nhom,b_ma_sp,b_cdich,b_goi,b_loai,b_kvuc,b_hthuc,b_ngay_hl);
b_so_idP:=FBH_NONGTS_BPHI_SO_ID(b_nhom,b_ma_sp,b_cdich,b_goi,b_loai,b_kvuc,b_hthuc,b_ngay_hl);
...
for b_lp in 1..pvi_ma.count loop
    pvi_loai(b_lp):=nvl(trim(pvi_loai(b_lp)),'C'); pvi_pp(b_lp):=nvl(trim(pvi_pp(b_lp)),'GP');
end loop;
...
FBH_NONGTSH_KHO(b_ngay_hl,b_ngay_kt,b_kho,b_loi,b_dk);
if b_loi is not null then return; end if;
b_kt:=0;
for b_lp_dk in 1..dk_ma.count loop
    select count(*) into b_i1 from bh_nongts_phi_dk where so_id=b_so_idP and ma=dk_ma(b_lp_dk);
    ...
    select ma,ten,tc,ma_ct,'T',lkeM,lkeP,lkeB,luy,ktru,ma_dk,lh_nv,t_suat,cap,nv into
        ...
        from bh_nongts_phi_dk where so_id=b_so_idP and ma=dk_ma(b_lp_dk);
...
for b_lp_bs in 1..bs_ma.count loop
    b_kt:=b_kt+1; b_ktL:=b_kt;
    select count(*) into b_i1 from bh_nongts_phi_dk where so_id=b_so_idP and ma=bs_ma(b_lp_bs);
    if b_i1=1 then
    select ma,ten,tc,ma_ct,'T',lkeM,lkeP,lkeB,luy,ktru,ma_dk,ma_dkC,lh_nv,t_suat,cap,nv into
        ...
        from bh_nongts_phi_dk where so_id=b_so_idP and ma=bs_ma(b_lp_bs);
    else
...
FBH_NONGTSH_PHIb(b_tp,a_ma,a_ma_ct,a_lkeP,a_cap,a_phi,a_ttoan,b_loi);
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
