create or replace function FBH_NONGVN_DVI(
...
b_so_idB:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select min(dvi) into b_kq from bh_nongvn_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_NONGVN_HL(
...
b_so_idB:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select ngay_hl,ngay_kt into b_ngay_hl,b_ngay_kt from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_idB;
if b_ngayd<=b_ngay_kt and b_ngay_hl<=b_ngayc then b_kq:='C'; end if;
return b_kq;
end;
/
create or replace function FBH_NONGVN_HLd(
...
b_so_idB:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
if b_so_idB<>0 then
    select ngay_hl,ngay_kt into b_ngay_hl,b_ngay_kt from bh_nongvn_dvi  where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=b_so_id_dt;
    if b_ngay between b_ngay_hl and b_ngay_kt then b_kq:='C'; end if;
end if;
return b_kq;
end;
/
create or replace function FBH_NONGVN_KIEU_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Nam - Tra so hop dong
select nvl(min(kieu_hd),' ') into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_NONGVN_KIEU_HDd(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(1); b_so_idD number:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
begin
-- Nam - Tra kieu hop dong qua so_id
select nvl(min(kieu_hd),' ') into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_idD;
return b_kq;
end;
/
create or replace function FBH_NONGVN_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Nam - Tra so hop dong
select min(so_hd) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_NONGVN_SO_HDd(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50); b_so_idD number;
begin
-- Nam - Tra so hop dong dau
b_so_idD:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
b_kq:=FBH_NONGVN_SO_HD(b_ma_dvi,b_so_idD);
return b_kq;
end;
/
create or replace function FBH_NONGVN_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2,b_nv varchar2:='') return number
as
    b_kq number;
begin
-- Nam - Tra so id
if nvl(b_nv,' ')=' ' then 
    select nvl(min(so_id),0) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
else 
    select nvl(min(so_id),0) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv;
end if;
return b_kq;
end;
/
create or replace function FBH_NONGVN_ID_ID_DT(b_ma_dvi varchar2,b_so_id number,b_gcn varchar2) return number
as
    b_kq number;
begin
-- Nam - Tra so id
select nvl(min(so_id_dt),0) into b_kq from bh_nongvn_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id and gcn=b_gcn;
return b_kq;
end;
/
create or replace function FBH_NONGVN_ID_GCN(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return varchar2
as
    b_kq varchar2(20);
begin
-- Nam - Tra gcn
select min(gcn) into b_kq from bh_nongvn_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_NONGVN_HD_ID_DT(b_ma_dvi varchar2,b_so_hd number,b_gcn varchar2) return number
as
    b_kq number:=0; b_so_id number;
begin
-- Nam - Tra so id DT
b_so_id:=FBH_NONGVN_SO_ID(b_ma_dvi,b_so_hd);
if b_so_id<>0 then b_kq:=FBH_NONGVN_ID_ID_DT(b_ma_dvi,b_so_id,b_gcn); end if;
return b_kq;
end;
/
create or replace function FBH_NONGVN_SO_IDd(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Nam - Tra so id dau
...
/
create or replace function FBH_NONGVN_SO_GCNd(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return number
as
    b_kq number; b_so_idD number;
begin
-- Nam - Tra so GCN dau
b_so_idD:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
select min(gcn) into b_kq from bh_nongvn_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idD and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace procedure PBH_NONGVN_SO_ID(
...
b_so_id:=FBH_NONGVN_SO_ID(b_ma_dvi,b_so_hd,b_nv);
...
/
create or replace function FBH_NONGVN_SO_IDc(b_ma_dvi varchar2,b_so_id number,b_ttrang varchar2:='K') return number
as
...
b_so_idD:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
if b_ttrang<>'C' then
    select nvl(max(so_id),0) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
else
    select nvl(max(so_id),0) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D';
end if;
return b_kq;
end;
/
create or replace function FBH_NONGVN_SO_IDb(
...
-- Nam - Tra so id cuoi
b_so_idD:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D' and ngay_cap<=b_ngay;
return b_kq;
end;
/
create or replace procedure FBH_NONGVN_SO_ID_DT(
    b_so_id_dt number,b_ma_dvi out varchar2,b_so_id out number,b_ngay number:=30000101)
as
begin
-- Nam - Tra so id cuoi qua so_id_dt
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_nongvn_dvi where so_id_dt=b_so_id_dt;
if b_so_id<>0 then b_so_id:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay); end if;
end;
/
create or replace procedure FBH_NONGVN_SO_ID_DTd(
    b_so_id_dt number,b_ma_dvi out varchar2,b_so_id out number)
as
begin
-- Nam - Tra so id dau qua so_id_dt
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_nongvn_dvi where so_id_dt=b_so_id_dt;
if b_so_id<>0 then b_so_id:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id); end if;
end;
/
create or replace procedure FBH_NONGVN_SO_ID_GCN(
    b_gcn varchar2,b_ma_dvi out varchar2,b_so_id out number,b_so_id_dt out number,b_ngay number:=30000101)
as
begin
-- Nam - Tra so id cuoi qua so_id_dt
select count(*) into b_so_id from bh_nongvn_dvi where gcn=b_gcn;
if b_so_id<>1 then
    b_ma_dvi:=''; b_so_id:=0; b_so_id_dt:=0;
else
    select ma_dvi,so_id,so_id_dt into b_ma_dvi,b_so_id,b_so_id_dt from bh_nongvn_dvi where gcn=b_gcn;
    b_so_id:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
end if;
end;
/
create or replace procedure FBH_NONGVN_SO_ID_GCNd(
    b_gcn varchar2,b_ma_dvi out varchar2,b_so_id out number,b_so_id_dt out number,b_ngay number:=30000101)
as
begin
-- Nam - Tra so id dau qua so_id_dt
...
if b_so_id<>1 then
    ...
    select ma_dvi,so_id,so_id_dt into b_ma_dvi,b_so_id,b_so_id_dt from bh_nongvn_dvi where gcn=b_gcn;
    b_so_id:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
end if;
end;
/
create or replace function FBH_NONGVN_NT_TIEN(
...
b_so_idB:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select nvl(min(nt_tien),'VND') into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_idB;
return b_kq;
end;
/
create or replace function FBH_NONGVN_NT_PHI(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return varchar2
as
    b_kq varchar2(5); b_so_idB number;
begin
...
b_so_idB:=FBH_NONGVN_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select nvl(min(nt_phi),'VND') into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_idB;
return b_kq;
end;
/
create or replace function FBH_NONGVN_CAP(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Nam - Tra ngay cap
select min(ngay_cap) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_NONGVN_NGAY(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Nam - Tra nhap
select nvl(min(ngay_ht),0) into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_NONGVN_SO_IDp(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return number
as
    b_kq number;
begin
-- Nam - Tra so_idP
select min(so_idP) into b_kq from bh_nongvn_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_NONGVN_TTRANG(
    b_ma_dvi varchar2,b_so_id number,b_dk varchar2:='K',b_ngay number:=30000101) return varchar2
as
    b_kq varchar2(1); b_i1 number;
begin
-- Nam - Tra ngay cap
select nvl(min(ttrang),'X') into b_kq from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_NONGVN_SO_ID_KTRA(b_ma_dvi varchar2,b_so_id_g number,b_loi out varchar2)
AS
    b_i1 number;
begin
-- Nam - Kiem tra SO_ID moi nhap
if b_so_id_g=0 then b_loi:=''; return; end if;
select count(*) into b_i1 from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id_g and ttrang<>'D';
if b_i1<>0 then b_loi:='loi:Hop dong cu da xoa hoac chua ky:loi'; return; end if;
select count(*) into b_i1 from bh_nongvn where ma_dvi=b_ma_dvi and so_id_g=b_so_id_g and kieu_hd<>'K';
if b_i1<>0 then b_loi:='loi:Hop dong cu da co bo sung, sua doi:loi'; return; end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace function FBH_NONGVN_MA_SDBS(b_so_id number) return nvarchar2
as
    b_kq nvarchar2(200);
begin
-- Nam - Tra so id dau
select FKH_JS_GTRIs(FKH_JS_BONH(txt),'ma_sdbs') into b_kq from bh_nongvn_txt where  so_id=b_so_id and loai='dt_ct';
return b_kq;
end;
/
CREATE OR REPLACE procedure PBH_NONGVN_BS(
...
select JSON_ARRAYAGG(json_object(ngay_ht,so_hd,gcn) order by ngay_ht desc,so_hd desc,gcn desc returning clob) into cs_lke from
        (select distinct a.ngay_ht,a.so_hd,b.gcn from bh_nongvn a,bh_nongvn_dvi b
...
/
create or replace procedure PBH_NONGVN_LKE(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_nongvn where ma_dvi=b_ma_dvi and 
    ...
    select JSON_ARRAYAGG(json_object(ma_dvi,so_id,so_hd,nsd) order by so_hd desc returning clob) into cs_lke from
        (select  ma_dvi,so_id,so_hd,nsd,rownum sott from bh_nongvn where
    ...
    select count(*) into b_dong from bh_nongvn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(json_object(ma_dvi,so_id,so_hd,nsd) order by so_hd desc returning clob) into cs_lke from
        (select ma_dvi,so_id,so_hd,nsd,rownum sott from bh_nongvn  where
...
/
create or replace procedure PBH_NONGVN_LKE_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
/
create or replace procedure PBH_NONGVN_XOA_XOA(
...
select so_id_kt,ttrang,ksoat into b_so_idK,b_ttrang,b_ksoat
    from bh_nongvn where so_id=b_so_id;
...
select count(*) into b_i1 from bh_nongvn where ma_dvi=b_ma_dvi and so_id_g=b_so_id;
if b_i1<>0 then b_loi:='loi:Khong sua, xoa GCN, hop dong da tao SDBS:loi'; return; end if;
b_loi:='loi:Loi xoa Table bh_nongvn:loi';
delete bh_nongvn_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_nongvn_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_nongvn_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_nongvn_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_nongvn_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_NONGVN_XOA(
...
PBH_NONGVN_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_NONGVN_HDBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_idD:=FBH_NONGVN_SO_IDd(b_ma_dvi,b_so_id);
if b_so_idD<>0 then
    select count(*) into b_dong from bh_nongvn where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
    select JSON_ARRAYAGG(json_object(ngay_ht,so_hd,ttoan,ttrang,ma_dvi,so_id,
           'ma_sdbs' value PKH_TEN_TENl(FBH_NONGVN_MA_SDBS(so_id))
    ) order by so_id desc returning clob)
        into cs_lke from bh_nongvn where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;   
end if;
select json_object('dong' value b_dong,'cs_lke' value cs_lke) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_NONGVN_TIM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if trim(b_cmt||b_mobi||b_email) is not null then
    ...
    for r_lp in (select distinct so_id_d from bh_nongvn where ma_dvi=b_ma_dvi and ma_kh = b_ma_kh and nv=b_nv
        ...
        b_so_idC:=FBH_NONGVN_SO_IDc(b_ma_dvi,r_lp.so_id_d);
        select max(so_hd),max(ngay_ht) into b_so_hd,b_ngay_ht from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_idC;
        insert into ket_qua(c1,c2,c3,c4,n1,n2,n3,n4,n10,c11)
            select b_so_hd,ttrang,ten,cmt,ngay_hl,ngay_kt,ngay_cap,b_ngay_ht,b_so_idC,FBH_NONGVN_MA_SP_TEN(ma_sp)
            from bh_nongvn where ma_dvi=b_ma_dvi and so_id=b_so_idC;
    end loop;
else
    for r_lp in (select distinct so_id_d from bh_nongvn where ma_dvi=b_ma_dvi and nv=b_nv
          and ngay_ht between b_ngayD and b_ngayC and phong=b_phong
          and (b_so_hd = ' ' OR so_hd LIKE '%' || b_so_hd || '%') and b_ma_sp in (' ',ma_sp)
          and b_ttrang in (' ',ttrang) and (b_ten = ' ' OR upper(ten) LIKE '%' || b_ten || '%') order by so_id_d desc) loop
        b_so_idC:=FBH_NONGVN_SO_IDc(b_ma_dvi,r_lp.so_id_d);
