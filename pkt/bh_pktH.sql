create or replace procedure FBH_PKTH_PHI(
    ...
    a_ma_ct out pht_type.a_var,a_kieu out pht_type.a_var,a_ktru out pht_type.a_var,
    a_lkeM out pht_type.a_var,a_lkeP out pht_type.a_var,a_lkeB out pht_type.a_var,a_luy out pht_type.a_var,
...
AS
    b_lenh varchar2(1000); b_i1 number; b_i2 number;
    b_so_id_dt number; b_so_idD number; b_hs number; b_capD number; b_capC number;
    b_kt number:=0; b_ktL number:=0;
    b_nhom varchar2(10); b_ma_sp varchar2(10); b_ktru nvarchar2(500);
    b_ma_nct varchar2(500); b_rru varchar2(1); b_ma_ntb nvarchar2(500);
    b_c_thue varchar2(1); b_kho number; b_tp number:=0; b_tg number:=1;
    ...
    dk_ma pht_type.a_var; dk_tien pht_type.a_num; dk_phiB pht_type.a_num;
    ...
    bs_ma pht_type.a_var; bs_tien pht_type.a_num;
    bs_ptB pht_type.a_num; bs_pt pht_type.a_num; bs_pp pht_type.a_var;

begin
-- Dan - Tinh phi
b_loi:='loi:Loi xu ly FBH_PKTG_PHI:loi';
b_lenh:=FKH_JS_LENH('so_id_dt,kieu_hd,so_hd_g,ngay_kt,ngay_cap,nt_tien,nt_phi,tygia,c_thue,so_idp');
EXECUTE IMMEDIATE b_lenh into b_so_id_dt,b_kieu_hd,b_so_hdG,b_ngay_kt,b_ngay_cap,b_nt_tien,b_nt_phi,b_tygia,b_c_thue,b_so_idP using dt_ct;
...
b_lenh:=FKH_JS_LENH('ma,tien,ptb,pt');
EXECUTE IMMEDIATE b_lenh bulk collect into bs_ma,bs_tien,bs_ptB,bs_pt using dt_dkbs;
...
if b_so_idG<>0 then
    ...
    select so_id,ngay_cap bulk collect into a_so_idG,a_ngay_capG from bh_pkt where
    ...
    if a_so_idG.count<>0 then
        ...
        for b_lpG in 1..a_so_idG.count loop
            select nvl(min(ngay_hl),0),nvl(min(ngay_kt),0) bulk collect into a_ngay_hlG,a_ngay_ktG from bh_pkt_dvi where
        ...
        for b_lpG in 1..a_so_idG.count loop
            ...
            --nampb: ma_dvi=b_ma_dvi and so_id=a_so_idG(b_lpG) and so_id_dt=b_so_id_dt and tc='C';
                ma_dvi=b_ma_dvi and so_id=a_so_idG(b_lpG) and so_id_dt=b_so_id_dt;
...
/
create or replace procedure PBH_PKTH_PHI(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(3000); b_lenh varchar2(2000); b_i1 number;
    dt_ctH clob; dt_ct clob; dt_dk clob; dt_dkbs clob; dt_pvi clob;
    ...
    a_ma_ct pht_type.a_var; a_kieu pht_type.a_var; a_ktru pht_type.a_var;
    a_lkeM pht_type.a_var; a_lkeP pht_type.a_var; a_lkeB pht_type.a_var; a_luy pht_type.a_var; 
...
b_lenh:=FKH_JS_LENHc('dt_ctH,dt_ct,dt_dk,dt_dkbs,dt_pvi');
EXECUTE IMMEDIATE b_lenh into dt_ctH,dt_ct,dt_dk,dt_dkbs,dt_pvi using b_oraIn;
FKH_JS_NULL(dt_ctH); FKH_JS_NULL(dt_ct); FKH_JSa_NULL(dt_dk); FKH_JSa_NULL(dt_dkbs); FKH_JSa_NULL(dt_pvi);
...
if b_so_id_dt>100000 and b_kieu_hd in('S','B') and b_gcnG<>' ' then
    select count(*) into b_i1 from bh_pkt_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcnG;
...
FBH_PKTH_PHI(b_ma_dvi,dt_ct,dt_dk,dt_dkbs,dt_pvi,b_so_idP,
    a_ma,a_ten,a_tc,a_ma_ct,a_kieu,a_ktru,a_lkeM,a_lkeP,a_lkeB,a_luy,a_ma_dk,a_ma_dkC,
...
for b_lp in 1..a_ma.count loop
    ...
    'ma_ct' value a_ma_ct(b_lp),'kieu' value a_kieu(b_lp),'ktru' value a_ktru(b_lp),'lkeM' value a_lkeM(b_lp),'lkeP' value a_lkeP(b_lp),'lkeB' value a_lkeB(b_lp),
...
/
create or replace procedure PBH_PKTH_MO(
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_pkt_sp a,(select distinct ma_sp from bh_pkt_phi where nhom in('H') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
    where a.ma=b.ma_sp and FBH_PKT_SP_HAN(a.ma)='C';
select json_object('cs_sp' value cs_sp returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PKTH_MOd(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraOut out clob)
AS
...
/
create or replace procedure PBH_PKTH_CT(
...
select json_object(so_hd,ma_kh,so_dt) into dt_ct from bh_pkt where ma_dvi=b_ma_dvi and so_id=b_so_id;
select txt into ds_ct from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ct';
...
select txt into ds_dk from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dk';
select txt into ds_pvi from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_pvi';
select count(*) into b_i1 from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
if b_i1=1 then
    select txt into ds_dkbs from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
end if;
select count(*) into b_i1 from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
if b_i1=1 then
    select txt into ds_lt from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
end if;
select count(*) into b_i1 from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
if b_i1=1 then
    select txt into ds_kbt from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
end if;
select count(*) into b_i1 from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
if b_i1=1 then
    select txt into ds_ttt from bh_pkt_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
end if;
...
/
create or replace procedure PBH_PKTH_TESTd(
    dt_ctH clob,dt_ct in out clob,dt_dk clob,dt_dkbs clob,dt_pvi clob,
    b_ma_dvi varchar2,b_nsd varchar2,b_ttrang varchar2,b_kieu_hd varchar2,b_ma_sp varchar2,
    ...
    b_dvi out nvarchar2,b_ddiem out nvarchar2,b_kvuc out varchar2,b_cdt out varchar2,
    b_tdx out number,b_tdy out number,b_bk out number,b_dk_lut out varchar2,b_hs_lut out number,

    b_ma_cct out varchar2,b_ma_nct out varchar2,b_ma_dkdl out varchar2,b_ma_dktc out varchar2,
    b_rru out varchar2,b_ma_ntb out varchar2,b_tgian out number,b_bhanh out number,

    b_gio_hl out varchar2,b_ngay_hl out number,b_gio_kt out varchar2,b_ngay_kt out number,b_ngay_cap out number,b_so_idP out number,
...
AS
    ...
    b_nhom varchar2(1); b_ma_spR varchar2(10); b_so_idPn number;
    a_thay pht_type.a_num;
begin
-- Dan - Nhap
b_loi:='loi:Loi xu ly PBH_PKTH_TESTd:loi';
b_lenh:=FKH_JS_LENH('so_id_dt,gcn,gcn_g,ngay_hl,ngay_kt,dvi,ddiem,tdx,tdy,dk_lut,hs_lut,ma_cct,ma_dkdl,ma_dktc,
                tgian,bhanh,so_idp,gio_hl,gio_kt,cdtc,cdtf,cdtx');
EXECUTE IMMEDIATE b_lenh into
    b_so_id_dt,b_gcn,b_gcnG,b_ngay_hl,b_ngay_kt,b_dvi,b_ddiem,b_tdx,b_tdy,b_dk_lut,b_hs_lut,b_ma_cct,b_ma_dkdl,b_ma_dktc,
    b_tgian,b_bhanh,b_so_idPn,b_gio_hl,b_gio_kt,b_cdtC,b_cdtF,b_cdtX using dt_ct;
b_gcnG:=nvl(trim(b_gcnG),' ');
...
if b_gio_hl=' ' then b_gio_hl:='0'; end if;
if b_gio_kt=' ' then b_gio_kt:='0'; end if;
b_ma_cct:=PKH_MA_TENl(b_ma_cct); b_ma_dkdl:=PKH_MA_TENl(b_ma_dkdl); b_ma_dktc:=PKH_MA_TENl(b_ma_dktc);
PBH_PKT_BPHI_TSO(dt_ct,b_nhom,b_ma_spR,b_ma_nct,b_rru,b_ma_ntb,b_i1);
PBH_PKT_BPHI_TSOt(b_ma_spR,b_ma_cct,b_ma_nct,b_ma_dkdl,b_ma_dktc,b_rru,b_ma_ntb,b_loi);
if b_loi is not null then return; end if;
...
if b_gcn=' ' or instr(b_gcn,'.')=2 then
    ...
    if b_kieu_gcn<>'G' then
        select count(*) into b_i1 from bh_pkt_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        if(b_i1>0) then
           select max(REGEXP_SUBSTR(gcn, 'B([0-9]+)', 1, 1, NULL, 1)) into b_i1 from bh_pkt_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        end if;
        b_gcn:=b_gcn||'/'||b_kieu_hd||to_char(b_i1+1);
    end if;
else
    select nvl(max(ngay_cap),b_ngay_capH) into b_ngay_cap from bh_pkt_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcn;
end if;
if trim(b_cdtC) is null then b_cdt:=' ';
else
...
b_lenh:='H,'||b_so_hdG||','||b_ma_sp||','||b_nt_tien||','||b_nt_phi||','||b_c_thue;
PKH_JS_THAYa(b_txt,'nhom,so_hd_g,ma_sp,nt_tien,nt_phi,c_thue',b_lenh);
FBH_PKTH_PHI(b_ma_dvi,b_txt,dt_dk,dt_dkbs,dt_pvi,b_so_idP,
...
if b_ttrang in('T','D') then
    ...
    if b_i1<>0 then
        ...
        PBH_PKTH_KHD(dt_ctH,dt_ct,dt_dk,dt_dkbs,dt_pvi,dt_khd,b_loi);
        if b_loi is not null then return; end if;
    end if;
end if;
b_loi:='';
...
/
create or replace procedure PBH_PKTH_TESTr(
    ...
    dvi_dvi out pht_type.a_nvar,dvi_ddiem out pht_type.a_nvar,dvi_kvuc out pht_type.a_var,
    dvi_cdt out pht_type.a_var,dvi_tdx out pht_type.a_num,dvi_tdy out pht_type.a_num,dvi_bk out pht_type.a_num,
    dvi_dk_lut out pht_type.a_var,dvi_hs_lut out pht_type.a_num,

    dvi_ma_cct out pht_type.a_var,dvi_ma_dt out pht_type.a_var,dvi_ma_dkdl out pht_type.a_var,
    dvi_ma_dktc out pht_type.a_var,dvi_rru out pht_type.a_var,
    dvi_tgian out pht_type.a_num,dvi_bhanh out pht_type.a_num,
...
AS
    ...
    b_c_thue varchar2(1); b_nt_tien varchar2(5); b_nt_phi varchar2(5);
    ...
    b_dvi nvarchar2(500); b_ddiem nvarchar2(500); b_kvuc varchar2(10); b_cdt varchar2(5);
    b_tdx number; b_tdy number; b_bk number; b_dk_lut varchar2(1); b_hs_lut number;
    b_ma_cct varchar2(10); b_ma_nct varchar2(500); b_ma_dkdl varchar2(10);
    b_ma_dktc varchar2(10); b_rru varchar2(1); b_ma_ntb nvarchar2(500); b_tgian number; b_bhanh number;
...
b_lenh:=FKH_JS_LENH('ttrang,kieu_hd,gio_hl,ngay_hl,gio_kt,ngay_kt,ngay_cap,
    ma_sp,thue,ttoan,nt_tien,nt_phi,tygia,c_thue,loai_khh,cmth,mobih,emailh,tenh,dchih');
EXECUTE IMMEDIATE b_lenh into b_ttrang,b_kieu_hd,b_gio_hl,b_ngay_hlH,b_gio_kt,b_ngay_ktH,b_ngay_capH,b_ma_sp,
...
if b_ma_sp<>' ' and FBH_PKT_SP_HAN(b_ma_sp)<>'C' then b_loi:='loi:Het han ma san pham:loi'; return; end if;
...
for ds_lp in 1..a_ds_ct.count loop
    ...
    b_ma_dvi,b_nsd,b_ttrang,b_kieu_hd,b_ma_sp,b_nt_tien,b_nt_phi,b_tygia,b_c_thue,b_ngay_capH,
    b_so_id_dt,b_kieu_gcn,b_gcn,b_gcnG,b_dvi,b_ddiem,b_kvuc,b_cdt,b_tdx,b_tdy,b_bk,b_dk_lut,b_hs_lut,
    b_ma_cct,b_ma_nct,b_ma_dkdl,b_ma_dktc,b_rru,b_ma_ntb,b_tgian,b_bhanh,
    b_gio_hl,b_ngay_hl,b_gio_kt,b_ngay_kt,b_ngay_cap,b_so_idP,
    a_ma,a_ten,a_tc,a_ma_ct,a_kieu,a_lkeM,a_lkeP,a_lkeB,a_luy,a_ktru,a_ma_dk,a_ma_dkC,a_lh_nv,a_t_suat,
    a_cap,a_lbh,a_nv,a_tien,a_pt,a_phi,a_thue,a_ttoan,a_ptB,a_phiB,a_pvi_ma,a_pvi_tc,a_pvi_ktru,b_loi);
    if b_loi is not null then return; end if;
    b_kt_ds:=b_kt_ds+1;
    dvi_so_id(b_kt_ds):=b_so_id_dt; dvi_kieu_gcn(b_kt_ds):=b_kieu_gcn; dvi_gcn(b_kt_ds):=b_gcn; dvi_gcnG(b_kt_ds):=b_gcnG;
    dvi_dvi(b_kt_ds):=b_dvi; dvi_ddiem(b_kt_ds):=b_ddiem; dvi_kvuc(b_kt_ds):=' ';
    dvi_cdt(b_kt_ds):=b_cdt; dvi_tdx(b_kt_ds):=b_tdx; dvi_tdy(b_kt_ds):=b_tdx; dvi_bk(b_kt_ds):=b_bk;
    dvi_dk_lut(b_kt_ds):=b_dk_lut; dvi_hs_lut(b_kt_ds):=b_hs_lut;
    --lay doi tuong
    if b_ma_nct<>' ' then dvi_ma_dt(b_kt_ds):=b_ma_nct;
    elsif b_ma_ntb<>' ' then dvi_ma_dt(b_kt_ds):=b_ma_ntb;
    else dvi_ma_dt(b_kt_ds):=' ';
    end if;
    dvi_ma_cct(b_kt_ds):=b_ma_cct; dvi_ma_dkdl(b_kt_ds):=b_ma_dkdl;
    dvi_ma_dktc(b_kt_ds):=b_ma_dktc; dvi_rru(b_kt_ds):=b_rru;
    dvi_tgian(b_kt_ds):=b_tgian;  dvi_bhanh(b_kt_ds):=b_bhanh;
    ...
    dvi_phi(b_kt_ds):=b_phi; dvi_thue(b_kt_ds):=b_thue; dvi_ttoan(b_kt_ds):=b_ttoan; dvi_so_idP(b_kt_ds):=b_so_idP;
    for b_lp in 1..a_ma.count loop
        b_kt_dk:=b_kt_dk+1;
...
if b_kieu_hd in('B','S') and b_so_idG<>0 then
    for r_lp in (select so_id_dt,dvi from bh_pkt_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idG) loop
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_PKTH_TESTr:loi'; end if;
end;
/
create or replace procedure PBH_PKTH_NH_NH(
...
-- Rieng
    ...
    dvi_dvi pht_type.a_nvar,dvi_ddiem pht_type.a_nvar,dvi_kvuc pht_type.a_var,
    dvi_cdt pht_type.a_var,dvi_tdx pht_type.a_num,dvi_tdy pht_type.a_num,dvi_bk pht_type.a_num,
    dvi_dk_lut pht_type.a_var,dvi_hs_lut pht_type.a_num,
    dvi_ma_cct pht_type.a_var,dvi_ma_dt pht_type.a_var,dvi_ma_dkdl pht_type.a_var,
    dvi_ma_dktc pht_type.a_var,dvi_rru pht_type.a_var,
    dvi_tgian pht_type.a_num,dvi_bhanh pht_type.a_num,
...
for b_lp in 1..dvi_so_id.count loop
    insert into bh_pkt_dvi values(b_ma_dvi,b_so_id,dvi_so_id(b_lp),b_lp,dvi_kieu_gcn(b_lp),dvi_gcn(b_lp),dvi_gcnG(b_lp),
        dvi_dvi(b_lp),dvi_kvuc(b_lp),dvi_ddiem(b_lp),dvi_dk_lut(b_lp),dvi_hs_lut(b_lp),
        dvi_cdt(b_lp),dvi_tdx(b_lp),dvi_tdy(b_lp),dvi_bk(b_lp),
        dvi_ma_cct(b_lp),dvi_ma_dt(b_lp),dvi_ma_dkdl(b_lp),
        dvi_ma_dktc(b_lp),dvi_rru(b_lp),dvi_tgian(b_lp),dvi_bhanh(b_lp),
        dvi_gio_hl(b_lp),dvi_ngay_hl(b_lp),dvi_gio_kt(b_lp),dvi_ngay_kt(b_lp),dvi_ngay_cap(b_lp),
        dvi_so_idP(b_lp),dvi_giam(b_lp),dvi_phi(b_lp),dvi_thue(b_lp),dvi_ttoan(b_lp));
    select json_object('ddiem' value dvi_ddiem(b_lp)) into b_txt from dual;
    insert into bh_hd_goc_ttdt values(b_ma_dvi,b_so_id,dvi_so_id(b_lp),'PKT',dvi_dvi(b_lp),b_ma_kh,dvi_ngay_kt(b_lp),b_txt);
end loop;
for b_lp in 1..dk_so_id.count loop
    if dk_lh_nv(b_lp)<>' ' and dk_kieu(b_lp)='T' then b_tien:=b_tien+dk_tien(b_lp); end if;
    insert into bh_pkt_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),b_lp,
        ...
        dk_lbh(b_lp),dk_nv(b_lp),dk_ktru(b_lp),dk_pvi_ma(b_lp),dk_pvi_tc(b_lp),dk_pvi_ktru(b_lp));
end loop;
insert into bh_pkt values(
    ...
    b_nt_tien,b_nt_phi,b_c_thue,b_ma_sp,b_so_dt,b_tien,
...
if dt_hk is not null then
    insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_ct',ds_ct);
insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_dk',ds_dk);
insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_pvi',ds_pvi);
if ds_dkbs is not null then
    insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_dkbs',ds_dkbs);
end if;
if ds_lt is not null then
    insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_lt',ds_lt);
end if;
if ds_kbt is not null then
    insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_kbt',ds_kbt);
end if;
if ds_ttt is not null then
    insert into bh_pkt_txt values(b_ma_dvi,b_so_id,'ds_ttt',ds_ttt);
end if;
...
if b_ttrang in('T','D') then
    for b_lp in 1..lt_so_id.count loop
        select JSON_ARRAYAGG(json_object(
            ma,ten,tc,ma_ct,tien,pt,phi,cap,ma_dk,ma_dkC,lh_nv,t_suat,thue,ttoan,
            ptB,ptG,phiG,lkeM,lkeP,lkeB,luy,pvi_ma,pvi_tc,pvi_ktru)
            order by bt returning clob) into b_txt
            from bh_pkt_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=lt_so_id(b_lp);
        insert into bh_pkt_kbt values(b_ma_dvi,b_so_id,lt_so_id(b_lp),b_txt,lt_lt(b_lp),lt_kbt(b_lp));
    end loop;
    for b_lp in 1..dvi_dvi.count loop
        if dvi_tdx(b_lp)<>0 and dvi_tdy(b_lp)<>0 and dvi_bk(b_lp)<>0 then
            insert into bh_pkt_ttu values(b_ma_dvi,b_so_idD,dvi_so_id(b_lp),dvi_dvi(b_lp),
    ...
    select json_object('ma_dvi' value b_ma_dvi,'nsd' value b_nsd,'so_id' value b_so_id,'so_hd' value b_so_hd,
        ...
        'ma_kh' value b_ma_kh,'ten' value b_ten,'kieu_gt' value b_kieu_gt,
        'ma_gt' value b_ma_gt,'so_id_d' value b_so_idD,'so_id_g' value b_so_idG,
        'ttrang' value b_ttrang,'dvi_ksoat' value b_ma_dvi,'ksoat' value b_nsd,'bangg' value 'bh_pkt',
...
if b_ttrang='D' and b_kieu_hd<>'U' then
    for b_lp in 1..dvi_so_id.count loop
        ...
        insert into bh_hd_goc_ttindt values(
            b_ma_dvi,b_so_id,dvi_so_id(b_lp),'PKT',dvi_dvi(b_lp),b_ma_kh,dvi_ngay_kt(b_lp),dvi_ddiem(b_lp),b_ma_ke);
    end loop;
end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
...
/
create or replace procedure PBH_PKTH_NH(
...
AS
    ...
    ds_pvi clob; ds_lt clob; ds_kbt clob; ds_ttt clob; dt_kytt clob;
-- Chung
...
PBH_PKTH_TESTr(
    ...
    b_ma_sp,
    dvi_so_id,dvi_kieu_gcn,dvi_gcn,dvi_gcnG,dvi_dvi,dvi_ddiem,dvi_kvuc,
    dvi_cdt,dvi_tdx,dvi_tdy,dvi_bk,dvi_dk_lut,dvi_hs_lut,
    dvi_ma_cct,dvi_ma_dt,dvi_ma_dkdl,dvi_ma_dktc,dvi_rru,dvi_tgian,dvi_bhanh,

    dvi_gio_hl,dvi_ngay_hl,dvi_gio_kt,dvi_ngay_kt,dvi_ngay_cap,
    dvi_phi,dvi_thue,dvi_ttoan,dvi_giam,dvi_so_idP,
