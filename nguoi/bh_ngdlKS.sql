create or replace procedure PBH_NGDL_KHD(dt_ct clob,dt_khd clob,b_loi out varchar2)
as
...
/
create or replace procedure PBH_NGDL_KHD_GIOI(b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
	b_gioi varchar2(1);
begin
-- Dan - Kiem soat GIOI
...
/
create or replace procedure PBH_NGDL_KHD_KTUOI(
    b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
    b_ng_sinh number; b_ngay_hl number; b_tuoi number;
    b_tu number:=0; b_den number:=200*365; -- max ngay tuoi
    a_ch pht_type.a_var;
begin
--nampb: kiem soat khoang tuoi theo D M Y
...
b_ngay_hl := FKH_JS_GTRIn(dt_ct,'ngay_hl');
b_tuoi:=nvl(FKH_KHO_NGSO(b_ng_sinh,b_ngay_hl),0)+1;
PKH_CH_ARR(b_nd, a_ch, '-');
b_tu:=FBH_KHO_KNGAY(a_ch(1));
if a_ch.count > 1 then
    b_den:=FBH_KHO_KNGAY(a_ch(2));
end if;
if b_tuoi between b_tu and b_den then
    b_loi := '';
end if;
exception when others then null; 
end;
/
create or replace procedure PBH_NGDL_KHD_TGT(b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
...
/
create or replace procedure PBH_NGDL_KHD_TGD(b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
  b_ngay_hl number; b_ngay_kt number; b_kho number;
begin
...
/
create or replace procedure PBH_NGDL_KHD_SNT(b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
  b_so_dt  number;
begin
...
/
create or replace procedure PBH_NGDL_KHD_SND(b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
  b_so_dt  number;
begin
