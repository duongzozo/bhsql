...
if trim(b_ma_dvi) is null or b_so_id=0 then
    b_oraOut:='';
else
    b_so_idB:=FBH_SKN_SO_IDb(b_ma_dvi,b_so_id);
    if b_so_idB=0 then b_loi:='loi:Hop dong nguyen tac da xoa:loi'; raise PROGRAM_ERROR; end if; 
    select dsach into b_oraOut from bh_skN where ma_dvi=b_ma_dvi and so_id=b_so_idB;
end if;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
CREATE OR REPLACE procedure PBH_SKN_BS
    (b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select JSON_ARRAYAGG(json_object(ngay_ht,so_hd) order by ngay_ht desc,so_hd) into cs_lke
    from bh_skN where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
select json_object('cs_lke' value cs_lke) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_SKN_SO_ID(
...
b_so_id:=FBH_SKN_SO_ID(b_ma_dvi,b_so_hd,b_nv);
...
/
create or replace procedure PBH_SKN_MO(
...
/
create or replace procedure PBH_SKN_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select json_object(so_hd,ma_kh,'tpa' value FBH_MA_GDINH_TENl(tpa) returning clob) into dt_ct from bh_skN where ma_dvi=b_ma_dvi and so_id=b_so_id;
select txt into dt_nh from bh_skN_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_nh';
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_skN_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
...
/
create or replace procedure PBH_SKN_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_skN where ma_dvi=b_ma_dvi and 
        ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_skN where
    ...
    select count(*) into b_dong from bh_skN where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_skN  where
...
/
create or replace procedure PBH_SKN_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hd:=FBH_SKN_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_skN where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
    ...
    select count(*) into b_dong from bh_skN where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_skN where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv order by so_hd)
        where so_hd>=b_so_hd;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_skN  where
...
/
create or replace procedure PBH_SKN_XOA_XOA(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_nh varchar2,b_loi out varchar2)
AS
    b_nsdC varchar(20); b_i1 number;
begin
-- Dan - Xoa
select nvl(min(trim(nsd)),' ') into b_nsdC from bh_skN where so_id=b_so_id;
if b_nsdC=' ' then b_loi:=''; return;
elsif b_nsd<>b_nsdC then b_loi:='loi:Khong sua, xoa so lieu nguoi khac:loi'; return;
elsif b_nh='X' then
    select count(*) into b_i1 from bh_skN where so_id_g=b_so_id;
    if b_i1<>0 then b_loi:='loi:Khong xoa hop dong dang co tham chieu:loi'; return; end if;
end if;
b_loi:='loi:Loi xoa Table bh_skN:loi';
delete bh_skN_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_skN where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_SKN_TESTr(
    b_ma_dvi varchar2,b_nsd varchar2,dt_ct in out clob,dt_nh clob,
    b_ma_sp out varchar2,b_cdich out varchar2,b_tpaH out varchar2,b_dsach out varchar2,b_loi out varchar2)
AS
    ...
    nh_dt_ct pht_type.a_clob;
    nh_so_id pht_type.a_num;nh_nhom pht_type.a_var;nh_ten pht_type.a_nvar;
    nh_goi pht_type.a_var;nh_so_idP pht_type.a_num;nh_tpa pht_type.a_var;nh_phi pht_type.a_num;
    nh_so_dt pht_type.a_num;nh_luong pht_type.a_num;
begin
-- Dan - Nhap
b_lenh:=FKH_JS_LENH('ma_sp,cdich,tpa,ngay_hl');
EXECUTE IMMEDIATE b_lenh into b_ma_sp,b_cdich,b_tpaH,b_ngay_hl using dt_ct;
...
b_dsach:=FBH_SK_SP_DSACH(b_ma_sp);
b_lenh:=FKH_JS_LENHc('dt_nh_ct');
EXECUTE IMMEDIATE b_lenh bulk collect into nh_dt_ct using dt_nh;
...
/
create or replace procedure PBH_SKN_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
-- Rieng
    b_ma_sp varchar2(10):=' '; b_cdich varchar2(10):=' '; b_tpaH varchar2(500):=' ';
    b_so_dt number:=0; b_tien number:=0; b_dsach varchar2(1); b_cdt varchar2(1);
begin
-- Dan - Nhap
...
FKH_JS_NULL(dt_ct);
if b_so_id<>0 then
    select count(*) into b_i1 from bh_skN where so_id=b_so_id;
    if b_i1<>0 then
        select kieu_hd,ttrang into b_kieu_hd,b_ttrang from bh_skN
        ...
        PBH_SKN_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,b_kieu_hd,b_loi);
        if b_loi is not null then raise PROGRAM_ERROR; end if;
    end if;
else
...
PBH_HD_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,b_i1,'bh_skN',dt_ct,'',
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_SKN_TESTr(b_ma_dvi,b_nsd,dt_ct,dt_nh,b_ma_sp,b_cdich,b_tpaH,b_dsach,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_loi:='loi:Loi Table bh_sk:loi';
insert into bh_skN values(
    ...
    b_ngay_cap,b_nt_tien,b_nt_phi,b_c_thue,b_ma_sp,b_cdich,b_tpaH,'K',b_so_dt,b_dsach,
...
insert into bh_skN_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_skN_txt values(b_ma_dvi,b_so_id,'dt_nh',dt_nh);
...
/
create or replace procedure PBH_SKN_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob)
AS
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_SKN_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
commit;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
