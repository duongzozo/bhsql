...
create or replace procedure PBH_NGTDC_TESTr(
    ...
    b_ma_sp out varchar2, b_cdich out varchar2,b_goi out varchar2,b_so_vay out number,
...
begin
-- Dan - Nhap
b_lenh:=FKH_JS_LENH('ttrang,ngay_hl,ngay_kt,loai_kh,ng_sinh,gioi,tend,
    dchid,cmtd,mobid,emaild,ng_sinhd,gioid,ma_sp,cdich,goi,ttoan,so_vay');
EXECUTE IMMEDIATE b_lenh into b_ttrang,b_ngay_hl,b_ngay_kt,b_loai_khM,b_ng_sinhM,b_gioiM,b_ten,b_dchi,b_cmt,b_mobi,b_email,
    b_ng_sinh,b_gioi,b_ma_sp,b_cdich,b_goi,b_ttoanH,b_so_vay using dt_ct;
...
b_so_vay:=nvl(b_so_vay,0);
if trim(b_ten) is null then b_i1:=b_ng_sinhM; else b_i1:=b_ng_sinh; end if;
b_so_idP:=FBH_NGTD_BPHI_SO_IDh('C',b_ma_sp,b_cdich,b_goi,b_i1,b_ngay_hl);
...
if b_so_vay=0 then b_loi:='loi:Nhap so tien vay:loi'; return; end if;
if trim(dt_dkbs) is not null then
    ...
    for b_lp in 1..dkB_ma.count loop
        ...
        dk_ptB(b_kt):=dkB_ptB(b_lp); dk_phiB(b_kt):=dkB_phiB(b_lp);
    end loop;
end if;
for b_lp in 1..b_kt loop
...
if b_ttrang in('T','D') then
    ...
    select count(*) into b_i1 from bh_ngtd_phi_txt where so_id=b_so_idP and loai='dt_khd';
    if b_i1<>0 then
        select txt into dt_khd from bh_ngtd_phi_txt where so_id=b_so_idP and loai='dt_khd';
        b_i1:=length(dt_khd)-2;
        dt_khd:=substr(dt_khd,2,b_i1);
        PBH_NGTD_KHD(dt_ct,dt_khd,b_loi);
        if b_loi is not null then return; end if;
    end if;
end if;
b_loi:='';
...
/
create or replace procedure PBH_NGTDC_NH_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,
    dt_ct in out clob,dt_dk clob,dt_dkbs clob,dt_ttt clob,dt_hk clob,
    ...
    b_ma_sp varchar2,b_cdich varchar2,b_goi varchar2,b_so_vay number,b_so_idP number,
...
AS
    b_lenh varchar2(1000); b_i1 number; b_ps varchar2(1);
    b_so_id_kt number:=-1; b_tien number:=0; b_ma_ke varchar2(20):=' ';
    dt_lt clob; dt_khd clob; dt_kbt clob; dt_cho clob; dt_bvi clob; b_txt clob;
begin
-- Dan - Nhap
b_loi:='loi:Loi Table BH_ngtd:loi';
if b_ttrang='D' then b_so_id_kt:=0; end if;
if b_ma_khD in(' ','VANGLAI') then
    --duchq bo truong cdt
    insert into bh_ngtd_ds values(b_ma_dvi,b_so_id,b_so_id,0,b_kieu_hd,b_so_hd,b_so_hd_g,
        b_ten,b_ng_sinh,b_gioi,b_cmt,b_mobi,b_email,b_dchi,
        b_ng_huong,b_gio_hl,b_ngay_hl,b_gio_kt,b_ngay_kt,b_ngay_cap,
        b_ma_sp,b_cdich,b_goi,b_so_vay,b_so_idP,b_phi,b_giam,b_ttoan,b_ma_kh);
else
   --duchq bo truong cdt
    insert into bh_ngtd_ds values(b_ma_dvi,b_so_id,b_so_id,0,b_kieu_hd,b_so_hd,b_so_hd_g,
        b_tenD,b_ng_sinhD,b_gioiD,b_cmtD,b_mobiD,b_emailD,b_dchiD,
        b_ng_huong,b_gio_hl,b_ngay_hl,b_gio_kt,b_ngay_kt,b_ngay_cap,b_ma_sp,b_cdich,b_goi,b_so_vay,
        b_so_idP,b_phi,b_giam,b_ttoan,b_ma_khD);
end if;
for b_lp in 1..dk_lh_nv.count loop
    if dk_lh_nv(b_lp)<>' ' and dk_kieu(b_lp)='T' then b_tien:=b_tien+dk_tien(b_lp); end if;
    insert into bh_ngtd_dk values(b_ma_dvi,b_so_id,b_so_id,b_lp,
...
insert into bh_ngtd_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_ngtd_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if trim(dt_hk) is not null then
    insert into bh_ngtd_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if trim(dt_ttt) is not null then
    insert into bh_ngtd_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if trim(dt_dkbs) is not null then
    insert into bh_ngtd_txt values(b_ma_dvi,b_so_id,'dt_dkbs',dt_dkbs);
end if;
for b_lp in 1..tt_ngay.count loop
    insert into bh_ngtd_tt values(b_ma_dvi,b_so_id,tt_ngay(b_lp),tt_tien(b_lp));
end loop;
-- viet anh
PBH_NGTD_BPHI_CTk(b_so_idP,dt_lt,dt_khd,dt_kbt,dt_cho,dt_bvi);
if trim(dt_khd) is not null then
    insert into bh_ngtd_kbt values(b_ma_dvi,b_so_id,b_so_idD,'dt_khd',dt_khd);
end if;
if trim(dt_kbt) is not null then
    insert into bh_ngtd_kbt values(b_ma_dvi,b_so_id,b_so_idD,'dt_kbt',dt_kbt);
end if;
if trim(dt_cho) is not null then
    insert into bh_ngtd_kbt values(b_ma_dvi,b_so_id,b_so_idD,'dt_cho',dt_cho);
end if;
if trim(dt_bvi) is not null then
    insert into bh_ngtd_kbt values(b_ma_dvi,b_so_id,b_so_idD,'dt_bvi',dt_bvi);
end if;
if b_ttrang in('T','D') then
    ...
    insert into bh_ng values(
        b_ma_dvi,b_so_id,b_so_hd,b_ngay_ht,'TDC',b_ttrang,b_kieu_hd,b_so_hd_g,b_kieu_kt,b_ma_kt,b_kieu_gt,b_ma_gt,b_ma_cb,
...
/
--duchq update length email
create or replace procedure PBH_NGTDC_NH(
...
AS
    ...
    dt_ct clob; dt_dk clob; dt_dkbs clob; dt_ttt clob; dt_kytt clob; dt_hk clob; 
...
-- Ra rieng
    ...
    b_ma_sp varchar2(10); b_cdich varchar2(10); b_goi varchar2(200); b_so_vay number;
...
b_lenh:=FKH_JS_LENHc('dt_ct,dt_dk,dt_dkbs,dt_ttt,dt_kytt,dt_hk,dt_cho,dt_bvi');
EXECUTE IMMEDIATE b_lenh into dt_ct,dt_dk,dt_dkbs,dt_ttt,dt_kytt,dt_hk,dt_cho,dt_bvi using b_oraIn;
FKH_JS_NULL(dt_ct); FKH_JSa_NULL(dt_ttt); FKH_JSa_NULL(dt_kytt); FKH_JSa_NULL(dt_hk);
FKH_JSa_NULL(dt_cho); FKH_JSa_NULL(dt_bvi);
if b_so_id<>0 then
    select count(*) into b_i1 from bh_ngtd where so_id=b_so_id;
    if b_i1<>0 then
        select ngay_ht,kieu_hd,ttrang into b_ngay_htC,b_kieu_hd,b_ttrang from bh_ngtd
        ...
        PBH_NGTD_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,b_kieu_hd,b_loi);
        if b_loi is not null then raise PROGRAM_ERROR; end if;
    end if;
else
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_HD_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,b_ngay_htC,'bh_ngtd',dt_ct,dt_kytt,
...
PBH_NGTDC_TESTr(
    ...
    --duchq bo truong b_cdt
    b_ma_sp,b_cdich,b_goi,b_so_vay,b_phi,b_ttoan,b_so_idP,
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_NGTDC_NH_NH(
    b_ma_dvi,b_nsd,b_so_id,dt_ct,dt_dk,dt_dkbs,dt_ttt,dt_hk,
    ...
    --duchq bo truong b_cdt
    b_ma_sp,b_cdich,b_goi,b_so_vay,b_so_idP,
...
/
create or replace procedure PBH_NGTDC_MO(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraOut out clob)
AS
    b_loi varchar2(1000); b_ngay number:=PKH_NG_CSO(sysdate);
    cs_sp clob; cs_cdich clob; cs_goi clob; cs_ttt clob; cs_tpa clob; cs_khd clob; cs_kbt clob;
begin
-- Dan
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','NG','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
select JSON_ARRAYAGG(json_object(ma,ten) order by ten) into cs_sp from bh_ngtd_sp where FBH_NGTD_SP_HAN(ma)='C';
...
select JSON_ARRAYAGG(json_object(ma,ten) order by ma) into cs_goi from bh_ngtd_goi where FBH_NGTD_GOI_HAN(ma)='C';
select JSON_ARRAYAGG(json_object(ma,ten) order by ten) into cs_tpa from
...
select json_object('cs_sp' value cs_sp,'cs_cdich' value cs_cdich,'cs_goi' value cs_goi, 'cs_tpa' value cs_tpa, 'cs_ttt' value cs_ttt,
                   'cs_khd' value cs_khd,'cs_kbt' value cs_kbt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_NGTDC_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_so_idP number; b_tpa nvarchar2(500);
    dt_ct clob; dt_dk clob; dt_dkbs clob; dt_lt clob; dt_khd clob; dt_kbt clob; dt_ttt clob; dt_kytt clob; 
...
b_so_idP:=FBH_NGTD_SO_IDp(b_ma_dvi,b_so_id);
-- viet anh
select FKH_JS_GTRIs(FKH_JS_BONH(txt),'tpa') into b_tpa from bh_ngtd_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
select json_object(so_hd,ma_kh,'tpa' value FBH_DTAC_MA_TENl(b_tpa)) into dt_ct from bh_ngtd where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
select JSON_ARRAYAGG(json_object(ngay,tien) order by ngay) into dt_kytt from bh_ngtd_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
PBH_NGTD_BPHI_CTk(b_so_idP,dt_lt,dt_khd,dt_kbt,dt_cho,dt_bvi);
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob)
    into dt_txt from bh_ngtd_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
select count(*) into b_i1 from bh_ngtd_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_ngtd_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
...
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,'so_idP' value b_so_idP,
    ...
    'dt_kbt' value dt_kbt,'dt_ttt' value dt_ttt,'dt_kytt' value dt_kytt,'dt_hk' value dt_hk,
  'dt_ct' value dt_ct,'dt_cho' value dt_cho,'dt_bvi' value dt_bvi,'txt' value dt_txt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
