create or replace function FBH_NGB_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Dan - Tra so hop dong
select min(so_hd) into b_kq from bh_ngB where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_NGB_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2,b_nv varchar2:='') return number
as
    b_kq number;
begin
-- Dan - Tra so id
if nvl(b_nv,' ')=' ' then 
    select nvl(min(so_id),0) into b_kq from bh_ngB where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
else 
    select nvl(min(so_id),0) into b_kq from bh_ngB where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv;
end if;
return b_kq;
end;
/
create or replace procedure PBH_NGB_LKE
...
if b_klk='N' then
    select count(*) into b_dong from bh_ngB where ma_dvi=b_ma_dvi and 
        ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_ngB where
    ...
    select count(*) into b_dong from bh_ngB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_ngB  where
...
/
create or replace procedure PBH_NGB_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_phong varchar2(10); b_so_hd varchar2(20);
    b_so_id number; b_ngay_ht number; b_nv varchar2(10);
...
b_so_hd:=FBH_NGB_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ngB where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
    ...
    select count(*) into b_dong from bh_ngB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ngB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv order by so_hd)
        where so_hd>=b_so_hd;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_ngB  where
...
/
create or replace procedure PBH_NGB_SO_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_id:=FBH_NGB_SO_ID(b_ma_dvi,b_so_hd,b_nv);
...
/
-- nam
create or replace procedure PBH_NGB_XOA_XOA(
...
begin
-- Dan - Xoa
b_loi:='loi:Loi xu ly PBH_NGB_XOA_XOA:loi';
select count(*) into b_i1 from bh_ngB where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1>0 then
  select nsd into b_nsdC from bh_ngB where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
b_loi:='loi:Loi xoa Table bh_ngB:loi';
if b_lan = 0 then
    delete bh_ngB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
else
    delete bh_ngB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan;
end if;
delete bh_ngB_ds where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ngB_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ngB where ma_dvi=b_ma_dvi and so_id=b_so_id;
PBH_BAO_NV_XOA(b_ma_dvi,b_nsd,b_so_id,b_nh,b_loi);
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_NGB_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob)
AS
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
--delete bh_ngB_ls where ma_dvi=b_ma_dvi and so_id=b_so_id;
PBH_NGB_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',0,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
commit;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_NGB_CHD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_ma_dvi varchar2(10); b_so_id number; b_lan number;
    b_i1 number; b_dsach varchar2(1);
begin
-- Dan - Chuyen hop dong
...
if b_so_id=0 or b_lan=0 then
    b_oraOut:='';
else
    select nvl(max(lan),0) into b_i1 from bh_ngB where ma_dvi=b_ma_dvi and so_id=b_so_id;
    if b_lan < b_i1 then b_loi:='loi:Khong tao hop dong tu bao gia cu:loi'; raise PROGRAM_ERROR; end if; 
    select dsach into b_oraOut from bh_ngB where ma_dvi=b_ma_dvi and so_id=b_so_id;
end if;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_NGB_HDBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_id<>0 then
    select JSON_ARRAYAGG(json_object( ma_dvi,so_id,lan,'ngay_ht' value FKH_JS_GTRIn(txt,'ngay_ht') ,'ttoan' value FKH_JS_GTRIn(txt,'ttoan'),
        ...
        from bh_ngB_ls where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
end if;
