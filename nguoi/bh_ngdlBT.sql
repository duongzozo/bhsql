create or replace procedure PBH_NGDLBT_CTbg(
...
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,
  'dt_ct' value dt_ct,'dt_ds' value dt_ds,'dt_nh' value dt_nh,
  'dt_giam' value dt_giam,'dt_kytt' value dt_kytt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_NGDLBT_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(2000); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_lan number;
...
select count(*) into b_i1 from bh_ngB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ds';
if b_i1=1 then
    select txt into dt_ds from bh_ngB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ds';
end if;
...
/
create or replace procedure PBH_NGDLBT_TESTr(
...
AS
    ...
    b_kieu_hd varchar2(1); b_ttrang varchar2(1); b_so_dt number;
    ...
    dk_tc pht_type.a_var; dk_ma_ct pht_type.a_var; dk_pt pht_type.a_num; 
    dk_thue pht_type.a_num; dk_ttoan pht_type.a_num; dk_cap pht_type.a_num; 
    dk_ma_dk pht_type.a_var; dk_t_suat pht_type.a_num; dk_ptB pht_type.a_num; 
    dk_phiG pht_type.a_num; dk_lkeP pht_type.a_var; dk_lkeB pht_type.a_var; 
...
begin
-- Dan - Nhap
b_lenh:=FKH_JS_LENH('ttrang,kieu_hd,ngay_hl,ngay_kt,ttoan,ngay_cap,so_dt');
EXECUTE IMMEDIATE b_lenh into b_ttrang,b_kieu_hd,b_ngay_hl,b_ngay_kt,b_ttoanH,b_ngay_cap,b_so_dt using dt_ct;
...
b_lenh:=FKH_JS_LENH('nhom,ten,loai,kvuc,ma_sp,cdich,goi,ngay_hl,ngay_kt,phi,cdt');
for b_lp in 1..nh_dt_ct.count loop
    EXECUTE IMMEDIATE b_lenh into nh_nhom(b_lp),nh_ten(b_lp),
        ...
        nh_ngay_hl(b_lp),nh_ngay_kt(b_lp),nh_phi(b_lp),nh_cdt(b_lp) using nh_dt_ct(b_lp);
    ...
    nh_cdich(b_lp):=nvl(trim(nh_cdich(b_lp)),' ');
    ...
    if nh_so_idP(b_lp)=0 then b_loi:='loi:Khong tim duoc bieu khi nhom '||nh_ten(b_lp)||':loi'; return; end if;
end loop;
...
/
create or replace procedure PBH_NGDLBT_NH_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_lan out number,
    dt_ct in out clob,dt_ds clob,dt_nh clob,dt_giam clob,dt_kytt clob,
    ...
    gcn_ma_kh pht_type.a_var,gcn_cdt pht_type.a_var,gcn_ma_sp pht_type.a_var,
...
forall b_lp in 1..gcn_so_id.count
    insert into bh_ngB_ds values(b_ma_dvi,b_so_id,gcn_so_id(b_lp),gcn_ten(b_lp),gcn_so_idP(b_lp),gcn_ma_sp(b_lp),gcn_ngay_hl(b_lp),
...
insert into bh_ngB values(
    b_ma_dvi,b_so_id,b_lan,b_so_hd,b_ngay_ht,'DLT',b_ttrang,b_kieu_hd,b_so_hd_g,
...
insert into bh_ngB_ls (ma_dvi,so_id,lan,loai,txt) select ma_dvi,so_id,lan,loai,txt from bh_ngB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id  and lan=b_lan;
...
/
create or replace procedure PBH_NGDLBT_NH(
...
-- Rieng
    ...
    dk_so_id pht_type.a_num; dk_ma pht_type.a_var; dk_ten pht_type.a_nvar; dk_kieu pht_type.a_var;
    dk_tien pht_type.a_num; dk_phi pht_type.a_num; 
...
PBH_BG_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,'bh_ngdl',dt_ct,dt_kytt,
