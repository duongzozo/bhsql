create or replace procedure PBH_HOPBG_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_lan number;
    dt_ct clob:=''; dt_hk clob:=''; dt_dk clob:=''; dt_dkbs clob:=''; dt_lt clob:=''; dt_kbt clob:=''; dt_kytt clob; dt_ttt clob:='';
begin
-- Nam - Xem
...
select txt into dt_dk from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dk';
select count(*) into b_i1 from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dkbs';
if b_i1<>0 then
  select txt into dt_dkbs from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dkbs';
end if;
select count(*) into b_i1 from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_lt';
if b_i1<>0 then
  select txt into dt_lt from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_lt';
end if;
select count(*) into b_i1 from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kbt';
if b_i1<>0 then
    select txt into dt_kbt from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kbt';
end if;
...
select count(*) into b_i1 from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ttt';
end if;
...
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,
  'dt_lt' value dt_lt,'dt_kbt' value dt_kbt,'dt_hk' value dt_hk,'dt_kytt' value dt_kytt,'dt_ttt' value dt_ttt,'dt_ct' value dt_ct,
  'dt_dk' value dt_dk,'dt_dkbs' value dt_dkbs returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_HOPBG_CTbg(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_lan number;
...
select count(1) into b_i1 from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ct';
if b_i1 > 0 then
  select txt into dt_ct from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ct';
  select txt into dt_dk from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dk';
  select count(*) into b_i1 from bh_hopB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dkbs';
...
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,'dt_hk' value dt_hk,
  'dt_lt' value dt_lt,'dt_kbt' value dt_kbt,'dt_kytt' value dt_kytt,'dt_ttt' value dt_ttt,'dt_ct' value dt_ct,
  'dt_dk' value dt_dk,'dt_dkbs' value dt_dkbs returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_HOPBG_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(2000); b_lan number;
    dt_ct clob; dt_dk clob; dt_dkbs clob; dt_lt clob; dt_kbt clob; dt_hk clob; dt_kytt clob; dt_ttt clob;
--  Chung
...
--  Rieng
    ...
    b_ma_sp varchar2(10); b_cdich varchar2(10); b_goi varchar2(10); b_dtuong nvarchar2(500); b_so_idP number;
...
b_lenh:=FKH_JS_LENHc('dt_ct,dt_dk,dt_dkbs,dt_lt,dt_kbt,dt_hk,dt_kytt,dt_ttt');
EXECUTE IMMEDIATE b_lenh into dt_ct,dt_dk,dt_dkbs,dt_lt,dt_kbt,dt_hk,dt_kytt,dt_ttt using b_oraIn;
FKH_JS_NULL(dt_ct); FKH_JSa_NULL(dt_dk); FKH_JSa_NULL(dt_dkbs); FKH_JSa_NULL(dt_hk);
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_HOPBG_NH_NH(
    b_ma_dvi,b_nsd,b_so_id,dt_ct,dt_hk,dt_dk,dt_dkbs,dt_lt,dt_kbt,dt_kytt,dt_ttt,
...
/
create or replace procedure PBH_HOPBG_NH_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,
    dt_ct in out clob,dt_hk clob,dt_dk clob,dt_dkbs clob,dt_lt clob,dt_kbt clob,dt_kytt clob, dt_ttt clob,
...
insert into bh_hopB values(
    b_ma_dvi,b_so_id,b_lan,b_so_hd,b_ngay_ht,'HOP','G',b_ttrang,b_kieu_hd,b_so_hd_g,
    b_phong,b_ma_kh,b_ten,b_ngay_hl,b_ngay_kt,b_ma_sp,b_nt_tien,b_tien,b_nt_phi,b_phi,' ',' ',b_nsd,sysdate);
insert into bh_hopB_ds values(b_ma_dvi,b_so_id,b_so_id,b_ten,b_ma_sp,b_ngay_hl,b_ngay_kt);
...
insert into bh_hopB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_dk',dt_dk);
insert into bh_hopB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_dkbs',dt_dkbs);
if dt_hk is not null then
  insert into bh_hopB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_hk',dt_hk);
end if;
if dt_ttt is not null then
  insert into bh_hopB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_ttt',dt_ttt);
end if;
if dt_lt is not null then
    insert into bh_hopB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_lt',dt_lt);
end if;
if dt_kbt is not null then
    insert into bh_hopB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_kbt',dt_kbt);
end if;
