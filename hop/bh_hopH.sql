create or replace procedure PBH_HOPH_MO(
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_hop_sp a,(select distinct ma_sp from bh_hop_phi where nhom='H' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_cdich from
    bh_ma_cdich a,(select distinct cdich from bh_hop_phi where nhom='H' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_goi from
    bh_hop_goi a,(select distinct goi from bh_hop_phi where nhom='H' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
/
create or replace procedure PBH_HOPH_MOd(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraOut out clob)
AS
    b_loi varchar2(1000); b_ngay number:=PKH_NG_CSO(sysdate);
...
/
create or replace procedure PBH_HOPH_CT(
...
select txt into ds_ct from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ct';
select txt into ds_dk from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dk';
select count(*) into b_i1 from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
if b_i1=1 then
    select txt into ds_dkbs from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
end if;
select count(*) into b_i1 from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
if b_i1=1 then
    select txt into ds_lt from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
end if;
select count(*) into b_i1 from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
if b_i1=1 then
    select txt into ds_kbt from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
end if;
select count(*) into b_i1 from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
if b_i1=1 then
    select txt into ds_ttt from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
end if;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_hop_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
...
/
create or replace procedure PBH_HOPH_NH_NH(
...
for b_lp in 1..dk_lh_nv.count loop
    ...
    insert into bh_hop_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),b_lp,
...
insert into bh_hop values(
        b_ma_dvi,b_so_id,b_so_hd,b_ngay_ht,'HOP','H',b_ttrang,b_kieu_hd,b_so_hd_g,b_kieu_kt,b_ma_kt,b_kieu_gt,
        b_ma_gt,b_ma_cb,b_phong,b_so_hdL,b_loai_kh,b_ma_kh,b_ten,b_dchi,b_cmt,b_mobi,b_email,b_gio_hl,
        b_ngay_hl,b_gio_kt,b_ngay_kt,b_ngay_cap,b_nt_tien,b_nt_phi,b_c_thue,b_so_dt,b_ma_sp,b_cdich,b_goi,b_tien,b_phi,
        b_giam,b_thue,b_ttoan,b_hhong,b_so_idG,b_so_idD,'','',b_so_id_kt,b_nsd,sysdate);
for b_lp in 1..dvi_so_id.count loop
  insert into bh_hop_ds values(
        ...
        dvi_gio_kt(b_lp),dvi_ngay_kt(b_lp),dvi_ngay_cap(b_lp),dvi_so_idP(b_lp),
...
insert into bh_hop_txt values(b_ma_dvi,b_so_id,'ds_ct',ds_ct);
insert into bh_hop_txt values(b_ma_dvi,b_so_id,'ds_dk',ds_dk);
if dt_hk is not null then
    insert into bh_hop_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if ds_dkbs is not null then
    insert into bh_hop_txt values(b_ma_dvi,b_so_id,'ds_dkbs',ds_dkbs);
end if;
if dt_ttt is not null then
    insert into bh_hop_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if ds_lt is not null then
    insert into bh_hop_txt values(b_ma_dvi,b_so_id,'ds_lt',ds_lt);
end if;
if ds_kbt is not null then
    insert into bh_hop_txt values(b_ma_dvi,b_so_id,'ds_kbt',ds_kbt);
end if;
...
if b_kieu_hd<>'U' and b_ttrang in('T','D') then
    for b_lp in 1..lt_so_id.count loop
        select JSON_ARRAYAGG(json_object(
            ...
            from bh_hop_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=lt_so_id(b_lp);
        if trim(lt_lt(b_lp)) is not null then b_txt_lt:=lt_lt(b_lp); else b_txt_lt:=ds_lt; end if;
        insert into bh_hop_kbt values(b_ma_dvi,b_so_id,lt_so_id(b_lp),b_txt,b_txt_lt,lt_kbt(b_lp));
    end loop;
...
if b_ttrang='D' and b_kieu_hd<>'U' then
    for b_lp in 1..dvi_so_id.count loop
        ...
        insert into bh_hd_goc_ttindt values(
            b_ma_dvi,b_so_id,dvi_so_id(b_lp),'HOP',dvi_dtuong(b_lp),b_ma_kh,dvi_ngay_kt(b_lp),'',b_ma_ke);
    end loop;
end if;
b_loi:='';
...
/
create or replace procedure PBH_HOPH_NH(
...
PBH_HOPH_NH_NH(
    ...
    dk_lh_nv,dk_t_suat,dk_ptB,dk_ptG,dk_phiG,dk_lkeM,dk_lkeP,dk_lkeB,dk_luy,dk_lh_bh,
    lt_so_id,lt_lt,lt_kbt,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
/
create or replace procedure PBH_HOPH_TESTr(
    ...
    dvi_dtuong out pht_type.a_nvar,
...
AS
    ...
    b_dtuong nvarchar2(500); b_gio_hl varchar2(50);
...
EXECUTE IMMEDIATE b_lenh into b_ttrang,b_kieu_hd,b_gio_hlH,b_ngay_hlH,b_gio_ktH,b_ngay_ktH,b_ngay_capH,b_ma_sp,b_cdich,b_goi,
    b_thueH,b_ttoanH,b_nt_tien,b_nt_phi,b_tygia,b_c_thue,b_loai_khH,b_cmtH,b_mobiH,b_emailH,b_tenH,b_dchiH using dt_ct;
if b_ma_sp<>' ' and FBH_HOP_SP_HAN(b_ma_sp)<>'C' then b_loi:='loi:Sai ma san pham:loi'; return; end if;
...
for ds_lp in 1..a_ds_ct.count loop
    ...
    dvi_dtuong(ds_lp):=b_dtuong; dvi_ngay_hl(ds_lp):=b_ngay_hl;
...
if b_kieu_hd in('B','S') and b_so_idG<>0 then
    for r_lp in (select so_id_dt,dtuong from bh_hop_ds where ma_dvi=b_ma_dvi and so_id=b_so_idG) loop
...
/
create or replace procedure PBH_HOPH_TESTd(
    ...
    b_ma_sp varchar2,b_cdich varchar2,b_goi varchar2,b_nt_tien varchar2, b_nt_phi varchar2,b_tygia number,b_c_thue varchar2,
    ...
    b_dtuong out nvarchar2,b_gio_hl out varchar2,b_ngay_hl out number,b_gio_kt out varchar2,b_ngay_kt out varchar2,
...
AS
    ...
    b_lenh varchar2(2000); b_txt clob; b_tp number:=0;
...
begin
-- Nam - Nhap
b_loi:='loi:Loi xu ly PBH_HOPH_TESTd:loi';
b_lenh:=FKH_JS_LENH('so_id_dt,gcn,gcn_g,ngay_hl,ngay_kt,gio_hl,gio_kt,dtuong,so_idp,cdtc,cdtf,cdtx');
EXECUTE IMMEDIATE b_lenh into
    b_so_id_dt,b_gcn,b_gcnG,b_ngay_hl,b_ngay_kt,b_gio_hl,b_gio_kt,b_dtuong,b_so_idPn,b_cdtC,b_cdtF,b_cdtX using dt_ct;
...
if b_so_id_dt<100000 then
    ...
    select count(*) into b_i1 from bh_hop_ds where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcnG;
...
if b_gcn=' ' or instr(b_gcn,'.')=2 then
    ...
    if b_kieu_gcn<>'G' then
        select count(*) into b_i1 from bh_hop_ds where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        if(b_i1>0) then
           select max(REGEXP_SUBSTR(gcn, 'B([0-9]+)', 1, 1, NULL, 1)) into b_i1 from bh_hop_ds where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        end if;
        b_gcn:=b_gcn||'/'||b_kieu_hd||to_char(b_i1+1);
    end if;
else
    select nvl(max(ngay_cap),b_ngay_capH) into b_ngay_cap from bh_hop_ds where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcn;
end if;
...
b_so_idP:=FBH_HOP_BPHI_SO_ID('H',b_ma_sp,b_cdich,b_goi,b_ngay_hl);
