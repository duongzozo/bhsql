create or replace procedure PBH_2B_IN_GCN(
...
select count(*) into b_i1 from bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dkbs';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_bs FROM bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dkbs';
  if dt_bs <> '""' then PKH_JS_THAYa(dt_ct,'bs_lb',N'Điều khoản bổ sung:'); end if;
end if;
-- lay dt_dk
select count(*) into b_i1 from bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dk';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_dk FROM bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dk';
    ...
    for b_lp in 1..a_dk_ma.count loop
        if a_dk_ma(b_lp) = 'TNDS_BB.1' THEN
            ...
            PKH_JS_THAY(dt_ct,'nntx_so_nguoi', a_dk_tien(b_lp) || N' người');
        end if;
...
-- lay dt_lt
select count(*) into b_i1 from bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_lt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_lt FROM bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_lt';
end if;

-- lay dt_kbt
select count(*) into b_i1 from bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_kbt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_kbt FROM bh_2b_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_kbt';
  if dt_kbt <> '""' then
...
if  b_i1 = 1 then
    select min(ngay) into b_ngay_tt from bh_2b_tt where ma_dvi = b_ma_dvi and so_id = b_so_id;
     PKH_JS_THAY(dt_ct,'ngay_tt',FBH_IN_CSO_NG(b_ngay_tt,'DD/MM/YYYY'));
...
select count(*) into b_i1 from bh_2b_ds where ma_dvi = b_ma_dvi and so_id = b_so_id;
if b_i1 > 1 then 
    b_loi := N'loi:Sai mẫu in HĐ 1 xe:loi';
    raise PROGRAM_ERROR; 
end if;
select xe_id into b_xe_id from bh_2b_ds where ma_dvi = b_ma_dvi and so_id = b_so_id;
