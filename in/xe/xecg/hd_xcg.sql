create or replace procedure PBH_XE_IN_HD(
...
if b_so_id = 0 then
    select so_id into b_so_id from bh_xe where ma_dvi = b_ma_dvi and so_hd = b_so_hd;
end if;

select bien_xe,so_id_dt into b_bien_xe,b_so_id_dt from bh_xe_ds where ma_dvi = b_ma_dvi and so_id = b_so_id 
    and (
        (trim(b_gcn) is not null and gcn = b_gcn) or
        (trim(b_gcn) is null and rownum = 1)
    );
...
-----dt_ds
select count(*) into b_i1 from bh_xe_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ds_txt';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_ds FROM bh_xe_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_ds_txt';
...
if dt_ct <> '""' then
    ...
    b_temp_var:= FBH_IN_SUBSTR(b_temp_var,'|','T');
    if trim(b_temp_var) is not null THEN
        select ten into b_temp_nvar from bh_xe_pb where ma = b_temp_var 
            and hieu = FBH_IN_SUBSTR( FKH_JS_GTRIs(dt_ct ,'hieu'),'|','T') and hang = FBH_IN_SUBSTR( FKH_JS_GTRIs(dt_ct ,'hang'),'|','T');
...
SELECT '{' || LISTAGG('"' || ma || '": ""', ',') WITHIN GROUP (ORDER BY ma) || '}' into b_kh_ttt FROM bh_kh_ttt WHERE nv = 'XE' AND ps = 'HD';
...
if b_i1 <> 0 then
PKH_JS_THAY(dt_ct,'hu_x',' ');b_hu := true;
...
select xe_id into b_xe_id from bh_xe_ds where ma_dvi = b_ma_dvi and so_id = b_so_id and so_id_dt = b_so_id_dt;
