create or replace function fbh_in_line_business(b_nv IN VARCHAR2)
return varchar2
is
    v_result   varchar2(50);
begin
    case b_nv
        when 'PHH' then v_result := 'Property';
        when 'PKT' then v_result := 'Engineering';
        when 'HANG' then v_result := 'Cargo';
        when 'TAU' then v_result := 'Marine hull';
        when 'PTN' then v_result := 'Liability';
        when '2B' then v_result := 'Motorcycle';
        when 'XE' then v_result := 'Motor';
        when 'HOP' then v_result := 'Miscellaneous';
        when 'NONG' then v_result := 'Agricultural';
        else v_result := 'unknown quarter';
    end case;
    return v_result;
exception
    when others then
...
END;
/
create or replace function fbh_in_get_quarter_text(p_date_str IN VARCHAR2)
return nvarchar2
is
    v_date     date;
    v_quarter  number;
...
begin
    ...
    v_quarter := to_number(to_char(v_date, 'Q'));
    v_year := to_char(v_date, 'YYYY');
    case v_quarter
        when 1 then v_result := '1st quarter, ' || v_year;
        when 2 then v_result := '2nd quarter, ' || v_year;
        when 3 then v_result := '3rd quarter, ' || v_year;
        when 4 then v_result := '4th quarter, ' || v_year;
        else v_result := 'unknown quarter';
    end case;
    return v_result;
exception
...
END;
/
create or replace procedure PBH_IN_TAI_SOATREATY_DS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(10):= '';
    b_ma_dvi varchar2(10);
    b_ma_login varchar2(10) :=FKH_JS_GTRIs(b_oraIn,'ma_login');
    b_so_id number:=FKH_JS_GTRIs(b_oraIn,'so_id');
    b_i1 number := 0;b_kieu varchar2(10):= '';
    dt_ds clob;
begin
select count(*) into b_i1 from ht_ma_nsd where ma_login = b_ma_login;
if b_i1 <> 0 THEN
    select ma_dvi into b_ma_dvi from ht_ma_nsd where ma_login = b_ma_login;
end if;

select count(*) into b_i1 from tbh_dc where so_id_dc = b_so_id and ma_dvi = b_ma_dvi;
if b_i1 <> 0 THEN
    select kieu into b_kieu from tbh_dc where so_id_dc = b_so_id and ma_dvi = b_ma_dvi;
    if b_kieu <> 'C' then 
        b_loi:= N'loi:Sai mẫu:loi';
        return;
    end if;
end if;
select count(*) into b_i1 from tbh_dc_pt where so_id_dc = b_so_id and ma_dvi = b_ma_dvi;
if b_i1 <> 0 THEN
        select JSON_ARRAYAGG(json_object('so_id_ta_hd' value so_id_ta_hd returning clob) returning clob) into dt_ds
            from  (select so_id_ta_hd from tbh_dc_pt where so_id_dc = b_so_id and ma_dvi = b_ma_dvi group by so_id_ta_hd);
end if;
select json_object('dt_ds' value dt_ds,'so_id' value to_char(b_so_id),'ma_dvi' value b_ma_dvi returning clob) into b_oraOut from dual;
...
/
create or replace procedure PBH_IN_TAI_SOATREATY(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    -- chung
    ...
    b_so_id number:=FKH_JS_GTRIs(b_oraIn,'so_id');
    b_so_id_ta_hd number:=FKH_JS_GTRIs(b_oraIn,'so_id_ta_hd');
    b_nv varchar2(10);b_nt_tra varchar2(10);
  --
...
if b_i1 <> 0 then
    ...
    if b_i1 <> 0 then
        ...
        if b_i1 = 1 THEN
            ...
            if b_i2 = 1 THEN
                PKH_JS_THAY(dt_ct,'commission_pt',N'Various value');
            else
    ...
    select sum(tai) into b_i1 
    from
    (select dp.so_id_hd,dp.tai from BH_BT_HS_DP dp
    left join bh_bt_hs bt on bt.so_id_hd = dp.so_id_hd
    left join tbh_ghep_pbo pbo on pbo.so_id_hd = dp.so_id_hd
    where pbo.so_id_ta_hd = b_so_id_ta_hd and  bt.ttrang = 'T' and dp.ma_nt = b_nt_tra and dp.nv= b_nv and dp.ksoat <> '0' 
        and TO_DATE(dp.ngay_dp, 'YYYYMMDD') BETWEEN TRUNC(TO_DATE(b_temp_var, 'DD/MM/YYYY'), 'Q') AND TO_DATE(b_temp_var, 'DD/MM/YYYY') group by dp.so_id_hd,dp.tai);
