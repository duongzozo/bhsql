create or replace procedure PBH_TNNN_IN_GCN(
...
AS
  ...
  --
    ...
    dt_ct clob; dt_bs clob; dt_ds clob; dt_dk clob; dt_lt clob;dt_kbt clob; dt_ttt clob;dt_dong clob;dt_hk clob;
    b_kh_ttt clob;dt_qt clob;dt_tt clob;b_dvi clob;dt_hu clob;dt_dkbs clob; ds_ct clob;dt_dd clob;
...
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ct FROM bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
    ...
    PKH_JS_THAYa(dt_ct,'ngay_cap_s',N'Ngày ' || FBH_IN_CSO_NG(b_i1,'DD') || N' tháng ' || FBH_IN_CSO_NG(b_i1,'MM')
    ...
    select sum(pt) into b_i1 from bh_ptnnn_dk where ma_dvi = b_ma_dvi and so_id = b_so_id and lh_nv is not null and lh_bh = 'C';
...
--ds_ct
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_ct';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_ct FROM bh_ptnnn_txt t WHERE t.ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_ct';
  if ds_ct <> '[""]' then
    b_lenh:=FKH_JS_LENHc('');
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using ds_ct;
    dt_ds:= a_clob(1); 
    
    b_i1:= FKH_JS_GTRIn(dt_ds ,'ngay_hoi');
...
--ttt
select count(*) into b_i1 from bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_ttt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ttt FROM bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_ttt';
  if dt_ttt <> '""' then
...
select count(*) into b_i1 from bh_ptnnn_tt where ma_dvi = b_ma_dvi and so_id = b_so_id;

if  b_i1 <> 0 then
    b_i2 := 1;
    for r_lp in (select ngay,tien from bh_ptnnn_tt where ma_dvi = b_ma_dvi and so_id = b_so_id order by ngay asc)
    loop
    ...
    if b_i1 = 1 then
      ...
      select min(ngay) into b_i2 from bh_ptnnn_tt where ma_dvi = b_ma_dvi and so_id = b_so_id;
...
--end dong 
--dt_hk
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_hk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_hk FROM bh_ptnnn_txt t WHERE t.ma_dvi = b_ma_dvi and t.so_id = b_so_id AND t.loai='dt_hk';
...
-- lay dt_dk
select count(*) into b_i1 from bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dk';
if b_i1 <> 0 then
    delete temp_1;delete temp_4;commit;
    SELECT FKH_JS_BONH(t.txt) INTO dt_dk FROM bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dk';
    if dt_dk <> '""' then
...
-- lay dt_dkbs
delete temp_1;delete temp_4;commit;
select count(*) into b_i1 from bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dkbs';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_bs FROM bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dkbs';
...
-- lay dt_lt
select count(*) into b_i1 from bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_lt';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_lt FROM bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_lt';
...
--dt_kbt
select count(*) into b_i1 from bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_kbt';
if b_i1 <> 0 then
  b_temp_nvar:= '';
  SELECT FKH_JS_BONH(t.txt) INTO dt_kbt FROM bh_ptnnn_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_kbt';
  if dt_kbt <> '""' then
    ...
    for b_lp in 1..a_kbt_ma_dk.count loop
              ...
              select ten into b_temp_nvar from bh_ptnnn_dk where so_id = b_so_id and ma = a_kbt_ma_dk(b_lp);
                   
              b_i2:= 0;
              for b_lp2 in 1..a_kbt_ma.count loop
                if a_kbt_ma(b_lp2) = 'KVU' then
                    if INSTR(UPPER(b_temp_nvar), N'TÀI SẢN') <> 0 then
                      b_temp_nvar:= N'Đối với Tài sản' || FBH_MKT(a_kbt_nd(b_lp2),b_nt_tien);
      ...
      if b_i2 = 0 then
        b_ktru := b_ktru || CHR(10) ||  N'Đối với người: Khônɡ áp dụng' ;
      end if;
...
/
--HD TNNN----
create or replace procedure PBH_TNNN_IN_HD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    -- chung
    b_loi varchar2(100); b_lenh varchar2(1000);
...
--ds_dk
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_dk FROM bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dk';
end if;
--ds_dkbs
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dkbs';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_dkbs FROM bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dkbs';
end if;
--ds_lt
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_lt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_lt FROM bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_lt';
end if;
--ds-kbt
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_kbt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_kbt FROM bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_kbt';
end if;
--dt_ct
select count(*) into b_i1 from bh_ptnnn_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
if b_i1 <> 0 then
...
if b_i1 <> 0 then
  ...
  if ds_ct <> '[""]' then
    ...
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using ds_ct;
    dt_ds:= a_clob(1);
    PKH_JS_THAY_D(dt_ct,'ddiem',FKH_JS_GTRIs(dt_ds ,'dtuong') );
    PKH_JS_THAY_D(dt_ct,'dchib',FKH_JS_GTRIs(dt_ds ,'dchib') );
    PKH_JS_THAY_D(dt_ct,'mota',FKH_JS_GTRIs(dt_ds ,'mota') );
    PKH_JS_THAY_D(dt_ct,'so_dt',FKH_JS_GTRIn(dt_ds ,'so_dt') );
