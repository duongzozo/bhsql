create or replace procedure PBH_TNCC_IN_GCN(
...
AS
  ...
  --
    ...
    dt_ct clob; ds_ct clob; dt_bs clob; dt_ds clob; dt_dk clob; dt_lt clob;dt_kbt clob; dt_ttt clob;dt_dong clob;
    ...
    b_ktru nvarchar2(4000);
    b_dvit varchar2(1); b_dvit_text varchar2(50); b_kq varchar2(200);
    b_ghan_d varchar2(1);
    b_ghan_d_text nvarchar2(200);
...
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ct FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
    ...
    b_dvit := FKH_JS_GTRIs(dt_ct,'dvit');
    if b_dvit = 'D' then
    b_dvit_text := N'đồng';
    elsif b_dvit = 'S' then
        b_dvit_text := N'người';
    elsif b_dvit = 'C' then
        b_dvit_text := N'chỗ ngồi';
    elsif b_dvit = 'P' then
        b_dvit_text := N'phòng';
    elsif b_dvit = 'M' then
        b_dvit_text := N'm2';
    ...
    b_kq := FBH_CSO_TIEN(b_i1,' ') || b_dvit_text;
    PKH_JS_THAY(dt_ct,'dthu',b_kq );
    
    b_ghan_d := FKH_JS_GTRIs(dt_ct,'ghan_d');

    case b_ghan_d
        when 'V' then b_ghan_d_text := N'Việt Nam';
        when 'T' then b_ghan_d_text := N'Toàn cầu';
        when 'L' then b_ghan_d_text := N'Loại trừ Mỹ và Canada';
    end case;

    PKH_JS_THAY(dt_ct,'dly',b_ghan_d_text);
    ...
    select sum(pt) into b_i1 from bh_ptncc_dk where ma_dvi = b_ma_dvi and so_id = b_so_id and lh_nv is not null and lh_bh = 'C';
    PKH_JS_THAY(dt_ct,'tlp', FBH_TO_CHAR(b_i1));
    ...
    PKH_JS_THAYa(dt_ct,'ng_dbh',N'NGƯỜI ĐƯỢC BẢO HIỂM');
...
--ds_ct
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_ct';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_ct FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_ct';
  b_i1:= FKH_JS_GTRIn(dt_ct ,'ngay_hoi');
    if b_i1 = 30000101 then PKH_JS_THAYa(dt_ct,'ngay_hoi',' '); 
    else PKH_JS_THAYa(dt_ct,'ngay_hoi',FBH_IN_CSO_NG(b_i1,'DD/MM/YYYY') );end if;
end if;
    -- nguoi dai dien
...
--ttt
select count(*) into b_i1 from bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_ttt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ttt FROM bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_ttt';
  if dt_ttt <> '""' then
...
select count(*) into b_i1 from bh_ptncc_tt where ma_dvi = b_ma_dvi and so_id = b_so_id;

if  b_i1 <> 0 then
    b_i2 := 1;
    for r_lp in (select ngay,tien from bh_ptncc_tt where ma_dvi = b_ma_dvi and so_id = b_so_id order by ngay asc)
    loop
      insert into temp_4(C1) values(N'-   Kỳ ' || b_i2 || '/' || b_i1 || N': Ngày thanh toán ' || FBH_IN_CSO_NG(r_lp.ngay,'DD/MM/YYYY') || N', số tiền thanh toán ' || FBH_CSO_TIEN(r_lp.tien,b_nt_tien) );
    b_i2:= b_i2 + 1;
    end loop;
    ---
    if b_i1 = 1 then
      b_temp_var:= FKH_JS_GTRIs(dt_ct ,'ngay_hl');
      select min(ngay) into b_i2 from bh_ptncc_tt where ma_dvi = b_ma_dvi and so_id = b_so_id;
...
if b_i1 <> 0 then
  ...
  loop
      b_temp_nvar:= N'NGƯỜI BẢO HIỂM THỨ ' || FBH_IN_SO_CHU(b_i2) || ': ' || r_lp.ten;
...
--end dong
--dt_hk
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_hk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_hk FROM bh_ptncc_txt t WHERE t.ma_dvi = b_ma_dvi and t.so_id = b_so_id AND t.loai='dt_hk';
...
-- lay dt_dk
select count(*) into b_i1 from bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dk';
if b_i1 <> 0 then
    delete temp_1;delete temp_4;commit;
    SELECT FKH_JS_BONH(t.txt) INTO dt_dk FROM bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dk';
    if dt_dk <> '""' then
      ...
      for b_lp in 1..a_dk_ma.count loop
          ...
          b_temp_nvar:= b_temp_nvar || ': ' || FBH_CSO_TIEN(a_dk_tien(b_lp),b_nt_tien);
          insert into temp_4(cl1) values(b_temp_nvar);
      end loop;
      
      select JSON_ARRAYAGG(json_object('ten' VALUE c2) returning clob) into dt_qt from temp_1;
      select JSON_ARRAYAGG(json_object('ten' VALUE cl1) returning clob) into dt_dk from temp_4;
    end if;
end if;

-- lay dt_dkbs
delete temp_1;delete temp_4;commit;
select count(*) into b_i1 from bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dkbs';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_bs FROM bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_dkbs';
...
-- lay dt_lt
select count(*) into b_i1 from bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_lt';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_lt FROM bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_lt';
...
--dt_kbt
select count(*) into b_i1 from bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_kbt';
if b_i1 <> 0 then
  b_temp_nvar:= '';
  SELECT FKH_JS_BONH(t.txt) INTO dt_kbt FROM bh_ptncc_txt t WHERE  t.so_id = b_so_id AND t.loai='dt_kbt';
  if dt_kbt <> '""' then
    ...
    for b_lp in 1..a_kbt_ma_dk.count loop
              ...
              select ten into b_temp_nvar from bh_ptncc_dk where so_id = b_so_id and ma = a_kbt_ma_dk(b_lp);
                ...
                if a_kbt_ma(b_lp2) = 'KVU' then
                    if INSTR(UPPER(b_temp_nvar), N'TÀI SẢN') <> 0 then
                      ...
                      if trim(a_kbt_nd(b_lp2)) is null then
                        b_temp_nvar := N'Đối với Người: Không áp dụng';
                      else
      ...
      if b_i2 = 0 then
        b_ktru := b_ktru || CHR(10) ||  N'Đối với Người: Không áp dụng' ;
      end if;
...
'dt_ttt' value dt_ttt,'dt_lt' value dt_lt,'dt_dong' value dt_dong,'dt_tt' value dt_tt returning clob) into b_oraOut from dual;
...
/
create or replace procedure PBH_TNCC_IN_HD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    -- chung
    b_loi varchar2(100); b_lenh varchar2(1000);
  ...
  --
    ...
    ds_ct clob;ds_dk clob;ds_dkbs clob;ds_lt clob;ds_kbt clob;dt_dd clob;dt_hk clob;
...
--ds_dk
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_dk FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dk';
end if;
--ds_dkbs
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dkbs';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_dkbs FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dkbs';
end if;
--ds_lt
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_lt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_lt FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_lt';
end if;
--ds-kbt
select count(*) into b_i1 from bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_kbt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO ds_kbt FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_kbt';
end if;
...
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ct FROM bh_ptncc_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
    ...
    if b_nt_tien = 'VND' then 
      b_nt_tien:= N'đồng';
      PKH_JS_THAYa(dt_ct,'nt_tien',N'đồng');
    end if;
...
if ds_ct <> '[""]' then
    ...
    PKH_JS_THAY_D(dt_ct,'ddiem',FKH_JS_GTRIs(dt_ds ,'dchib') );
    PKH_JS_THAY_D(dt_ct,'mota',FKH_JS_GTRIs(dt_ds ,'mota') );

    b_dvit := FKH_JS_GTRIs(dt_ds,'dvit');
    if b_dvit = 'D' then
    b_dvit_text := N'đồng';
    elsif b_dvit = 'S' then
        b_dvit_text := N'người';
    ...
    b_i1:= FKH_JS_GTRIn(dt_ds ,'dthu');
    b_kq := FBH_CSO_TIEN(b_i1,' ') || b_dvit_text;
    PKH_JS_THAY(dt_ct,'dthu',b_kq );
    
    b_ghan_d := FKH_JS_GTRIs(dt_ds,'ghan_d');
    case b_ghan_d
        when 'V' then b_ghan_d_text := N'Việt Nam';
        when 'T' then b_ghan_d_text := N'Toàn cầu';
        when 'L' then b_ghan_d_text := N'Loại trừ Mỹ và Canada';
...
if b_i1 <> 0 then
  PKH_JS_THAY(dt_ct,'dbh','C');
  PKH_JS_THAY(dt_ct,'nbh',N'NGƯỜI BẢO HIỂM ĐƯỢC ĐỨNG ĐẦU');
  b_i1:= 0;b_i2:= 1;
  select count(*) into b_i3 from BH_HD_DO_TL where ma_dvi = b_ma_dvi and so_id = b_so_id;
  for r_lp in (select b.ten,a.pt,b.dchi,b.mobi,b.cmt,b.ma_tk,b.nhang,b.ma from BH_HD_DO_TL a, BH_MA_NBH b where a.so_id = b_so_id and a.nha_bh=b.ma)
  loop
...
if ds_dkbs <> '[""]' then
    b_lenh:=FKH_JS_LENHc('');
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using ds_dkbs;
    dt_bs:= a_clob(1);
...
if ds_kbt <> '[""]' then
    ...
    if dt_kbt <> '""' then
      ...
      for b_lp in 1..a_kbt_ma_dk.count loop
              ...
              for b_lp2 in 1..a_kbt_ma.count loop
                if a_kbt_ma(b_lp2) = 'KVU' then
                    if INSTR(UPPER(b_temp_nvar), N'TÀI SẢN') <> 0 then
                      b_temp_nvar:= N'Đối với Tài sản: ' || FBH_MKT(a_kbt_nd(b_lp2),b_nt_tien);
                    elsif INSTR(UPPER(b_temp_nvar), N'NGƯỜI') <> 0 then
                      b_i2:= 1;
                      if trim(a_kbt_nd(b_lp2)) is null then
