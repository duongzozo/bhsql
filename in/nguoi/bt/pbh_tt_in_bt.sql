create or replace procedure PBH_TT_IN_BT(
...
AS
    ...
    b_so_hd number:=FKH_JS_GTRIs(b_oraIn,'so_hd');
    b_gcn varchar2(20):= b_so_hd;
    
    b_temp_clob  clob;
    b_i1 number := 0;
    dt_ct clob; dt_dk clob; dt_hk clob; dt_tba clob; dt_kbt clob;dt_ttt clob;
    dt_grv clob;dt_bvi clob; dt_dk_new clob;
    b_dvi clob;
    ...
    b_kh_ttt clob;
    b_ma_phong varchar2(20);b_ten_phong nvarchar2(500) :=' ';
    b_dvi_ten nvarchar2(500) :=' ';b_dvi_tengon nvarchar2(500) :=' ';
    ...
    --
    b_quy_tac varchar2(200);b_temp_nvar varchar2(500);
    b_ma_sp pht_type.a_var; b_ma_qtac pht_type.a_var;
    --bien mang
    ...
    --a_hk
    a_hk_ma pht_type.a_var;a_hk_ten pht_type.a_nvar;a_hk_tien pht_type.a_num;a_hk_tien_bangchu pht_type.a_nvar;
    a_hk_cmt pht_type.a_nvar;a_hk_so_tk pht_type.a_nvar;a_hk_ma_nh pht_type.a_nvar;
    a_hk_ten_tk pht_type.a_nvar;a_hk_thue pht_type.a_num;a_hk_mobi pht_type.a_nvar;a_hk_email pht_type.a_nvar;
    --tltt,tlpt
...
if b_i1 <> 0 then
  ...
  if b_i1 = 30000101 or b_i1 = 0 then PKH_JS_THAYa(dt_ct,'ngay_gui',' '); 
    ...
    PKH_JS_THAYa(dt_ct,'ngay_gui',FBH_IN_CSO_NG(b_i1,'DD/MM/YYYY') );
     PKH_JS_THAYa(dt_ct,'ngay_gui_s',u'Ng\00e0y ' || FBH_IN_CSO_NG(b_i1,'DD') || u' th\00e1ng ' || FBH_IN_CSO_NG(b_i1,'MM')
...
--       end if;
      if a_dk_pt_bt(b_lp) > 0 and a_dk_tien_bh(b_lp) >0  then
        b_ten_temp:= a_dk_pt_bt(b_lp) || '% x ' || FBH_CSO_TIEN(a_dk_tien_bh(b_lp),'') || u' Tr.\0111= '||  FBH_CSO_TIEN(a_dk_t_that(b_lp),'');
  ...
  't_that' value c6,'giam' value c8,'bt_con' value c7,'nd' value c13,'so_ngay' value b_so_ngay,
   'co_so' value b_co_so returning clob) returning clob) into dt_dk_new FROM temp_1;
  delete temp_1;delete temp_2;
  commit;
end if;

--dt_hk
  PKH_JS_THAY(dt_ct,'ma_nth','');
  PKH_JS_THAY(dt_ct,'ten_nth','');
  PKH_JS_THAY(dt_ct,'ten_tk_nth','');
  PKH_JS_THAY(dt_ct,'tien_nth','');
  PKH_JS_THAY(dt_ct,'cmt_nth','');
  PKH_JS_THAY(dt_ct,'mobi_nth','');
  PKH_JS_THAY(dt_ct,'email_nth','');
  PKH_JS_THAY(dt_ct,'so_tk_nth','');
  PKH_JS_THAY(dt_ct,'ma_nh_nth','');
  PKH_JS_THAY(dt_ct,'thue_nth','');
  PKH_JS_THAY(dt_ct,'bang_chu_nth','');
...
if b_i1 <> 0 then
  ...
  b_lenh := FKH_JS_LENH('ma,ten,tien,cmt,mobi,email,so_tk,ma_nh,ten_tk,thue');
  EXECUTE IMMEDIATE b_lenh bulk collect INTO a_hk_ma,a_hk_ten,a_hk_tien,a_hk_cmt,a_hk_mobi,a_hk_email,a_hk_so_tk,
          a_hk_ma_nh,a_hk_ten_tk,a_hk_thue USING dt_hk;
          delete temp_1;
     for b_lp in 1..a_hk_ma.count loop
       b_temp_clob := FBH_IN_CSO_CHU(a_hk_tien(b_lp),b_nt_tien);
        insert into temp_1(c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11) 
            values(a_hk_ma(b_lp),a_hk_ten(b_lp),FBH_CSO_TIEN(a_hk_tien(b_lp),''),a_hk_cmt(b_lp),a_hk_so_tk(b_lp),a_hk_ma_nh(b_lp),
              a_hk_mobi(b_lp),a_hk_email(b_lp),a_hk_ten_tk(b_lp),a_hk_thue(b_lp),b_temp_clob);
     end loop;
  select JSON_ARRAYAGG(json_object('ma' value c1,'ten' value c2,'tien' value c3,'cmt' value c4,'so_tk' value c5,
  'ma_nh' value c6,'mobi' value c7,'email' value c8,'ten_tk' value c9,'thue' value c10,'bang_chu' value c11 returning clob) returning clob) into dt_hk FROM temp_1;
...
if b_i1 <> 0 then
  ...
  for b_lp in 1..a_ten.count loop
    b_ten_temp:= u'T\1eeb ' || FBH_IN_SUBSTR(a_muc(b_lp), '-', 'T') || u'% \0111\1ebfn ' || FBH_IN_SUBSTR(a_muc(b_lp), '-', 'S') || '%'; 
...
if b_i1 = 0 then
  b_ttrang_tt:= u'Ch\01b0a thanh to\00e1n';
else
  ...
  IF TO_DATE(TO_CHAR(b_ngay_tt), 'YYYYMMDD') < TO_DATE(TO_CHAR(b_ngay_da_tt), 'YYYYMMDD') THEN
       b_ttrang_tt:= u'\0110\00e3 thanh to\00e1n tr\01b0\1edbc h\1ea1n';
  ELSE
      b_ttrang_tt:= u'Thanh to\00e1n tr\1ec5 h\1ea1n';
  END IF;
  b_ngay_nop:= TO_CHAR(pkh_so_cng_date(b_ngay_da_tt),'DD/MM/YYYY');
end if;
-- lay ma quy tac bh
select count(*) into b_i1 from bh_ngdl_ds t WHERE t.ma_dvi=b_ma_dvi AND t.gcn = b_gcn;
if b_i1 <> 0 then
select ma_sp bulk collect into b_ma_sp from bh_ngdl_ds t WHERE t.ma_dvi=b_ma_dvi AND t.gcn = b_gcn;
    for b_lp in 1..b_ma_sp.count loop
        select count(*) into b_i1 from bh_ngdl_sp where ma = b_ma_sp(b_lp);
        if b_i1 <> 0 then
          SELECT FKH_JS_GTRIc(FKH_JS_BONH(t.txt) ,'qtac') into b_temp_nvar from bh_ngdl_sp t where  t.ma= b_ma_sp(b_lp) and rownum = 1;
          if b_temp_clob is null then b_temp_clob := b_temp_nvar;
          elsif instr(b_temp_clob, b_temp_nvar) = 0 then
          b_temp_clob := b_temp_clob || ',' || b_temp_nvar;end if;
      end if;
      PKH_JS_THAY(dt_ct,'ma_qtac', b_temp_clob);
    end loop;
end if;
-- lay phong/ban
select count(*) into b_i1 from bh_hd_goc t WHERE t.ma_dvi=b_ma_dvi AND t.so_hd = b_gcn;
if b_i1 <> 0 then
  select phong into b_ma_phong from bh_hd_goc t WHERE t.ma_dvi=b_ma_dvi AND t.so_hd = b_gcn;
  select ten into b_ten_phong from ht_ma_phong t WHERE t.ma_dvi=b_ma_dvi AND t.ma = b_ma_phong;
    PKH_JS_THAY(dt_ct,'ten_pban', b_ten_phong);
end if;

select count(*) into b_i1 from ht_ma_dvi t WHERE t.ma=b_ma_dvi;
if b_i1 <> 0 then
select ten,ten_gon into b_dvi_ten,b_dvi_tengon from ht_ma_dvi t WHERE t.ma=b_ma_dvi;
    PKH_JS_THAY(dt_ct,'ten_dvi', b_dvi_ten);
    PKH_JS_THAY(dt_ct,'tengon_dvi', b_dvi_tengon);
end if;
PKH_JS_THAYa(dt_ct,'ttrang_tt,ngay_nop,pthu',b_ttrang_tt ||','||b_ngay_nop ||','|| b_pthu);
select json_object('dt_ct' value dt_ct,'dt_dk' value dt_dk,'dt_dk_new' value dt_dk_new,'hd_ct' value hd_ct,'ls_bt' value b_ls_bt,'dt_hk' value dt_hk,
