create or replace procedure PBH_PKT_IN_HD(
...
AS
    ...
    a_pvbh_ppts pht_type.a_var;a_pvbh_ppkh pht_type.a_var;
    -- a_dkbs
...
--dt_hk
select count(*) into b_i1 from bh_pkt_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_hk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_hk FROM bh_pkt_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_hk';
...
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ct FROM bh_pkt_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_ct';
    ...
    if b_nt_tien= 'VND' then PKH_JS_THAY_D(dt_ct,'nt_tien', N'Đồng');b_nt_tien:=N'đồng'; end if;
    b_temp_var:=FKH_JS_GTRIs(dt_ct ,'gio_hl');
    PKH_JS_THAY_D(dt_ct,'gio_hl',NVL(FBH_IN_SUBSTR(b_temp_var, '|', 'T') ,'00') || N' giờ ' ||  NVL(FBH_IN_SUBSTR(b_temp_var, '|', 'S') ,'00') );
    b_temp_var:=FKH_JS_GTRIs(dt_ct ,'gio_kt');
    PKH_JS_THAY_D(dt_ct,'gio_kt',NVL(FBH_IN_SUBSTR(b_temp_var, '|', 'T') ,'00') || N' giờ ' ||  NVL(FBH_IN_SUBSTR(b_temp_var, '|', 'S') ,'00'));
    ---
    b_i1:= FKH_JS_GTRIn(dt_ct ,'ngay_cap');
    if b_i1 = 30000101 then PKH_JS_THAY_D(dt_ct,'ngay_cap',' '); 
    else PKH_JS_THAY_D(dt_ct,'ngay_cap',FBH_IN_CSO_NG(b_i1,'DD/MM/YYYY') );end if;
    ...
    PKH_JS_THAY_D(dt_ct,'phi',FBH_CSO_TIEN(b_i1,b_nt_tien) );

    b_i1:= FKH_JS_GTRIn(dt_ct ,'gia');
    PKH_JS_THAY_D(dt_ct,'gia',FBH_CSO_TIEN(b_i1,b_nt_tien) );

    b_i1:= FKH_JS_GTRIn(dt_ct ,'thue');
    PKH_JS_THAY_D(dt_ct,'thue',FBH_CSO_TIEN(b_i1,b_nt_tien) );

    b_i1:= FKH_JS_GTRIn(dt_ct ,'ttoan');
    PKH_JS_THAY_D(dt_ct,'ttoan',FBH_CSO_TIEN(b_i1,b_nt_tien) );
    PKH_JS_THAY_D(dt_ct,'bangchu',FBH_IN_CSO_CHU(b_i1,b_nt_tien) );

    PKH_JS_THAY_D(dt_ct,'ma_nh',' ' );
...
if b_i1 <> 0 then
  ...
  if b_temp_clob <> '[""]' then
    ...
    ds_ct:= a_clob(1);
    PKH_JS_THAY_D(dt_ct,'dvi',FKH_JS_GTRIs(ds_ct ,'dvi') );
    PKH_JS_THAY_D(dt_ct,'ddiemc',FKH_JS_GTRIs(ds_ct ,'ddiemc') );
    PKH_JS_THAY_D(dt_ct,'lvuc',FKH_JS_GTRIs(ds_ct ,'lvuc') );
    b_temp_nvar:= FBH_IN_SUBSTR(FKH_JS_GTRIs(ds_ct ,'ma_nct'),'|','S');
    PKH_JS_THAY_D(dt_ct,'ma_nct',b_temp_nvar);
    PKH_JS_THAY_D(dt_ct,'tt_ctrinh',FKH_JS_GTRIs(ds_ct ,'tt_ctrinh'));
    PKH_JS_THAY_D(dt_ct,'tt_tbi',FKH_JS_GTRIs(ds_ct ,'tt_tbi'));
    PKH_JS_THAY_D(dt_ct,'ten_nct',FKH_JS_GTRIs(ds_ct ,'ten_nct'));
    b_i1:= FKH_JS_GTRIn(ds_ct ,'gia_nct');
    PKH_JS_THAY_D(dt_ct,'gia_nct',FBH_CSO_TIEN(b_i1,b_nt_tien));

    if trim(FKH_JS_GTRIs(ds_ct ,'mo_ta')) is not null then
         PKH_JS_THAY_D(dt_ct,'mota',FKH_JS_GTRIs(ds_ct ,'mo_ta'));
    else
      b_temp_nvar:= N'Từ ' || FKH_JS_GTRIs(dt_ct ,'gio_hl') || N' ngày '|| FKH_JS_GTRIs(dt_ct ,'ngay_hl') || N' đến '|| FKH_JS_GTRIs(dt_ct ,'gio_kt') || N' ngày '|| FKH_JS_GTRIs(dt_ct ,'ngay_kt');
      PKH_JS_THAY_D(dt_ct,'mota',b_temp_nvar);
    end if;
...
select JSON_ARRAYAGG(json_object('TEN' VALUE c3 || ' - ' || C1 returning clob) returning clob) into dt_bs_vc from temp_1 where c2  = 'KT_THVC';
select JSON_ARRAYAGG(json_object('TEN' VALUE c3 || ' - ' || C1 returning clob) returning clob) into dt_bs_gdkd from temp_1 where c2 = 'KT_GDKD';
select JSON_ARRAYAGG(json_object('TEN' VALUE c3 || ' - ' || C1 returning clob) returning clob) into dt_bs_t3 from temp_1 where c2 = 'KT_TNTB';
...
if b_i1 <> 0 then
  ...
  if b_temp_clob <> '[""]' then
    ...
    if ds_pvi <> '"[]"' then
        ...
        EXECUTE IMMEDIATE b_lenh bulk collect INTO a_pvbh_ten,a_pvbh_ma,a_pvbh_pttsb,a_pvbh_ptts,a_pvbh_ptkhb,a_pvbh_ptkh,a_pvbh_tc,
        a_pvbh_ktru,a_pvbh_loai,a_pvbh_ma_ct,a_pvbh_ppts,a_pvbh_ppkh USING ds_pvi;
...
if b_i1 <> 0 then
    ...
    if b_i2 <> 0 then
      if b_i2 > 1 then
        raise_application_error(-20001,N'loi:Có nhiều hơn 1 điều khoản TS cấp bằng 1:loi');
      end if;
      select n2 into b_tong_tien_v from temp_1 where n1 = 1 and c3 = 'TS';
      PKH_JS_THAY_D(dt_ct,'tong_tien_v',FBH_CSO_TIEN(b_tong_tien_v,b_nt_tien));
    end if;

    select count(*) into b_i2 from temp_1 where n1 = 2 and c3 = 'KH';
    if b_i2 <> 0 then
      --- hôm nay mr Huy lấy cấp bang 2
      if b_i2 > 1 then
        raise_application_error(-20001,N'loi:Có nhiều hơn 1 điều khoản KH cấp bằng 2:loi');
      end if;
      select n2 into b_tong_tien_g from temp_1 where n1 = 2 and c3 = 'KH';
      PKH_JS_THAY_D(dt_ct,'tong_tien_g',FBH_CSO_TIEN(b_tong_tien_g,b_nt_tien));
    end if;
    select count(*) into b_i2 from temp_1 where n1 = 1 and c3 = 'BI';
      if b_i2 <> 0 then
      if b_i2 > 1 then
        raise_application_error(-20001,N'loi:Có nhiều hơn 1 điều khoản BI cấp bằng 1:loi');
      end if;
      select n2 into b_tong_tien_bi from temp_1 where n1 = 1 and c3 = 'BI';
      PKH_JS_THAY_D(dt_ct,'tong_tien_bi',FBH_CSO_TIEN(b_tong_tien_bi,b_nt_tien));
    end if;
    ...
    for b_lp in 1..a_pvbh_ma.count loop
      ...
      case b_temp_var
        ...
        when 'D' then
          ...
          b_tlp_bi:= b_tlp_bi + fbh_in_tinh_tlp(a_pvbh_ptkhb(b_lp),a_pvbh_ptkh(b_lp),a_pvbh_ppkh(b_lp),b_tong_tien_bi);
          if trim(a_pvbh_ktru(b_lp)) is not null then
    ...
    PKH_JS_THAY_D(dt_ct,'tlp_v',FBH_TO_CHAR(b_tlp_v));
    PKH_JS_THAY_D(dt_ct,'tlp_bb',FBH_TO_CHAR(b_tlp_bb));
    PKH_JS_THAY_D(dt_ct,'tlp_rr',FBH_TO_CHAR(b_tlp_rr));
    PKH_JS_THAY_D(dt_ct,'tlp_g',FBH_TO_CHAR(b_tlp_g));
    PKH_JS_THAY_D(dt_ct,'tlp_bi',FBH_TO_CHAR(b_tlp_bi));


    PKH_JS_THAY_D(dt_ct,'phi_v',FBH_CSO_TIEN(b_phi_v,b_nt_tien));
    PKH_JS_THAY_D(dt_ct,'phi_bb',FBH_CSO_TIEN(b_phi_bb,b_nt_tien));
    PKH_JS_THAY_D(dt_ct,'phi_rr',FBH_CSO_TIEN(b_phi_rr,b_nt_tien));
    PKH_JS_THAY_D(dt_ct,'phi_g',FBH_CSO_TIEN(b_phi_g,b_nt_tien));
    PKH_JS_THAY_D(dt_ct,'phi_bi',FBH_CSO_TIEN(b_phi_bi,b_nt_tien));
    ------------muc khau tru

    PKH_JS_THAY_D(dt_ct,'mkt_b',LTRIM(b_mkt_b,','));
    PKH_JS_THAY_D(dt_ct,'mkt_c',LTRIM(b_mkt_c,','));
    --PKH_JS_THAY_D(dt_ct,'mkt_m',LTRIM(b_mkt_m,','));
    PKH_JS_THAY_D(dt_ct,'mkt_d',LTRIM(b_mkt_d,','));
        ...
        if a_kbt_ma_dk(b_lp) = 'VCX' then
            ...
            if a_kbt_ma(b_lp2) = 'KVU' then
                if trim(FKH_JS_GTRIs(dt_ct ,'ktru')) is null then
                    ...
                    PKH_JS_THAY_D(dt_ct,'ktru',b_temp_nvar);
                end if;
            end if;
            end loop;
        else
...
if  b_i1 <> 0 then
    ...
    loop
      insert into temp_4(C1) values(N'-   Kỳ ' || b_i2 || '/' || b_i1 || N': Ngày thanh toán ' || FBH_IN_CSO_NG(r_lp.ngay,'DD/MM/YYYY') || N', số tiền thanh toán ' || FBH_CSO_TIEN(r_lp.tien,b_nt_tien) );
    b_i2:= b_i2 + 1;
    end loop;
    ---
    if b_i1 = 1 then
      b_temp_var:= FKH_JS_GTRIs(dt_ct ,'ngay_hl');
      select min(ngay) into b_i2 from bh_pkt_tt where ma_dvi = b_ma_dvi and so_id = b_so_id;
...
--lay dt dong
delete temp_2;commit;
PKH_JS_THAY_D(dt_ct,'nbh',N'NGƯỜI BẢO HIỂM');
PKH_JS_THAY_D(dt_ct,'dbh','K');
select count(*) into b_i1 from BH_HD_DO_TL where ma_dvi = b_ma_dvi and so_id = b_so_id;
if b_i1 <> 0 then
  PKH_JS_THAY_D(dt_ct,'dbh','C');
  PKH_JS_THAY_D(dt_ct,'nbh',N'NGƯỜI BẢO HIỂM ĐỨNG ĐẦU');
  b_i1:= 0;b_i2:= 1;
...
if b_i1 <> 0 then
  ...
  b_i2:= b_i2 + b_i1;
  PKH_JS_THAY_D(dt_ct,'tien_dong',FBH_CSO_TIEN(b_i1,''));
  PKH_JS_THAY_D(dt_ct,'tong_tien_dong',FBH_CSO_TIEN(b_i2,''));
end if;
