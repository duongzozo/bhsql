create or replace procedure PBH_PHH_IN_HD_GCN(
...
if b_i1 <> 0 then
  ...
  if b_temp_clob <> '[""]' then
    ...
    b_i1:= FKH_JS_GTRIn(ds_ct ,'gia');
    PKH_JS_THAY(dt_ct,'gia',FBH_CSO_TIEN(b_i1,b_nt_tien) );

    b_i1:= FKH_JS_GTRIn(ds_ct ,'thue');
    PKH_JS_THAY(dt_ct,'thue',FBH_CSO_TIEN(b_i1,b_nt_tien) );

    b_i1:= FKH_JS_GTRIn(ds_ct ,'ttoan');
    PKH_JS_THAY(dt_ct,'ttoan',FBH_CSO_TIEN(b_i1,b_nt_tien) );
    PKH_JS_THAYa(dt_ct,'bangchu',FBH_IN_CSO_CHU(b_i1,b_nt_tien) );
  end if;
end if;
...
select JSON_ARRAYAGG(json_object('TEN' VALUE C1 returning clob) returning clob) into dt_bs_vc from temp_1 where c2  = 'THVC';
select JSON_ARRAYAGG(json_object('TEN' VALUE C1 returning clob) returning clob) into dt_bs_gdkd from temp_1 where c2 = 'GDKD';
...
insert into temp_1(c1,c2,c3,n1,n2) 
  ...
  dk.so_id_dt = b_so_id_dt and  dk.ma = lh.ma and dk.ma_dvi = b_ma_dvi and dk.so_id = b_so_id and dk.nv = 'C' and trim(dk.PVI_MA) is null;
...
if b_i1 <> 0 then
    ...
    b_lenh := FKH_JS_LENH('ma,ten,tien,tienkb,cap');
    EXECUTE IMMEDIATE b_lenh bulk collect INTO a_dk_ma,a_dk_ten,a_dk_tien,a_dk_tienkb,a_dk_cap USING dt_dk;
    for b_lp in 1..a_dk_ma.count loop
        update temp_1 set n3 = a_dk_tienkb(b_lp) where c1 = a_dk_ma(b_lp);
    end loop;
