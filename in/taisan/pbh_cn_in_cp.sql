create or replace procedure PBH_CN_IN_CP(
...
as
    ...
    --dt_dkbs
    b_co_chung number;b_co_thvc number;b_co_gdkd number;
    b_dk_chung clob;b_dk_thvc clob; b_dk_gdkd clob;
    ...
    ---
    dt_bs_chung clob;dt_bs_vc clob;dt_bs_gdkd clob;
...
begin
 b_ma_dvi:=nvl(trim(b_ma_dvi),b_ma_dviN);
     --Lay nguoi duoc BH
select count(*) into b_i1 from bh_phhb t WHERE ma_dvi = b_ma_dvi and t.so_id = b_so_id;
if b_i1 <> 0 then
SELECT lan INTO b_lan FROM bh_phhb t WHERE ma_dvi = b_ma_dvi and t.so_id = b_so_id ;
end if;
        
--dt-ct  --Lay ben mua BH
select count(*) into b_i1 from bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi and t.so_id = b_so_id AND t.loai='dt_ct' AND t.lan = b_lan;
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ct FROM bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi AND t.so_id = b_so_id AND t.loai='dt_ct' AND t.lan = b_lan;
...
PKH_JS_THAY(dt_ct,'nbh',N'NGƯỜI BẢO HIỂM');
...
--end dong
--dt_hk
select count(*) into b_i1 from bh_phhB_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_hk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_hk FROM bh_phhB_txt t WHERE t.ma_dvi = b_ma_dvi and t.so_id = b_so_id AND t.loai='dt_hk' and t.lan = b_lan;
  ...
  for b_lp in 1..a_hu_ten.count loop
    ...
    b_temp_nvar:= N'NGƯỜI THỤ HƯỞNG THỨ ' || FBH_IN_SO_CHU(b_i1) || ': ' || a_hu_ten(b_lp);
    end if;
...
---ds_ct
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_ct';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO b_temp_clob FROM bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_ct' and t.lan = b_lan;
  if b_temp_clob <> '[""]' then
    b_lenh:=FKH_JS_LENHc('');
...
---ds_pvi
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_pvi';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO b_temp_clob FROM bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_pvi' AND t.lan = b_lan;
  if b_temp_clob <> '[""]' then
    b_lenh:=FKH_JS_LENHc('');
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using b_temp_clob;
    ds_pvi:= a_clob(1);
...
select count(*) into b_i1 from bh_phhb_txt t WHERE  t.so_id = b_so_id AND t.loai='ds_dk';
if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO b_temp_clob FROM bh_phhb_txt t WHERE  t.so_id = b_so_id AND t.loai='ds_dk' AND t.lan = b_lan;
    b_lenh:=FKH_JS_LENHc('');
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using b_temp_clob;
    dt_dk:= a_clob(1);
    ...
    insert into temp_1(c1,c2,c3,n1,n2) select dk.ma,dk.ten,lh.loai,dk.cap,dk.tien           
      ...
      join bh_phh_lbh lh on dk.ma = lh.ma;
...
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_dkbs';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_dkbs FROM bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_dkbs' AND t.lan = b_lan;
...
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dkbs';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO b_temp_clob FROM bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_dkbs' and t.lan = b_lan;
  if b_temp_clob <> '[""]' then
    b_lenh:=FKH_JS_LENHc('');
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using b_temp_clob;
    ds_dkbs:= a_clob(1);
...
if b_co_chung=1 then
      ...
      if b_co_gdkd=1 then
         ...
         PKH_JS_THAY(dt_ct,'dkbs_gdkd',
         N'Điều khoản bổ sung áp dụng đối với Gián đoạn kinh doanh:');
      end if;
...
-----------------ds_lt
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_lt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO b_temp_clob FROM bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='ds_lt' and t.lan = b_lan;
  if b_temp_clob <> '[""]' then
    b_lenh:=FKH_JS_LENHc('');
    EXECUTE IMMEDIATE b_lenh bulk collect into a_clob using b_temp_clob;
    dt_lt:= a_clob(1);
...
--dt_lt
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_lt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_lt FROM bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_lt' and t.lan = b_lan;
  b_lenh := FKH_JS_LENH('ten,ma_lt');
  EXECUTE IMMEDIATE b_lenh bulk collect INTO a_ten,a_ma USING dt_lt;
  for b_lp in 1..a_ten.count loop
      select count(*) into b_i1 from bh_ma_dklt where ma = a_ma(b_lp);
...
select JSON_ARRAYAGG(json_object('TEN' VALUE c3 || ' - ' || C1 returning clob) returning clob) into dt_bs_chung from temp_1 where NVL(TRIM(c2),'#') NOT IN ('THVC','GDKD');

delete temp_4;delete temp_1;commit;

--lay dt_kytt
select count(*) into b_i1 from bh_phhb_txt t WHERE ma_dvi = b_ma_dvi and  t.so_id = b_so_id AND t.loai='dt_kytt';
  if b_i1 <> 0 then
    SELECT FKH_JS_BONH(t.txt) INTO dt_kytt FROM bh_phhb_txt t WHERE t.ma_dvi = b_ma_dvi AND t.so_id = b_so_id AND t.loai='dt_kytt' AND t.lan = b_lan;
    ...
    b_ky_tt := b_ky_tt ||
        N'- Kỳ ' || i || '/' || b_tt_ngay.count || N': Ngày thanh toán ' ||
        FBH_IN_CSO_NG(b_tt_ngay(i),'DD/MM/YYYY') || 
        N', số tiền thanh toán ' || FBH_CSO_TIEN(b_tt_tien(i),N'đồng');
...
--
select json_object('dt_ct' value dt_ct,'dt_dong' value dt_dong,'ds_ct' value ds_ct,'dt_pvi' value dt_pvi,'dt_hk' value dt_hk,
'dt_dkbs' value dt_dkbs,'dt_kytt' value dt_kytt,'dt_dk_v'  value dt_dk_v,'dt_dk_g' value dt_dk_g,
'dt_lt' value dt_lt,'dt_dong_bh' value dt_dong_bh,'dt_bs_chung' value dt_bs_chung,
'dt_bs_vc' value dt_bs_vc,'dt_bs_gdkd' value dt_bs_gdkd  returning clob) into b_oraOut from dual;
