drop procedure PBH_PHH_IN_BT;
/
create or replace procedure PBH_PHH_IN_BT(
...
select so_id_hd,so_hs,ma_dvi into b_so_id_hd,b_so_hs,b_ma_dvi from bh_bt_phh where so_id = b_so_id;

--dt_ct
select count(*) into b_i1 from bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_ct';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ct FROM bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_ct';
  ...
  if trim(b_n_temp) is not null then
    select count(1) into b_i1 from bh_phh_nntt where ma = b_n_temp;
    if b_i1 <> 0 then
        select ten into b_n_temp from bh_phh_nntt where ma = b_n_temp;
    end if;
  end if;
...
-- lay dt_dk
-- select count(*) into b_i1 from bh_bt_phh t,bh_phh_dk t1 where t.so_id_dt = t1.so_id_dt and t.so_id = b_so_id;
-- if b_i1 <> 0 then
...
--                    INTO dt_dk from bh_bt_phh t,bh_phh_dk t1 where t.so_id_dt = t1.so_id_dt and t.so_id = b_so_id;
select count(*) into b_i1 from bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_dk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_dk FROM bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_dk';
end if;
-- lay dt_dvi
select count(*) into b_i1 from bh_bt_phh t,bh_phh_dvi t1 where t.so_id_dt = t1.so_id_dt and t.so_id = b_so_id;
if b_i1 <> 0 then
  select JSON_OBJECT(t1.so_id_dt,t1.gcn) into dt_dvi from bh_bt_phh t,bh_phh_dvi t1 where t.so_id_dt = t1.so_id_dt and t.so_id = b_so_id and rownum = 1;
end if;
--dt_hk
select count(*) into b_i1 from bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_hk';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_hk FROM bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_hk';
end if;
-- lay dt_tba
select count(*) into b_i1 from bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_tba';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_tba FROM bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_tba';
end if;

--dt_kbt
select count(*) into b_i1 from bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_kbt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_kbt FROM bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_kbt';
end if;
-- lay dt_tba
select count(*) into b_i1 from bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_ttt';
if b_i1 <> 0 then
  SELECT FKH_JS_BONH(t.txt) INTO dt_ttt FROM bh_bt_phh_txt t WHERE t.so_id = b_so_id AND t.loai='dt_ttt';
end if;
-------------thong tin hop dong
--hd_ct
select FKH_JS_BONH(t1.txt),t3.ma_sp INTO hd_ct,b_ma_sp from bh_phh_txt t1,bh_bt_phh t2,bh_phh t3
    where t1.so_id = t2.so_id_hd and t1.so_id = t3.so_id and t2.so_id = b_so_id and t1.loai = 'dt_ct';
select upper(ten) into b_ten_sp from bh_phh_sp WHERE ma=b_ma_sp;
...
--hd kbt
select count(*) into b_i1 from bh_phh_kbt t where t.so_id = b_so_id_hd;
if b_i1 <> 0 then
    select FKH_JS_BONH(kbt) into hd_kbt from bh_phh_kbt where so_id=b_so_id_hd;
...
---hd_pvi
select count(*) into b_i1 from bh_phh_txt t where  t.so_id = b_so_id_hd AND t.loai='ds_pvi';
if b_i1 <> 0 then
    select FKH_JS_BONH(txt) into hd_pvi from bh_phh_txt where  so_id=b_so_id_hd and loai='ds_pvi';
end if;
...
select count(1) into b_i1 FROM bh_phh_tt where so_id = b_so_id_hd;
if b_i1 <> 0 then
select JSON_ARRAYAGG(json_object('TEN' VALUE  (trim(TO_CHAR(tien, '999,999,999,999,999,999PR')) || ' ' || b_nt_tien || U' thanh to\00E1n ng\00E0y ' || TO_CHAR(TO_DATE(ngay, 'YYYYMMDD'), 'DD/MM/YYYY')  ) ) returning clob) 
  into hd_tt FROM bh_phh_tt where so_id = b_so_id_hd;
end if;

---khieu nai,bt dt_tien_kn clob;dt_tien_bt clob;
select JSON_ARRAYAGG(json_object('TEN' VALUE ten,'TIEN' value trim(TO_CHAR(t_that, '999,999,999,999,999,999PR')) || ' ' || b_nt_tien ) returning clob) into dt_tien_kn from bh_bt_phh_dk where so_id = b_so_id and tc = 'T' and t_that <> 0;
select JSON_ARRAYAGG(json_object('TEN' VALUE ten,'TIEN' value trim(TO_CHAR(tien_qd, '999,999,999,999,999,999PR')) || ' ' || b_nt_tien ) returning clob) into dt_tien_bt from bh_bt_phh_dk where so_id = b_so_id and tc = 'T' and t_that <> 0;

select sum(t_that) into b_i1 from bh_bt_phh_dk where so_id = b_so_id and tc = 'C' and t_that <> 0;
PKH_JS_THAY(dt_ct,'tien_kn',(trim(TO_CHAR(b_i1, '999,999,999,999,999,999PR')) || ' ' || b_nt_tien));
select sum(tien_qd) into b_i1 from bh_bt_phh_dk where so_id = b_so_id and tc = 'C' and t_that <> 0;
...
-- thu doi dong bh
delete temp_2;
select tien into b_mtn from bh_phh where so_id = b_so_id_hd;
...
if b_i1 <> 0 then
  ...
  loop
      b_dong_mtn:= trim(TO_CHAR(r_lp.pt * b_mtn/100, '999,999,999,999,999,999PR')) || ' ' || b_nt_tien;
      ...
      --insert into temp_2(C1) values(b_ten_nha_bh || ': ' || r_lp.pt || N'% đồng, MTN: ' || b_dong_mtn || N' số tiền thu đòi: ' || b_dong_thudoi );
...
for r_lp in(select ps.so_id,ps.so_id_ta_ps,ps.so_id_ta_hd,ps.goc,ps.pthuc,ps.tien,ps.nha_bh from tbh_ps ps,bh_bt_tu tu
    ...
    union
    select ps.so_id,ps.so_id_ta_ps,ps.so_id_ta_hd,ps.goc,ps.pthuc,ps.tien,ps.nha_bh from tbh_ps ps,bh_bt_phh bt
...
-- tinh tong tien
select NVL(sum(tien),0) into b_i1 from (select tien from bh_bt_phh where so_id = b_so_id
...
-- union
-- select tien from bh_bt_tba where so_id_hs = b_so_id
);
b_tien := b_i1 - b_i2;
--insert into temp_2(N1,C1,N2,C3,C4) values(1,N'Bảo hiểm AAA',0,(trim(TO_CHAR(b_tien - b_tien_th, '999,999,999,999,999,999PR'))|| ' ' || b_nt_tien ),'AAA');
...
for r_lp in(
  ...
  where ps.so_id = tu.so_id and tu.so_id_hs = b_so_id
  union
  select ps.* from tbh_ps ps,bh_bt_phh bt
  where ps.so_id = bt.so_id and bt.so_id = b_so_id
  union
...
loop
  if r_lp.pthuc in('Q','S') then
      select sum(pt) into b_i1 from TBH_GHEP_PHI where so_id = r_lp.so_id_ta_ps and nha_bh = r_lp.nha_bh and pthuc = r_lp.pthuc and kieu = 'C';
...
--end tai bh
select count(*) into b_i1 from bh_phh_txt t where  t.so_id = b_so_id_hd AND t.loai='ds_ct';
if b_i1 <> 0 then
    select FKH_JS_BONH(txt) into hd_ds from bh_phh_txt where  so_id=b_so_id_hd and loai='ds_ct';
end if;
