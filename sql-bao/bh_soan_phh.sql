create or replace procedure PBH_SOAN_DK_PHH(
...
select a.so_id_dt,a.ddiem,a.ma_dt,b.lh_nv,b_nt_tien,b.tien,b_nt_phi,b.phi
    ...
    from bh_phh_dvi a,bh_phh_dk b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_SOAN_DK_PHH:loi'; end if;
end;
/
create or replace procedure PTBH_SOAN_NV_PHHd(
...
PBH_SOAN_DK_PHH(b_ma_dvi,b_so_id,b_so_id_dt,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP2(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_SOAN_NV_PHHd:loi'; end if;
end;
/
create or replace PROCEDURE PTBH_SOAN_NV_PHH(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
    a_ma_dvi pht_type.a_var,a_so_id pht_type.a_num,a_so_id_dt pht_type.a_num,
    b_ma_dt out varchar2,b_loi out varchar2)
AS
    b_ngay_ht number; b_ngay_hl number; b_ngay_kt number;
    b_nv varchar2(10); b_so_hd varchar2(20); b_ttrang varchar2(1);
    b_nt_tien varchar2(5); b_nt_phi varchar2(5); b_ma_dtT varchar2(10);
begin
-- Dan - Ghep tich tu voi 1 dtuong
PTBH_SOAN_TTINf(b_ma_dvi,b_so_id,b_nv,b_so_hd,b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi,b_loi);
if b_loi is not null then return; end if;
b_ma_dt:=' ';
for b_lp in 1..a_ma_dvi.count loop
    ...
    PTBH_SOAN_NV_PHHd(a_ma_dvi(b_lp),a_so_id(b_lp),a_so_id_dt(b_lp),
