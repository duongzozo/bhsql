create or replace function FBH_BAO_BGHD_XE(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
...
/
create or replace procedure PTBH_BAO_GHDT_XE(
    b_ma_dvi varchar2,b_so_id number,a_so_id_dt out pht_type.a_num,b_loi out varchar2)
AS
begin
-- Dan - Doi tuong bao gia XE
--nam: '' -> ' '
if FBH_BAO_BGHD_XE(b_ma_dvi,b_so_id)=' ' then
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_GHDT_XE:loi'; end if;
end;
/
create or replace procedure PBH_BAO_DK_XE(
...
if b_nt_tien<>' ' then
    ...
    select a.so_id_dt,a.ten,a.ma_dt,b.lh_nv,b_nt_tien,b.tien,b.tien,b_nt_phi,b.phi
        from bh_xeB_ds a,bh_xeB_dk b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id
        and b.ma_dvi=b_ma_dvi and b.so_id=b_so_id and b.so_id_dt=a.so_id_dt;
end if;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_BAO_DK_XE:loi'; end if;
end;
/
create or replace procedure PTBH_BAO_NV_XEd(
...
PBH_BAO_DK_XE(b_ma_dvi,b_so_id,b_so_id_dt,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP2(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_XEd:loi'; end if;
end;
/
create or replace procedure PTBH_BAO_NV_XE(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
b_loi:='loi:Loi xu ly PTBH_BAO_NV_XE:loi';
...
if FBH_BAO_BGHD_XE(b_ma_dvi,b_so_id)=' ' then
	select ngay_ht,ngay_hl,ngay_kt,nt_tien,nt_phi into b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi
		from bh_xeB where ma_dvi=b_ma_dvi and so_id=b_so_id;
    b_ma_dt:=FBH_XEB_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
else
	select ngay_ht,ngay_hl,ngay_kt,nt_tien,nt_phi into b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi
		from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
    b_ma_dt:=FBH_XE_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
end if;
PTBH_BAO_NV_XEd(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP(b_ma_dvi,b_so_id,b_so_id_dt,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_XE:loi'; end if;
end;
/
create or replace procedure PTBH_BAO_NV_XEc(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_ma_dt out varchar2,b_loi out varchar2)
AS
    b_i1 number; b_ngay_ht number; b_ngay_hl number; b_ngay_kt number;
...
--nam
    ...
    select ngay_ht,ngay_hl,ngay_kt,nt_tien,nt_phi into b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi
        from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
    b_ma_dt:=FBH_XE_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
    PTBH_GHEP_NV_1(0,b_ngay_ht,b_ngay_hl,b_nt_tien,b_nt_phi,b_ma_dvi,b_so_id,b_so_id_dt,b_loi,'{"xly":"C","nv":"XE"}');
end if;
if b_loi is not null then return; end if;
PTBH_BAO_TEMPc(b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_XEc:loi'; end if;
end;
/
