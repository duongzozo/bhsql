create or replace function FBH_BAO_BGHD_2B(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_GHDT_2B:loi'; else null; end if;
end;
/
create or replace procedure PBH_BAO_DK_2B(
...
if b_nt_tien<>' ' then
    ...
    select a.so_id_dt,a.ten,a.ma_dt,b.lh_nv,b_nt_tien,b.tien,b.tien,b_nt_phi,b.phi
        from bh_2bB_ds a,bh_2bB_dk b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id
        and b.ma_dvi=b_ma_dvi and b.so_id=b_so_id and b.so_id_dt=a.so_id_dt;
end if;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_BAO_DK_2B:loi'; else null; end if;
end;
/
create or replace procedure PTBH_BAO_NV_2Bd(
...
PBH_BAO_DK_2B(b_ma_dvi,b_so_id,b_so_id_dt,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP2(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_XEd:loi'; else null; end if;
end;
/
create or replace procedure PTBH_BAO_NV_2B(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
if FBH_BAO_BGHD_2B(b_ma_dvi,b_so_id)=' ' then
	select ngay_ht,ngay_hl,ngay_kt,nt_tien,nt_phi into b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi
		from bh_2bB where ma_dvi=b_ma_dvi and so_id=b_so_id;
	b_ma_dt:=FBH_2BB_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
else
	select ngay_ht,ngay_hl,ngay_kt,nt_tien,nt_phi into b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi
		from bh_2b where ma_dvi=b_ma_dvi and so_id=b_so_id;
	b_ma_dt:=FBH_2B_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
end if;
PTBH_BAO_NV_2Bd(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP(b_ma_dvi,b_so_id,b_so_id_dt,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_2B:loi'; else null; end if;
end;
/
create or replace procedure PTBH_BAO_NV_2Bc(
...
--nam
    ...
    select ngay_ht,ngay_hl,ngay_kt,nt_tien,nt_phi into b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi
        from bh_2b where ma_dvi=b_ma_dvi and so_id=b_so_id;
    b_ma_dt:=FBH_2B_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
    PTBH_GHEP_NV_1(0,b_ngay_ht,b_ngay_hl,b_nt_tien,b_nt_phi,b_ma_dvi,b_so_id,b_so_id_dt,b_loi,'{"xly":"C","nv":"2B"}');
end if;
if b_loi is not null then return; end if;
PTBH_BAO_TEMPc(b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_2Bc:loi'; else null; end if;
end;
/
