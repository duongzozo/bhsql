create or replace procedure PBH_SOAN_DK_TAU(
...
select ngay_ht,nt_tien,nt_phi into b_ngay_ht,b_nt_tien,b_nt_phi from bh_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
select a.so_id_dt,a.ten_tau,a.loai,b.lh_nv,b_nt_tien,b.tien,b_nt_phi,b.phi
    bulk collect into a_so_id_dtK,a_dviK,a_ma_dtK,a_lh_nvK,a_nt_tienK,a_tienK,a_nt_phiK,a_phiK
    from bh_tau_ds a,bh_tau_dk b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_SOAN_DK_TAU:loi'; end if;
end;
/
create or replace procedure PTBH_SOAN_NV_TAUd(
...
PBH_SOAN_DK_TAU(b_ma_dvi,b_so_id,b_so_id_dt,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP2(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_SOAN_NV_TAUd:loi'; end if;
end;
/
create or replace PROCEDURE PTBH_SOAN_NV_TAU(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
PTBH_SOAN_NV_TAUd(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP(b_ma_dvi,b_so_id,b_so_id_dt,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_SOAN_NV_TAU:loi'; end if;
end;
/
