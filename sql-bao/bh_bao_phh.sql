create or replace function FBH_BAO_BGHD_PHH(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra da chuyen HD
select nvl(min(ttrang),' ') into b_kq from bh_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace procedure PTBH_BAO_TAO_PHH(
...
if FBH_BAO_BGHD_PHH(b_ma_dvi,b_so_id)=' ' then
    select ngay_hl,ngay_kt,tdx,tdy,bk,ddiem into b_ngay_hl,b_ngay_kt,b_tdx,b_tdy,b_bk,b_ddiem
        from bh_phhB_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
else
    select ngay_hl,ngay_kt,tdx,tdy,bk,ddiem into b_ngay_hl,b_ngay_kt,b_tdx,b_tdy,b_bk,b_ddiem
...
for r_lp in (select ma_dvi,so_id,so_id_dt,tdx,tdy,bk,ngay_hl,ngay_kt from bh_phh_ttu where 
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_TAO_PHH:loi'; end if;
end;
/
create or replace procedure PTBH_BAO_TAO_PHHt(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_TAO_PHHt:loi'; end if;
end;
/
create or replace procedure PTBH_BAO_GHDT_PHH(
...
for b_lp in 1..a_so_id_dt.count loop
    PTBH_BAO_TAO_PHH(b_ma_dvi,b_so_id,a_so_id_dt(b_lp),a_ma_dviT,a_so_idT,a_so_id_dtT,b_loi);
    if b_loi is not null then return; end if;
    b_so_id_gh:=0;
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_GHDT_PHH:loi'; end if;
end;
/
create or replace procedure PBH_BAO_DK_PHH(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_loi out varchar2)
AS
    b_nt_tien varchar2(5); b_nt_phi varchar2(5);
    b_ten nvarchar2(500); b_ma_dt varchar2(10);
begin
...
if b_nt_tien<>' ' then
    select ten,ma_dt into b_ten,b_ma_dt from bh_phhB_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
    insert into bh_hd_nv_temp 
        select b_so_id_dt,b_ten,b_ma_dt,lh_nv,b_nt_tien,tien,tien,b_nt_phi,phi
        from bh_phhB_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
end if;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_BAO_DK_PHH:loi'; end if;
end;
/
create or replace procedure PTBH_BAO_NV_PHHd(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
PBH_BAO_DK_PHH(b_ma_dvi,b_so_id,b_so_id_dt,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP2(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_PHHd:loi'; end if;
end;
/
create or replace PROCEDURE PTBH_BAO_NV_PHH(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
for b_lp in 1..a_ma_dvi.count loop
    if FBH_BAO_BGHD_PHH(a_ma_dvi(b_lp),a_so_id(b_lp))=' ' then
        select nvl(max(ma_dt),' ') into b_ma_dtT from bh_phhB_dvi where
            ma_dvi=a_ma_dvi(b_lp) and so_id=a_so_id(b_lp) and so_id_dt=a_so_id_dt(b_lp);
        PTBH_BAO_NV_PHHd(a_ma_dvi(b_lp),a_so_id(b_lp),a_so_id_dt(b_lp),
            b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi,b_loi);
    else
        select nvl(max(ma_dt),' ') into b_ma_dtT from bh_phh_dvi where
            ma_dvi=a_ma_dvi(b_lp) and so_id=a_so_id(b_lp) and so_id_dt=a_so_id_dt(b_lp);
        PTBH_GHEP_NV_1(0,b_ngay_ht,b_ngay_hl,b_nt_tien,b_nt_phi,
            a_ma_dvi(b_lp),a_so_id(b_lp),a_so_id_dt(b_lp),b_loi,'{"xly":"C","nv":"PHH","kieu_ps":"B"}');
...
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_PHH:loi'; end if;
end;
/
create or replace PROCEDURE PTBH_BAO_NV_PHHc(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
for b_lp in 1..a_ma_dvi.count loop
    if FBH_BAO_BGHD_PHH(a_ma_dvi(b_lp),a_so_id(b_lp))=' ' then
        ...
        select nvl(max(ma_dt),' ') into b_ma_dtT from bh_phh_dvi where
            ma_dvi=a_ma_dvi(b_lp) and so_id=a_so_id(b_lp) and so_id_dt=a_so_id_dt(b_lp);
        PTBH_GHEP_NV_1(0,b_ngay_ht,b_ngay_hl,b_nt_tien,b_nt_phi,
            a_ma_dvi(b_lp),a_so_id(b_lp),a_so_id_dt(b_lp),b_loi,'{"xly":"C","nv":"PHH"}');
    end if;
    if b_loi is not null then return; end if;
    b_ma_dtT:=FBH_PHH_DTUONG_CAT(b_ma_dtT);
    if b_ma_dtT>b_ma_dt then b_ma_dt:=b_ma_dtT; end if;
...
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_BAO_NV_PHHc:loi'; end if;
end;
/
