create or replace procedure PBH_SOAN_DK_XE(
...
select ngay_ht,nt_tien,nt_phi into b_ngay_ht,b_nt_tien,b_nt_phi from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
select a.so_id_dt,decode(a.bien_xe,' ',a.so_khung,a.bien_xe),a.loai_xe,b.lh_nv,b_nt_tien,b.tien,b_nt_phi,b.phi
    ...
    from bh_xe_ds a,bh_xe_dk b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_SOAN_DK_XE:loi'; end if;
end;
/
create or replace procedure PTBH_SOAN_NV_XEd(
...
PBH_SOAN_DK_XE(b_ma_dvi,b_so_id,b_so_id_dt,b_loi);
if b_loi is not null then return; end if;
PTBH_BAO_TEMP2(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_SOAN_NV_XEd:loi'; end if;
end;
/
create or replace PROCEDURE PTBH_SOAN_NV_XE(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,
...
PTBH_SOAN_NV_XEd(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_nt_phi,b_loi);
if b_loi is not null then return; end if;
b_ma_dt:=FBH_XEB_MA_DT(b_ma_dvi,b_so_id,b_so_id_dt);
PTBH_BAO_TEMP(b_ma_dvi,b_so_id,b_so_id_dt,b_nt_tien,b_nt_phi,b_loi);
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PTBH_SOAN_NV_XE:loi'; end if;
end;
/
