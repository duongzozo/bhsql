/*** Thanh toan ***/
create or replace function FBH_HD_DO_TT_TXT(b_ma_dvi varchar2,b_so_id_tt number,b_tim varchar2) return nvarchar2
AS
    b_kq nvarchar2(500):=' '; b_i1 number; b_txt clob;
begin
select count(*) into b_i1 from bh_hd_do_tt_txt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt and loai='dt_ct';
if b_i1=1 then
    select txt into b_txt from bh_hd_do_tt_txt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt and loai='dt_ct';
    b_kq:=FKH_JS_GTRIu(b_txt,b_tim);
end if;
return b_kq;
end;
/
create or replace function FBH_HD_DO_TT_NGAY_HT(b_ma_dvi varchar2,b_so_id_ps number) return number
AS
    b_kq number;
begin
-- Dan - Tra ngay phat sinh
select nvl(min(ngay_ht),0) into b_kq from bh_hd_do_ps where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
return b_kq;
end;
/
create or replace function FBH_HD_DO_TT_SO_CT(b_ma_dvi varchar2,b_so_id_ps number) return varchar2
AS
    b_kq varchar2(20);
begin
-- Dan - Tra so_ct phat sinh
select nvl(min(so_ct),' ') into b_kq from bh_hd_do_ps where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
return b_kq;
end;
/
create or replace function FBH_HD_DO_TT_PTHUC(b_ma_dvi varchar2,b_so_id_ps number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra pthuc
select min(pthuc) into b_kq from bh_hd_do_ps where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
return b_kq;
end;
/
create or replace procedure PBH_DO_TT_PT(
...
select nha_bh,ngay_ht,chi_qd,thu_qd,thue_v_qd,thue_r_qd into b_nha_bh,b_ngay_ht,a_t(1),a_t(2),a_v(1),a_v(2)
    from bh_hd_do_tt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
insert into temp_1(n1,n2,c1,c2,c3,c4,n3,n4,n5,n6)
    select so_id_ps,so_id,nhom,loai,nv,ma_nt,tien,tien_qd,thue,thue_qd
    from bh_hd_do_ct where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
update temp_1 set (c7,n7,n8)=(select min(pthuc),sum(tien),sum(thue) from bh_hd_do_ps
...
for r_lp in (select n1 so_id_ps,n2 so_id,c1 nhom,c2 loai,c3 nv,c8 pthuc,c4 ma_nt,n3 tien,n4 tien_qd,n5 thue,n6 thue_qd,n7 tien_goc,n8 thue_goc
    ...
    for r_lp1 in (select lh_nv,so_id_dt,tien,thue from bh_hd_do_ps where ma_dvi=b_ma_dvi and so_id_ps=r_lp.so_id_ps
    ...
    for b_lp in 1..a_lh_nv.count loop
        b_bt:=b_bt+1;
        insert into bh_hd_do_pt values(b_ma_dvi,b_so_id_tt,b_bt,b_ngay_ht,r_lp.so_id,a_so_id_dt(b_lp),r_lp.so_id_ps,r_lp.nhom,r_lp.loai,
            r_lp.nv,r_lp.pthuc,b_kieu,r_lp.ma_nt,a_lh_nv(b_lp),a_tien(b_lp),a_tien_qd(b_lp),a_thue(b_lp),a_thue_qd(b_lp));
    end loop;
end loop;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_DO_TT_PT:loi'; end if;
end;
/
create or replace procedure PBH_DO_TT_PBO(b_ma_dvi varchar2,b_so_id_tt number,b_loi out varchar2)
AS
...
select nvl(min(ngay_ht),0) into b_ngay_ht from bh_hd_do_pt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
if b_ngay_ht=0 then b_loi:=''; return; end if;
b_bt:=0;
for r_lp in (select so_id,nhom,loai,nv,pthuc,kieu,lh_nv,ma_nt,sum(tien) tien,sum(tien_qd) tien_qd from bh_hd_do_pt
    ...
    if r_lp.nv='C' and r_lp.loai in('CH_LE_HH','CH_LE_DL') then
        ...
        if r_lp.ma_nt='VND' then b_tp:=0; else b_tp:=2; end if;
        for r_lp1 in (select nha_bh,pthuc,max(pt) pt from bh_hd_do_tl where ma_dvi=b_ma_dvi and so_id=b_so_id
            and pthuc='D' and lh_nv in(' ',r_lp.lh_nv) group by nha_bh,pthuc) loop
            b_tien_t:=round(b_tien*r_lp1.pt/100,b_tp);
            if b_tp=0 then b_tien_qd_t:=b_tien_t; else b_tien_qd_t:=round(b_tien_qd*r_lp1.pt/100,0); end if;
            if r_lp1.pthuc='D' then b_dvi_xl:=r_lp1.nha_bh; else b_dvi_xl:=b_ma_dvi; end if;
            b_bt:=b_bt+1;
            insert into bh_hd_do_pb values(b_dvi_xl,b_so_id_tt,b_bt,b_so_id,b_ngay_ht,b_ma_dvi,
        ...
        if b_tien_c<>0 then
            b_bt:=b_bt+1;
            insert into bh_hd_do_pb values(b_ma_dvi,b_so_id_tt,b_bt,b_so_id,b_ngay_ht,b_ma_dvi,b_phong,
                r_lp.nhom,r_lp.loai,r_lp.nv,r_lp.pthuc,r_lp.kieu,r_lp.lh_nv,r_lp.ma_nt,b_tien_c,b_tien_qd_c,-1);
        end if;
    end if;
end loop;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_DO_TT_PBO:loi'; end if;
end;
/
create or replace procedure FBH_HD_DO_TT_NBH(b_oraIn varchar2,b_loi out varchar2)
AS
...
insert into bh_kh_hoi_temp1 select nha_bh,FBH_MA_NBH_TEN(nha_bh) from bh_hd_do_tl
    where ma_dvi=b_ma_dvi and so_id=b_so_id and pthuc='C';
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly FBH_HD_DO_TT_NBH:loi'; end if;
end;
/
create or replace procedure PBH_HD_DO_TT_HD(
...
if b_so_hd<>' ' then
    ...
    if b_so_idD<>0 then
        ...
        select count(*) into b_i1 from bh_hd_do_tl where ma_dvi=b_ma_dvi and so_id=b_so_idD and pthuc='C';
        if b_i1=1 then
             select FBH_MA_NBH_TENl(nha_bh) into b_nbh from bh_hd_do_tl
                where ma_dvi=b_ma_dvi and so_id=b_so_idD and pthuc='C';
        end if;
...
/
create or replace procedure PBH_HD_DO_TT_TON(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
insert into temp_1(n1,n2,c1,c2,c3,c4,n3,n4)
    select so_id,so_id_ps,nhom,loai,nv,ma_nt,tien,thue
        from bh_hd_do_ps where ma_dvi=b_ma_dvi and nha_bh=b_nha_bh union all
    select b.so_id,b.so_id_ps,b.nhom,b.loai,nv,b.ma_nt,-b.tien,-b.thue
        from bh_hd_do_tt a,bh_hd_do_ct b where a.ma_dvi=b_ma_dvi and a.nha_bh=b_nha_bh and
...
update ket_qua set c5=FBH_DONG(b_ma_dvi,n1),c6=FBH_HD_GOC_SO_HD(b_ma_dvi,n1),
    n5=FBH_HD_DO_TT_NGAY_HT(b_ma_dvi,n2),c7=FBH_HD_DO_TT_SO_CT(b_ma_dvi,n2);
...
/
create or replace procedure FBH_HD_DO_TT_QD(
...
if b_ma_nt='VND' then
    ...
    select sum(tien),sum(thue),sum(tien_qd),sum(thue_qd) into b_tien_g,b_thue_g,b_tien_qd_g,b_thue_qd_g
        from bh_hd_do_ps where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_ps=b_so_id_ps and ma_nt=b_ma_nt;
    select nvl(sum(tien),0),nvl(sum(thue),0),nvl(sum(tien_qd),0),nvl(sum(thue_qd),0)
        into b_tien_c,b_thue_c,b_tien_qd_c,b_thue_qd_c
        from bh_hd_do_ct where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_ps=b_so_id_ps and ma_nt=b_ma_nt;
...
for b_lp in 1.. a_so_id.count loop
    FBH_HD_DO_TT_QD(b_ma_dvi,a_so_id(b_lp),a_so_id_ps(b_lp),a_ma_nt(b_lp),a_tien(b_lp),a_thue(b_lp),b_tien,b_thue);
    if a_loai(b_lp)='CH_LEPd' then b_cit:=b_cit+b_thue; end if;
...
/
create or replace procedure PBH_HD_DO_TT_CT(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select JSON_ARRAYAGG(json_object(
    ...
    FBH_HD_DO_TT_NGAY_HT(b_ma_dvi,so_id_ps) ngay_ht,FBH_HD_DO_TT_PTHUC(b_ma_dvi,so_id_ps) pthuc
    from bh_hd_do_ct a where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt);
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_hd_do_tt_txt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt and loai='dt_ct';
...
/
create or replace procedure PBH_HD_DO_TT_LKE(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_hd_do_tt where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd;
    if b_dong<>0 then
        PKH_LKE_TRANG(b_dong,b_tu,b_den);
        select JSON_ARRAYAGG(json_object(so_id_tt,'nha_bh' value FBH_MA_NBH_TEN(nha_bh)) returning clob) into cs_lke from
            (select so_id_tt,nha_bh,rownum sott from bh_hd_do_tt where 
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_id_tt desc)
            where sott between b_tu and b_den;
    end if;
else
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_hd_do_tt where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    if b_dong<>0 then
        PKH_LKE_TRANG(b_dong,b_tu,b_den);
        select JSON_ARRAYAGG(json_object(so_id_tt,'nha_bh' value FBH_MA_NBH_TEN(nha_bh)) returning clob) into cs_lke from
            (select so_id_tt,nha_bh,rownum sott from bh_hd_do_tt where 
...
/
create or replace procedure PBH_HD_DO_TT_LKE_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk ='N' then
    select count(*) into b_dong from bh_hd_do_tt where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd;
    select nvl(min(sott),b_dong) into b_tu from (select so_id_tt,rownum sott from bh_hd_do_tt where
...
/
create or replace procedure PBH_HD_DO_TT_TEST(
...
for b_lp in 1.. a_so_id.count loop
    FBH_HD_DO_TT_QD(b_ma_dvi,a_so_id(b_lp),a_so_id_ps(b_lp),a_ma_nt_tt(b_lp),
...
if b_nt_tra='VND' then
    ...
    a_tien_qd(1):=PBH_DO_BH_CN_QD(b_ma_dvi,b_nha_bh,b_nt_tra,b_ngay_ht,'T',a_tien(1));
end if;
...
/
create or replace procedure PBH_HD_DO_TT_XOA_VAT(b_ma_dvi varchar2,b_so_id_tt number,b_loi out varchar2)
AS
begin
-- Dan - Xoa VAT
delete bh_hd_do_vat_txt where ma_dvi=b_ma_dvi and so_id_vat
    in(select distinct so_id_vat from bh_hd_do_vat_ct where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt);
delete bh_hd_do_vat where ma_dvi=b_ma_dvi and so_id_vat
    in(select distinct so_id_vat from bh_hd_do_vat_ct where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt);
delete bh_hd_do_vat_ct where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_HD_DO_TT_XOA_XOA
...
select count(*) into b_i1 from bh_hd_do_vat_ct where so_id_tt=b_so_id_tt;
if b_i1<>0 then b_loi:='loi:Da phat hach hoa don:loi'; return; end if;
select min(ma_dvi),nvl(max(so_id_kt),0) into b_dvi_xl,b_i1 from bh_hd_do_pb where dvi_xl=b_ma_dvi and so_id_tt=b_so_id_tt;
if b_i1>0 then
    b_loi:='loi:Don vi '||b_dvi_xl||' da hach toan phan bo chi nha dong:loi'; return;
end if;
if b_nh=false then
    if FBH_HD_DO_PS_VAT(b_ma_dvi,b_so_id_tt)<>0 then
        b_loi:='loi:Thanh toan dong bao hiem da co hoa don VAT:loi'; return;
    end if;
end if;
b_i1:=0;
for r_lp in (select so_id,so_id_ps from bh_hd_do_ct where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt) loop
    b_i1:=b_i1+1; a_so_id(b_i1):=r_lp.so_id; a_so_id_ps(b_i1):=r_lp.so_id_ps;
end loop;
b_i1:=0;
for r_lp in (select ma_nt,tien,tien_qd from bh_hd_do_pp where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt and pt='C') loop
    if r_lp.tien>0 then b_nv:='T'; else b_nv:='C'; end if;
    PBH_DO_BH_CN_THOP(b_ma_dvi,b_nv,b_ngay_ht,b_nha_bh,r_lp.ma_nt,-abs(r_lp.tien),-abs(r_lp.tien_qd),b_loi);
    if b_loi is not null then return; end if;
     b_ktra:=true;
end loop;
if b_ktra then
...
delete bh_hd_do_pb where dvi_xl=b_ma_dvi and so_id_tt=b_so_id_tt;
delete bh_hd_do_pt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
delete bh_hd_do_tt_txt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
delete bh_hd_do_pp where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
delete bh_hd_do_ct where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
delete bh_hd_do_tt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_tt;
for b_lp in 1..a_so_id.count loop
    PBH_HD_DO_TH_PS(b_ma_dvi,a_so_id(b_lp),a_so_id_ps(b_lp),b_loi);
    if b_loi is not null then return; end if;
end loop;
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_HD_DO_TT_XOA_XOA:loi'; end if;
end;
/
create or replace procedure PBH_HD_DO_TT_NH_NH
...
for b_lp in 1..a_pt.count loop
    if a_pt(b_lp)='C' then
        ...
        PBH_DO_BH_CN_THOP(b_ma_dvi,b_nv,b_ngay_ht,b_nha_bh,a_ma_nt(b_lp),b_tien,b_tien_qd,b_loi);
        if b_loi is not null then return; end if;
        b_ktra:=true;
    end if;
end loop;
...
insert into bh_hd_do_tt values(
...
b_loi:='loi:Loi Table BH_HD_DO_CT:loi';
for b_lp in 1..a_so_id.count loop
    insert into bh_hd_do_ct values(b_ma_dvi,b_so_id_tt,b_lp,a_so_id(b_lp),a_so_id_ps(b_lp),a_nhom(b_lp),a_loai(b_lp),
        a_nv(b_lp),a_ma_nt_tt(b_lp),a_tien_g(b_lp),a_thue_g(b_lp),a_tien_g_qd(b_lp),a_thue_g_qd(b_lp));
end loop;
b_loi:='loi:Loi Table BH_HD_DO_PP:loi';
for b_lp in 1..a_pt.count loop
    insert into bh_hd_do_pp values(b_ma_dvi,b_so_id_tt,b_lp,a_pt(b_lp),a_ma_nt(b_lp),a_tien(b_lp),a_tien_qd(b_lp));
end loop;
insert into bh_hd_do_tt_txt values(b_ma_dvi,b_so_id_tt,'dt_ct',dt_ct);
PBH_DO_TT_PT(b_ma_dvi,b_so_id_tt,b_loi);
if b_loi is not null then return; end if;
for b_lp in 1..a_so_id.count loop
    PBH_HD_DO_TH_PS(b_ma_dvi,a_so_id(b_lp),a_so_id_ps(b_lp),b_loi);
    if b_loi is not null then return; end if;
...
PBH_HD_VAT_JOB_INSERT(b_ma_dvi,b_so_id_tt,'TTD',b_nsd);
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi Table BH_HD_DO_TT:loi'; end if;
end;
/
create or replace procedure PBH_HD_DO_TT_NH(
...
if b_so_id_tt>0 then
    PBH_HD_DO_TT_XOA_VAT(b_ma_dvi,b_so_id_tt,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
    PBH_HD_DO_TT_XOA_XOA(b_ma_dvi,b_nsd,b_so_id_tt,true,b_loi);
else
    PHT_ID_MOI(b_so_id_tt,b_loi);
end if;
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
PBH_HD_DO_TT_TEST(b_ma_dvi,b_so_id_tt,b_phong,dt_ct,dt_hd,b_ngay_ht,b_nha_bh,b_so_ct,
    b_chi_qd,b_thu_qd,b_thue_v_qd,b_thue_r_qd,
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_HD_DO_TT_NH_NH(b_ma_dvi,b_nsd,b_phong,b_so_id_tt,b_ngay_ht,b_nha_bh,b_so_ct,
    b_chi_qd,b_thu_qd,b_thue_v_qd,b_thue_r_qd,
	b_nt_tra,b_pt_tra,b_tra,b_tra_qd,b_cit,b_cit_qd,
    a_so_id,a_so_id_ps,a_nhom,a_loai,a_nv,a_ma_nt_tt,
    a_tien_tt,a_thue_tt,a_tien_tt_qd,a_thue_tt_qd,
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_DO_TT_PBO(b_ma_dvi,b_so_id_tt,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
/
create or replace procedure PBH_HD_DO_TT_XOA
...
PBH_HD_DO_TT_XOA_VAT(b_ma_dvi,b_so_id_tt,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_HD_DO_TT_XOA_XOA(b_ma_dvi,b_nsd,b_so_id_tt,false,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_HD_DO_TH_VAT(b_ma_dvi,b_so_id_tt,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
...
/
create or replace procedure PBH_HD_DO_TT_TIM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select JSON_ARRAYAGG(json_object(
    ...
    (select distinct a.ngay_ht,a.nha_bh,a.so_id_tt,b.so_id from bh_hd_do_tt a, bh_hd_do_ct b where
