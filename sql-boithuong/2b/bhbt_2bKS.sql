create or replace procedure PBH_BT_2B_KBT(
...
select ma,sum(tien) bt_lke bulk collect into dkL_ma,dkL_tien from bh_bt_2b_dk
    where (ma_dvi,so_id) in (select ma_dvi,so_id from bh_bt_2b where so_id_dt=b_so_id_dt and ttrang='D') group by ma;
