create or replace procedure PBH_BT_PKT_KBT(
...
as
    ...
    b_tien_bh number; b_lke number; b_t_that number; b_tien number;
	  b_ngay_xr number; b_ngay_ph number; b_ngay_hl number; b_ngay_kt number; b_kho number;
    dk_ma pht_type.a_var; dk_lh_nv pht_type.a_var;
    dk_tien_bh pht_type.a_num; dk_lke pht_type.a_num; dk_t_that pht_type.a_num; dk_bth pht_type.a_num; dk_luy pht_type.a_var; dk_lkeB pht_type.a_var;
...
--Nam: them ngay phuc hoi
b_ngay_xr:=FKH_JS_GTRIn(dt_ct,'ngay_xr');
b_ngay_ph:=FBH_PKTP_NGAY_PH(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_xr);
select ma,sum(tien) bt_lke bulk collect into dkL_ma,dkL_tien from bh_bt_pkt_dk where (ma_dvi,so_id) in
...
FBH_PKT_DT_NG(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_hl,b_ngay_kt);
        ...
        if b_c is not null then
            ...
            for b_lp1 in 1..kma_ma.count loop
                if kma_ma(b_lp1) in('MVU','GVU','KVU') then
					...
					b_s:='PBH_BT_PKT_KBT_'||kma_ma(b_lp1);
...
/
create or replace procedure PBH_BT_PKT_KBT_GVU(
...
/
create or replace procedure PBH_BT_PKT_KBT_KVU(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_tien_bh number,b_lke number,
    b_t_that number,b_bth number,b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
...
/
create or replace procedure PBH_BT_PKT_KBT_MVU(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_tien_bh number,b_lke number,
    b_t_that number,b_bth number,b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
    b_ndN number:=PKH_LOC_CHU_SO(b_nd,'F');
