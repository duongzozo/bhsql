create or replace procedure PBH_BT_PHH_KBT(
...
select ma,sum(tien) bt_lke bulk collect into dkL_ma,dkL_tien from bh_bt_phh_dk where (ma_dvi,so_id) in
...
FBH_PHH_DT_NG(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_hl,b_ngay_kt);
b_kho:=FKH_KHO_NGSO(b_ngay_hl,b_ngay_kt);
for b_lp in 1..dk_ma.count loop
  if dk_bth(b_lp)>0 then --nam: dk_bth(b_lp)>0
        ...
        if b_c is not null then
            ...
            for b_lp1 in 1..kma_ma.count loop
                if kma_ma(b_lp1) in('MVU','GVU','KVU') then
                    ...
                    b_s:='PBH_BT_PHH_KBT_'||kma_ma(b_lp1);
