create or replace function FBH_BT_PHH_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return varchar2
AS
    b_kq varchar2(100):=' '; b_i1 number; b_txt clob;
begin
-- Dan - Tra gia tri varchar2 trong txt
select count(*) into b_i1 from bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
if b_i1=1 then
...
/
create or replace function FBH_BT_PHH_TTRANG(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra tinh trang
select nvl(min(ttrang),'X') into b_kq from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_PHH_NV(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra nghiep vu
select nvl(min(ttrang),'G') into b_kq from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_PHH_BTH_LKE(
...
b_ngay_ph:=FBH_PHHP_NGAY_PH(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_xr);
select JSON_ARRAYAGG(json_object(ma,bt_lke) returning clob) into dt_bth from
    (select ma,sum(tien) bt_lke from bh_bt_phh_dk where (ma_dvi,so_id) in
    (select ma_dvi,so_id from bh_bt_phh where ma_dvi_ql=b_ma_dvi and so_id_hd=b_so_id and
    so_id_dt=b_so_id_dt and ttrang='D' and ngay_xr between b_ngay_ph and b_ngay_xr) group by ma);
return dt_bth;
end;
/
create or replace procedure PBH_BT_PHH_MO
...
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_ttt
    from bh_kh_ttt where ps='BT' and nv='PHH';
select json_object('cs_kbt' value cs_kbt,'cs_ttt' value cs_ttt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace function FBH_BT_PHH_SO_HS(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(30);
begin
-- Dan - Tra so_id
select min(so_hs) into b_kq from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace procedure FBH_BT_PHH_SO_ID(b_so_hs varchar2,b_ma_dvi out varchar2,b_so_id out number)
AS
begin
-- Dan - Tra ma_dvi, so_id
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_bt_phh where so_hs=b_so_hs;
end;
/
create or replace procedure PBH_BT_PHH_SO_ID(
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_BT_PHH_SO_ID(b_so_hs,b_ma_dvi,b_so_id);
...
/
create or replace procedure PBH_BT_PHH_FBPHI(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select ma_dvi_ql,so_id_hd,so_id_dt into b_ma_dvi_hd,b_so_id_hd,b_so_id_dt from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
select nvl(min(so_idP),0) into b_so_idP from bh_phh_dvi where ma_dvi=b_ma_dvi_hd and so_id=b_so_id_hd and so_id_dt=b_so_id_dt;
b_oraOut:=to_char(b_so_idP);
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_FHD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_ma_dvi varchar2(10); b_so_id number;
...
/
create or replace procedure PBH_BT_PHH_LKE_GCN(
...
b_so_id_hd:=FBH_PHH_HD_SO_IDd(b_ma_dvi_ql,b_so_hd);
...
select JSON_ARRAYAGG(json_object(so_hs,ngay_mo,ttrang,tien,ma_dvi,so_id) order by ngay_mo desc)
    into cs_bth from bh_bt_phh where so_id_dt=b_so_id_dt;
select json_object('cs_phi' value cs_phi,'cs_bth' value cs_bth) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_GCN(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    ...
    dt_ct clob; dt_kbt clob; dt_btlke clob; dt_dk clob; dt_lt clob; dt_pvi clob; dt_hk clob;
begin
...
FBH_PHH_SO_ID_DTf(b_so_id_dt,b_ma_dvi,b_so_idG);
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
b_so_id:=FBH_PHH_SO_IDb(b_ma_dvi,b_so_idG,b_ngay_xr);
select nt_tien into b_nt_tien from bh_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
select json_object(a.so_hd,b.gcn,a.ten,'ma_dvi_ql' value a.ma_dvi,b.dvi,b.ngay_hl,b.ngay_kt,a.nt_tien,a.c_thue,
    ...
    from bh_phh a,bh_phh_dvi b where a.ma_dvi=b.ma_dvi and a.so_id=b.so_id and
...
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    ...
    order by bt returning clob) into dt_dk from bh_phh_dk where
    ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt and pvi_ma=' ' and nv='C';
-- nam: lay dklt, dkbt
select lt,kbt into dt_lt,dt_kbt from bh_phh_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
...
dt_btlke:=FBH_BT_PHH_BTH_LKE(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_xr);
select JSON_ARRAYAGG(json_object('ten' value FBH_PHH_PVI_TEN(ma),'chon' value '',ma,ktru) order by ma returning clob) into dt_pvi from
    (select distinct pvi_ma ma,pvi_ktru ktru from bh_phh_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt and pvi_ma<>' ');
...
/
create or replace procedure PBH_BT_PHH_GCNdk(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    ...
    order by bt returning clob) into b_oraOut from bh_phh_dk where
    ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt and pvi_ma=' '; 
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_GCNpv(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(2000); b_lenh varchar2(2000);
    b_so_id_dt number; b_ngay_xr number;
...
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
b_so_id:=FBH_PHH_SO_IDb(b_ma_dvi,b_so_idG,b_ngay_xr);
select JSON_ARRAYAGG(json_object('ten' value FBH_PHH_PVI_TEN(ma),'chon' value '',ma,ktru) order by ma returning clob) into b_oraOut from
    (select distinct pvi_ma ma,pvi_ktru ktru from bh_phh_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt and pvi_ma<>' ');
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_GCNd(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_PHH_SO_ID_GCN(b_oraIn,b_ma_dvi,b_so_id,b_so_id_dt);
select json_object('so_id_dt' value b_so_id_dt) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_LKE(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(json_object(ma_dvi,so_id,so_hs,nsd) order by so_hs desc) into cs_lke from
        (select ma_dvi,so_id,so_hs,nsd,rownum sott from bh_bt_phh where
        ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hs desc)
        where sott between b_tu and b_den;
elsif b_klk='P' then
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(json_object(ma_dvi,so_id,so_hs,nsd) order by so_hs desc) into cs_lke from
        (select ma_dvi,so_id,so_hs,nsd,rownum sott from bh_bt_phh where
        ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong order by so_hs desc)
        where sott between b_tu and b_den;
elsif FHT_MA_NSD_QU(b_ma_dvi,b_nsd,'BH','BT','X')='C' then
    select count(*) into b_dong from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(json_object(ma_dvi,so_id,so_hs,nsd) order by so_hs desc) into cs_lke from
        (select ma_dvi,so_id,so_hs,nsd,rownum sott from bh_bt_phh where
        ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs desc)
        where sott between b_tu and b_den;
end if;
...
/
create or replace procedure PBH_BT_PHH_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hs:=FBH_BT_PHH_SO_HS(b_ma_dvi,b_so_id);
if b_so_hs is null then b_loi:='loi:Ho so da xoa:loi'; raise PROGRAM_ERROR; end if;
if b_klk ='N' then
    select count(*) into b_dong from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hs desc)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott
		from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hs desc)
        where sott between b_tu and b_den;
elsif b_klk='P' then
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong order by so_hs desc)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott
		from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong order by so_hs desc)
        where sott between b_tu and b_den;
elsif FHT_MA_NSD_QU(b_ma_dvi,b_nsd,'BH','BT','X')='C' then
    select count(*) into b_dong from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs desc)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_phh where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs desc)
        where sott between b_tu and b_den;
end if;
...
/
create or replace procedure PBH_BT_PHH_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
EXECUTE IMMEDIATE b_lenh into b_ma_dvi,b_so_id using b_oraIn;
b_loi:='loi:Ho so da xoa:loi';
select ma_dvi_ql,so_id_hd,so_id_dt,ngay_xr into b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ngay_xr from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_so_id_hdB:=Fbh_phh_SO_IDb(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select lt,kbt into dt_lt,dt_kbt from bh_phh_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
select json_object(so_hs,tien,tienHK,'ma_nn' value FBH_PHH_NNTT_TENl(ma_nn) returning clob)
    into dt_ct from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
select JSON_ARRAYAGG(json_object(ten,tien) order by bt) into dt_hk
    from bh_bt_phh_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ten,ma_nt,tien) order by bt) into dt_tba from bh_bt_phh_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai not in('dt_tba','dt_kbt','dt_pvi');
select count(*) into b_i1 from bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
select count(*) into b_i1 from bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_pvi';
if b_i1<>0 then
    select txt into dt_pvi from bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_pvi';
end if;
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,'so_id_dt' value b_so_id_dt,
    ...
    'dt_tba' value dt_tba,'dt_thoi' value dt_thoi,'dt_hk' value dt_hk,
    'dt_ct' value dt_ct,'dt_dk' value dt_dk,'txt' value dt_txt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_TESTr(
...
AS
    b_lenh varchar2(2000); b_i1 number; b_i2 number; b_tp number:=0; b_tg number;
...
b_so_id_hd:=FBH_HD_GOC_SO_ID(b_ma_dvi_ql,b_so_hd);
...
b_so_id_hdB:=Fbh_phh_SO_IDb(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select count(*) into b_i1 from bh_phh where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and nt_tien=b_nt_tien;
...
select min(gcn) into b_gcn from bh_phh_dvi where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and
    so_id_dt=b_so_id_dt and b_ngay_xr between ngay_hl and ngay_kt;
if b_gcn is null then b_loi:='loi:Ngay xay ra ngoai hieu luc bao hiem:loi'; return; end if;
select so_hd,ma_kh,ten,ma_sp into b_so_hd,b_ma_kh,b_ten,b_ma_sp from bh_phh where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB;
...
if b_ttrang in('T','D') then
    ...
    if FBH_PHH_HLd(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ngay_xr)='K' then
    ...
    select kbt into dt_kbt from bh_phh_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
    dt_kbt:=FKH_JS_BONH(dt_kbt);
    PBH_BT_PHH_KBT(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,dt_ct,dt_dk,dt_kbt,b_loi);
...
/
create or replace procedure PBH_BT_PHH_XOA_XOA
    (b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_nh boolean,b_loi out varchar2)
AS 
    b_i1 number; r_hs bh_bt_phh%rowtype;
Begin
-- Dan - Xoa boi thuong
select count(*) into b_i1 from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1=0 then b_loi:=''; return; end if;
b_loi:='loi:Loi xoa ho so:loi';
select * into r_hs from bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
if b_loi is not null then return; end if;
delete bh_bt_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_phh_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_phh_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_phh_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
--nam: xoa du phong
delete bh_bt_phh_duph where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_PHH_NH_NH(
...
AS
    b_i1 number; b_so_id_kt number:=-1;
    b_dvi_ksoat varchar2(10):=''; b_ksoat varchar2(10):=''; b_duph varchar2(1);
begin
-- Dan - Nhap boi thuong
...
b_loi:='loi:Loi Table bh_bt_phh:loi';
insert into bh_bt_phh values(b_ma_dvi,b_so_id,b_ngay_ht,b_so_hs,b_ttrang,b_kieu_hs,b_so_hs_g,
...
b_loi:='loi:Loi Table bh_bt_phh_dk:loi';
for b_lp in 1..dk_ma.count loop
    insert into bh_bt_phh_dk values(b_ma_dvi,b_so_id,b_lp,
        ...
        dk_tien_qd(b_lp),dk_thue_qd(b_lp),dk_tien_qd(b_lp)+dk_thue_qd(b_lp),dk_lkeB(b_lp),dk_luy(b_lp),dk_ktru(b_lp));
end loop;
if hk_tien.count<>0 then
    b_loi:='loi:Loi Table bh_bt_phh_HK:loi';
    for b_lp in 1..hk_tien.count loop
        insert into bh_bt_phh_hk values(
...
if tba_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_phh_tba:loi';
    for b_lp in 1..tba_ma_nt.count loop
        insert into bh_bt_phh_tba values(b_ma_dvi,b_so_id,b_lp,tba_ten(b_lp),tba_ma_nt(b_lp),tba_tien(b_lp));
    end loop;
end if;
insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if length(dt_hk)<>0 then
    insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if length(dt_tba)<>0 then
    insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_tba',dt_tba);
end if;
if length(dt_kbt)<>0 then
    insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
if length(dt_ttt)<>0 then
    insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if length(dt_pvi)<>0 then
    insert into bh_bt_phh_txt values(b_ma_dvi,b_so_id,'dt_pvi',dt_pvi);
end if;
PBH_BT_HSBS_NH(b_ma_dvi,b_so_id,b_so_hs,b_ma_dvi_ql,b_so_hd,b_tien+b_thue,
    dt_ct,dt_dk,dt_hk,dt_tba,'',dt_kbt,b_loi);
if b_loi is not null then return; end if;
PBH_BT_PHH_GOC(b_ma_dvi,b_nsd,b_so_id,b_duph,b_loi);
if b_loi is not null then return; end if;
if b_ttrang='T' and b_duph='C' then
    b_i1:=PKH_NG_CSO(sysdate);
    insert into bh_bt_phh_duph select a.*,b_i1 from bh_bt_phh_txt a where ma_dvi=b_ma_dvi and so_id=b_so_id;
end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_PHH_NH(
...
if b_so_id<>0 then
    select count(*) into b_i1 from bh_bt_phh where so_id=b_so_id;
...
if b_so_id=0 then
    ...
    select ma_dvi,ngay_ht into b_ma_dviC,b_ngay_htC from bh_bt_phh where so_id=b_so_id for update nowait;
    if sql%rowcount=0 then raise PROGRAM_ERROR; end if;
    PBH_BT_PHH_XOA_XOA(b_ma_dviC,b_nsd,b_so_id,true,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
end if;
...
-- Test rieng
    ...
    dk_cap,dk_ma_dk,dk_ma_dkC,dk_lh_nv,dk_t_suat,dk_lkeB,dk_luy,dk_ktru,
    hk_ma,hk_ten,hk_ma_nt,hk_tien,hk_thue,hk_tien_qd,hk_thue_qd,
    tba_ten,tba_ma_nt,tba_tien,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
/
create or replace procedure PBH_BT_PHH_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_comm varchar2:='C')
AS
...
PBH_BT_PHH_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,false,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PHH_GOC(
...
b_loi:='loi:Loi xu ly PBH_BT_PHH_GOC:loi';
...
b_so_idB:=FBH_PHH_SO_IDb(b_ma_dvi_ql,b_so_id_hd);
select ma_dt into b_ma_dt from bh_phh_dvi where ma_dvi=b_ma_dvi_ql and so_id=b_so_idB and so_id_dt=b_so_id_dt;
insert into bh_bt_goc_temp1
    select lh_nv,sum(tien_bh),max(pt_bt),sum(t_that),sum(tien),sum(tien_qd),sum(thue),sum(thue_qd)
    from bh_bt_phh_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_nv<>' ' group by lh_nv;
select 'PHH',ngay_ht,so_hs,ttrang,kieu_hs,so_hs_g,ma_dvi_ql,so_hd,ma_kh,ten,' ',' ',
    ngay_gui,ngay_xr,ngay_do,n_trinh,n_duyet,ngay_qd,nop,bphi,dung,traN,'bh_bt_phh'
...
select ten,ma_nt,tien bulk collect into tba_ten,tba_ma_nt,tba_tien
    from bh_bt_phh_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_goc_temp1;
...
/
create or replace procedure PBH_BT_PHH_HSBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_hs_g <>' ' then
  ...
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_phh where ma_dvi=b_ma_dvi and so_hs=b_so_hs_g union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_phh where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs_g);
else 
  b_so_hs:=FBH_BT_HS_SOHS(b_ma_dvi,b_so_id);
  select JSON_ARRAYAGG(json_object(so_hs,'ma_dvi' value ma_dvi_ql,so_hd,tien,'ngay' value ngay_ht,so_id,'csot' value '') order by so_id desc returning clob) into cs_lke from 
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_phh where ma_dvi=b_ma_dvi and so_hs=b_so_hs union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_phh where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs);
end if;
