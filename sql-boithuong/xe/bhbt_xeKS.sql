create or replace procedure PBH_BT_XE_KBT(
...
b_lenh:=FKH_JS_LENH('ma,lh_nv,tien_bh,t_that,tien,luy,lkeM');
EXECUTE IMMEDIATE b_lenh bulk collect into dk_ma,dk_lh_nv,dk_tien_bh,dk_t_that,dk_bth,dk_luy,dk_lkeM using dt_dk;
b_lenh:=FKH_JS_LENH('ma,kbt');
EXECUTE IMMEDIATE b_lenh bulk collect into kbt_ma,kbt_kbt using dt_kbt;
select ma,sum(tien) bt_lke bulk collect into dkL_ma,dkL_tien from bh_bt_xe_dk
    where (ma_dvi,so_id) in (select ma_dvi,so_id from bh_bt_xe where so_id_dt=b_so_id_dt and ttrang='D') group by ma;
...
for b_lp in 1..dk_ma.count loop
  if dk_bth(b_lp)>0 then
    if dk_tien_bh(b_lp)<>0 then
        ...
        if b_i1>dk_tien_bh(b_lp) and dk_lkeM(b_lp)<>'M' then b_loi:='loi:Dieu khoan '||dk_ma(b_lp)||' boi thuong vuot muc trach nhiem:loi'; return; end if;
    end if;
        ...
        if b_c is not null then
            ...
            for b_lp1 in 1..kma_ma.count loop
                if kma_ma(b_lp1) in('MVU','GVU','KVU') then
                  -- viet anh -- bo KVU cho xe -- cong yeu cau
                   if kma_ma(b_lp1) in('MVU','GVU') then
                      b_loi:=FBH_BT_KBT(kma_ma(b_lp1),dk_tien_bh(b_lp),dk_t_that(b_lp),dk_bth(b_lp),kma_nd(b_lp1));
                   end if; 
                else
                    b_s:='PBH_BT_XE_KBT_'||kma_ma(b_lp1);
