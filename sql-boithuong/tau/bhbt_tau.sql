create or replace function FBH_BT_TAU_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return varchar2
AS
    b_kq varchar2(100):=' '; b_i1 number; b_txt clob;
begin
-- Dan - Tra gia tri varchar2 trong txt
select count(*) into b_i1 from bh_bt_tau_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
if b_i1=1 then
...
/
create or replace function FBH_BT_TAU_TTRANG(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra tinh trang
select nvl(min(ttrang),'X') into b_kq from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_TAU_NV(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra nghiep vu
select nvl(min(ttrang),'G') into b_kq from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_TAU_BTH_LKE(
...
select JSON_ARRAYAGG(json_object(ma,bt_lke) returning clob) into dt_bth from
    (select ma,sum(tien) bt_lke from bh_bt_tau_dk where (ma_dvi,so_id) in 
    (select ma_dvi,so_id from bh_bt_tau where ma_dvi=b_ma_dvi and so_id_hd=b_so_id 
            and so_id_dt=b_so_id_dt and ttrang='D') group by ma);
return dt_bth;
end;
/
create or replace procedure PBH_BT_TAU_MO
...
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_ttt
    from bh_kh_ttt where ps='BT' and nv='TAU';
select json_object('cs_kbt' value cs_kbt,'cs_ttt' value cs_ttt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace function FBH_BT_TAU_SO_HS(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(30);
begin
-- Dan - Tra so_id
select min(so_hs) into b_kq from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace procedure FBH_BT_TAU_SO_ID(b_so_hs varchar2,b_ma_dvi out varchar2,b_so_id out number)
AS
begin
-- Dan - Tra ma_dvi, so_id
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_bt_tau where so_hs=b_so_hs;
end;
/
create or replace procedure PBH_BT_TAU_SO_ID(
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_BT_TAU_SO_ID(b_so_hs,b_ma_dvi,b_so_id);
...
/
create or replace procedure PBH_BT_TAU_FBPHI(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select ma_dvi_ql,so_id_hd,so_id_dt into b_ma_dvi_hd,b_so_id_hd,b_so_id_dt from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
select nvl(min(so_id),0) into b_so_idP from bh_tau_ds where ma_dvi=b_ma_dvi_hd and so_id=b_so_id_hd and so_id_dt=b_so_id_dt;
b_oraOut:=to_char(b_so_idP);
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_TAU_FHD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_ma_dvi varchar2(10); b_so_id number;
...
/
create or replace procedure PBH_BT_TAU_LKE_GCN
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_TAU_HD_SO_ID_DTd(b_gcn,b_ma_dvi_ql,b_so_id_hd,b_so_id_dt);
...
select JSON_ARRAYAGG(json_object(so_hs,ngay_mo,ttrang,tien,ma_dvi,so_id) order by ngay_mo desc)
    into cs_bth from bh_bt_tau where so_id_dt=b_so_id_dt;
select json_object('cs_phi' value cs_phi,'cs_bth' value cs_bth) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_TAU_GCN(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
FBH_TAU_SO_ID_DTf(b_so_id_dt,b_ma_dvi,b_so_idG,b_ngay_xr);
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
b_so_id:=FBH_TAU_SO_IDb(b_ma_dvi,b_so_idG,b_ngay_xr);
...
select json_object(a.so_hd,a.ten,'ma_dvi_ql' value a.ma_dvi,b.gcn,b.tenC,
    'so_dk' value so_dk||'/'||ten_tau,b.ng_huong,'tygia' value b_tg,
    b.ngay_hl,b.ngay_kt,a.nt_tien,a.c_thue,'lhe_ten' value a.ten,'lhe_mobi' value a.mobi,'lhe_mail' value a.email) into dt_ct
    from bh_tau a,bh_tau_ds b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id and
    b.ma_dvi=b_ma_dvi and b.so_id=b_so_id and b.so_id_dt=b_so_id_dt;
select lt,kbt into dt_lt,dt_kbt from bh_tau_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    ...
    cap,tc,ma_ct,ma_dk,kieu,lh_nv,t_suat,ptB,lkeP,lkeB,luy)
    order by bt returning clob) into dt_dk from bh_tau_dk where
    ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
dt_btlke:=FBH_BT_TAU_BTH_LKE(b_ma_dvi,b_so_id,b_so_id_dt);
...
/
create or replace procedure PBH_BT_TAU_GCNd(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_TAU_SO_ID_GCN(b_oraIn,b_ma_dvi,b_so_id,b_so_id_dt);
select json_object('so_id_dt' value b_so_id_dt) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_TAU_GCNdk(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(2000); b_lenh varchar2(2000);
...
FBH_TAU_SO_ID_DTf(b_so_id_dt,b_ma_dvi,b_so_idG);
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
b_so_id:=FBH_TAU_SO_IDb(b_ma_dvi,b_so_idG,b_ngay_xr);
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    'pt_bt' value 0,'ycau' value 0,'t_that' value 0,'tien' value 0,'nd' value '',
    'bt_con' value 0,'bt_lke' value 0,
    cap,tc,ma_ct,ma_dk,kieu,lh_nv,t_suat,ptB,lkeP,lkeB,luy,'bt' value bt)
    order by bt returning clob) into b_oraOut from bh_tau_dk where
    ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_TAU_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_tau where
    ...
    select count(*) into b_dong from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_tau  where
    ...
    select count(*) into b_dong from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_tau  where
...
/
create or replace procedure PBH_BT_TAU_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hs:=FBH_BT_TAU_SO_HS(b_ma_dvi,b_so_id);
if b_so_hs is null then b_loi:='loi:Ho so da xoa:loi'; raise PROGRAM_ERROR; end if;
if b_klk ='N' then
    select count(*) into b_dong from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hs)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs)
        where sott between b_tu and b_den;
elsif b_klk='P' or FHT_MA_NSD_QU(b_ma_dvi,b_nsd,'BH','BT','X')<>'C' then
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong order by so_hs)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs)
    ...
    select count(*) into b_dong from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hd)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_tau where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hd)
        where sott between b_tu and b_den;
end if;
...
/
create or replace procedure PBH_BT_TAU_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select lt,kbt into dt_lt,dt_kbt from bh_tau_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
select json_object(so_hs,tien,tienHK,'ma_nn' value FBH_TAU_NNTT_TENl(ma_nn),'n_duyet' value PHT_MA_CBJ_TENl(n_duyet) returning clob)
    into dt_ct from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ma) order by bt) into dt_dk from bh_bt_tau_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ma,ten,tien) order by bt) into dt_hk
    from bh_bt_tau_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ten,ma_nt,tien) order by bt) into dt_tba
    from bh_bt_tau_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(loai,txt) returning clob) into dt_txt
    from bh_bt_tau_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai not in('dt_tba','dt_kbt');
select count(*) into b_i1 from bh_bt_tau_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_bt_tau_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
...
/
create or replace procedure PBH_BT_TAU_TESTr(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id_dt number,
    dt_ct clob,dt_dk clob,dt_hk clob,dt_tba in out clob,dt_kbt out clob,
    ...
    b_so_dk out varchar2,b_ten_tau out nvarchar2,b_ng_huong out nvarchar2,
...
b_so_id_hd:=FBH_TAU_SO_ID(b_ma_dvi_ql,b_so_hd);
...
b_so_id_hdB:=FBH_TAU_SO_IDb(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select count(*) into b_i1 from bh_tau where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and nt_tien=b_nt_tien and c_thue=b_c_thue;
...
select count(*) into b_i1 from bh_tau_ds where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and
...
select gcn,so_dk,ten_tau,ng_huong,ma_sp into b_gcn,b_so_dk,b_ten_tau,b_ng_huong,b_ma_sp
    from bh_tau_ds where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
select so_hd,ma_kh,ten into b_so_hd,b_ma_kh,b_ten from bh_tau where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB;
...
EXECUTE IMMEDIATE b_lenh bulk collect into
    ...
    dk_cap,dk_ma_dk,dk_ma_bs,dk_lh_nv,dk_t_suat,dk_lkeB,dk_bt_con using dt_dk;
...
if trim(dt_hk) is null then
    ...
    for b_lp in 1..hk_tien.count loop
        ...
        hk_tien(b_lp):=nvl(hk_tien(b_lp),0); hk_thue(b_lp):=nvl(hk_thue(b_lp),0);
...
if trim(dt_tba) is null then
    ...
    for b_lp in 1..tba_ten.count loop
...
if b_ttrang in('T','D') then
    ...
    if FBH_TAU_HLd(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ngay_xr)='K' then
    ...
    select kbt into dt_kbt from bh_tau_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
    dt_kbt:=FKH_JS_BONH(dt_kbt);
    PBH_BT_TAU_KBT(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,dt_ct,dt_dk,dt_kbt,b_loi);
...
/
create or replace procedure PBH_BT_TAU_XOA_XOA
    (b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_nh boolean,b_loi out varchar2)
AS 
    b_i1 number; r_hs bh_bt_tau%rowtype;
Begin
-- Dan - Xoa boi thuong
select count(*) into b_i1 from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1=0 then b_loi:=''; return; end if;
b_loi:='loi:Loi xoa ho so:loi';
select * into r_hs from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
if b_loi is not null then return; end if;
delete bh_bt_tau_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_tau_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_tau_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_tau_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_tau_ct where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
--nam: xoa du phong
delete bh_bt_tau_duph where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_TAU_NH_NH(
...
-- Test rieng
    ...
    b_so_dk varchar2,b_ten_tau nvarchar2,b_ng_huong nvarchar2,
...
b_loi:='loi:Loi Table bh_bt_tau:loi';
insert into bh_bt_tau values(b_ma_dvi,b_so_id,b_ngay_ht,b_so_hs,b_ttrang,b_kieu_hs,b_so_hs_g,
...
b_loi:='loi:Loi Table bh_bt_tau_ct:loi';
insert into bh_bt_tau_ct values(b_ma_dvi,b_so_id,b_so_dk,b_ten_tau,b_ng_huong);
b_loi:='loi:Loi Table bh_bt_tau_dk:loi';
for b_lp in 1..dk_ma.count loop
    insert into bh_bt_tau_dk values(b_ma_dvi,b_so_id,b_lp,
...
if hk_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_tau_HK:loi';
    for b_lp in 1..hk_ma_nt.count loop
        insert into bh_bt_tau_hk values(
...
if tba_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_tau_tba:loi';
    for b_lp in 1..tba_ma_nt.count loop
        insert into bh_bt_tau_tba values(b_ma_dvi,b_so_id,b_lp,tba_ten(b_lp),tba_ma_nt(b_lp),tba_tien(b_lp));
    end loop;
end if;
insert into bh_bt_tau_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_bt_tau_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if dt_hk is not null then
    insert into bh_bt_tau_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if dt_tba is not null then
    insert into bh_bt_tau_txt values(b_ma_dvi,b_so_id,'dt_tba',dt_tba);
end if;
if dt_kbt is not null then
    insert into bh_bt_tau_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
if dt_ttt is not null then
    insert into bh_bt_tau_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
PBH_BT_HSBS_NH(b_ma_dvi,b_so_id,b_so_hs,b_ma_dvi_ql,b_so_hd,b_tien+b_thue,
    dt_ct,dt_dk,dt_hk,dt_tba,'',dt_kbt,b_loi);
if b_loi is not null then return; end if;
PBH_BT_TAU_GOC(b_ma_dvi,b_nsd,b_so_id,b_duph,b_loi);
if b_loi is not null then return; end if;
if b_ttrang='T' and b_duph='C' then
    b_i1:=PKH_NG_CSO(sysdate);
    insert into bh_bt_tau_duph select a.*,b_i1 from bh_bt_tau_txt a where ma_dvi=b_ma_dvi and so_id=b_so_id;
end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_TAU_NH(
...
-- Test rieng
    ...
    b_so_dk varchar2(20); b_ten_tau nvarchar2(500); b_ng_huong nvarchar2(1000);
    dk_ma pht_type.a_var; dk_ten pht_type.a_nvar; dk_tc pht_type.a_var; dk_ma_ct pht_type.a_var;
    dk_tien_bh pht_type.a_num; dk_pt_bt pht_type.a_num; dk_t_that pht_type.a_num;
    dk_tien pht_type.a_num; dk_thue pht_type.a_num; dk_tien_qd pht_type.a_num; dk_thue_qd pht_type.a_num;
    dk_cap pht_type.a_var; dk_ma_dk pht_type.a_var; dk_ma_bs pht_type.a_var;
...
if b_so_id<>0 then
    select count(*) into b_i1 from bh_bt_tau where so_id=b_so_id;
...
if b_so_id=0 then
    ...
    select ma_dvi,ngay_ht into b_ma_dviC,b_ngay_htC from bh_bt_tau where so_id=b_so_id for update nowait;
    if sql%rowcount=0 then raise PROGRAM_ERROR; end if;
    PBH_BT_TAU_XOA_XOA(b_ma_dviC,b_nsd,b_so_id,true,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
end if;
...
PBH_BT_TAU_TESTr(
    ...
    b_gcn,b_ma_dvi_ql,b_so_hd,b_so_id_hd,b_ma_kh,b_ten,b_tienHK,b_ma_nn,b_so_dk,b_ten_tau,b_ng_huong,
...
-- Test rieng
    b_gcn,b_ma_dvi_ql,b_so_hd,b_so_id_hd,b_so_id_dt,b_ma_kh,b_ten,b_tienHK,b_ma_nn,b_so_dk,b_ten_tau,b_ng_huong,
...
/
create or replace procedure PBH_BT_TAU_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_comm varchar2:='C')
AS
    b_loi varchar2(100); b_so_id number:=FKH_JS_GTRIn(b_oraIn,'so_id');
begin
-- Dan - Xoa GCN
if b_comm='C' then
    b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','BT','N',b_comm);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
end if;
PBH_BT_TAU_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,false,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_TAU_GOC(
...
select ma_dvi_ql,so_id_hd,so_id_dt,nt_tien into b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ma_nt
    from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_so_idB:=FBH_TAU_SO_IDb(b_ma_dvi_ql,b_so_id_hd);
select nhom into b_ma_dt from bh_tau_ds where ma_dvi=b_ma_dvi_ql and so_id=b_so_idB and so_id_dt=b_so_id_dt;
insert into bh_bt_goc_temp1
    select lh_nv,sum(tien_bh),max(pt_bt),sum(t_that),sum(tien),sum(tien_qd),sum(thue),sum(thue_qd)
    from bh_bt_tau_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_nv<>' ' group by lh_nv;
select 'TAU',ngay_ht,so_hs,ttrang,kieu_hs,so_hs_g,ma_dvi_ql,so_hd,ma_kh,ten,' ',
    ngay_gui,ngay_xr,ngay_do,n_trinh,n_duyet,ngay_qd,nop,bphi,dung,traN,'bh_bt_tau'
    into b_nv,b_ngay_ht,b_so_hs,b_ttrang,b_kieu_hs,b_so_hs_g,b_ma_dvi_ql,b_so_hd,b_ma_kh,b_ten,b_ma_khH,
    b_ngay_gui,b_ngay_xr,b_ngay_do,b_n_trinh,b_n_duyet,b_ngay_qd,b_noP,b_bphi,b_dung,b_traN,b_bangG
    from bh_bt_tau where ma_dvi=b_ma_dvi and so_id=b_so_id;
select ng_huong into b_tenH from bh_bt_tau_ct where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_BT_TAU_HSBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_hs_g <>' ' then
  ...
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_tau where ma_dvi=b_ma_dvi and so_hs=b_so_hs_g union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_tau where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs_g);
else 
  b_so_hs:=FBH_BT_HS_SOHS(b_ma_dvi,b_so_id);
  select JSON_ARRAYAGG(json_object(so_hs,'ma_dvi' value ma_dvi_ql,so_hd,tien,'ngay' value ngay_ht,so_id,'csot' value '') order by so_id desc returning clob) into cs_lke from 
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_tau where ma_dvi=b_ma_dvi and so_hs=b_so_hs union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_tau where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs);
end if;
