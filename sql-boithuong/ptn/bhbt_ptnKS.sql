create or replace procedure PBH_BT_PTN_KBT(
...
select ma,sum(tien) bt_lke bulk collect into dkL_ma,dkL_tien from bh_bt_ptn_dk
    where (ma_dvi,so_id) in (select ma_dvi,so_id from bh_bt_ptn where so_id_dt=b_so_id_dt and ttrang='D') group by ma;
        ...
        if b_c is not null then
            ...
            for b_lp1 in 1..kma_ma.count loop
                if kma_ma(b_lp1) in('MVU','GVU','KVU') then
					...
					b_s:='PBH_BT_PTN_KBT_'||kma_ma(b_lp1);
...
/
create or replace procedure PBH_BT_PTN_KBT_GVU(
...
/
create or replace procedure PBH_BT_PTN_KBT_KVU(
    b_ma_dvi varchar2,b_so_id number,b_tien_bh number,b_lke number,b_t_that number,b_bth number,b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
...
/
create or replace procedure PBH_BT_PTN_KBT_MVU(
    b_ma_dvi varchar2,b_so_id number,b_tien_bh number,b_lke number,b_t_that number,b_bth number,b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
    b_ndN number:=PKH_LOC_CHU_SO(b_nd,'F');
begin
