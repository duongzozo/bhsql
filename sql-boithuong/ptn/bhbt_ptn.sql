create or replace function FBH_BT_PTN_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return varchar2
AS
    b_kq varchar2(100):=' '; b_i1 number; b_txt clob;
begin
-- Dan - Tra gia tri varchar2 trong txt
select count(*) into b_i1 from bh_bt_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
if b_i1=1 then
...
/
create or replace function FBH_BT_PTN_TTRANG(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra tinh trang
select nvl(min(ttrang),'X') into b_kq from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_PTN_NV(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(10);
begin
-- nampb - Tra nghiep vu
select min(nv) into b_kq from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_PTN_BTH_LKE(
...
select JSON_ARRAYAGG(json_object(ma,bt_lke) returning clob) into dt_bth from
    (select ma,sum(tien) bt_lke from bh_bt_ptn_dk where (ma_dvi,so_id) in
    (select ma_dvi,so_id from bh_bt_ptn where ma_dvi_ql=b_ma_dvi and so_id_hd=b_so_id and so_id_dt=b_so_id_dt and ttrang='D') group by ma);
return dt_bth;
end;
/
create or replace function FBH_PTN_NNTT_TENl(b_ma varchar2) return nvarchar2
AS
    b_kq nvarchar2(200);
begin
-- Nam
select min(ma||'|'||ten) into b_kq from bh_ptn_nntt where ma=b_ma and tc='C';
return b_kq;
end;
/
create or replace procedure FBH_BT_PTN_SO_IDh(b_so_hd varchar2,b_ma_dvi out varchar2,b_so_id out number)
AS
begin
-- Dan - Tra ma_dvi, so_id
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_bt_ptn where so_hd=b_so_hd;
end;
/
create or replace procedure FBH_PTN_SO_ID_HD(
...
select count(*) into b_so_id from bh_ptn_dvi where gcn=b_so_gcn;
if b_so_id<>1 then
    b_ma_dvi:=''; b_so_id:=0; b_so_id_dt:=0;
else
    select ma_dvi,so_id,so_id_dt into b_ma_dvi,b_so_id,b_so_id_dt from bh_ptn_dvi where gcn=b_so_gcn;
    b_so_id:=FBH_PTN_SO_IDbt(b_ma_dvi,b_so_id,b_ngay);
end if;
end;
/
create or replace procedure PBH_BT_PTN_SO_IDh(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_ma_dvi varchar2(10); b_gcn varchar2(20):=FKH_JS_GTRIs(b_oraIn,'gcn');
...
FBH_PTN_SO_ID_HD(b_gcn,b_ma_dvi,b_so_id,b_so_id_dt);
...
/
create or replace procedure PBH_BT_PTN_MO
...
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_ttt
    from bh_kh_ttt where ps='BT' and nv='PTN';
select json_object('cs_kbt' value cs_kbt, 'cs_ttt' value cs_ttt) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace function FBH_BT_PTN_SO_HS(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(30);
begin
-- Dan - Tra so_id
select min(so_hs) into b_kq from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace procedure FBH_BT_PTN_SO_ID(b_so_hs varchar2,b_ma_dvi out varchar2,b_so_id out number)
AS
begin
-- Dan - Tra ma_dvi, so_id
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_bt_ptn where so_hs=b_so_hs;
end;
/
create or replace procedure PBH_BT_PTN_SO_ID(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_BT_PTN_SO_ID(b_so_hs,b_ma_dvi,b_so_id);
if b_so_id=0 then b_loi:='loi:Ho so da xoa:loi'; raise PROGRAM_ERROR; end if;
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PTN_FBPHI(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_ma_dvi varchar2(10); b_so_id number; b_nv varchar2(10);
    b_ma_dvi_hd varchar2(10); b_so_id_hd number; b_so_idP number; b_so_id_dt number;
begin
-- Dan - Tra ma_dvi,so_id
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
select ma_dvi_ql,so_id_hd,nv,so_id_dt into b_ma_dvi_hd,b_so_id_hd,b_nv,b_so_id_dt from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
select nvl(min(so_idp),0) into b_so_idP from bh_ptn_dvi where ma_dvi=b_ma_dvi_hd and so_id=b_so_id_hd and so_id_dt=b_so_id_dt;
select json_object('so_idP' value b_so_idP,'nv' value b_nv) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PTN_FHD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_ma_dvi varchar2(10); b_so_id number;
    b_ma_dvi_hd varchar2(10); b_so_id_hd number; b_nv varchar(5); b_so_id_dt number;
begin
-- Dan - Tra ma_dvi,so_id
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
/
create or replace procedure PBH_BT_PTN_LKE_GCN
...
begin
-- Nam - Liet ke phi va boi thuong cua 1 GCN
...
b_so_id_hd:=FBH_PTN_HD_SO_IDd(b_ma_dvi_ql,b_so_hd);
...
select JSON_ARRAYAGG(json_object(so_hs,ngay_mo,ttrang,tien,ma_dvi,so_id) order by ngay_mo desc)
    into cs_bth from bh_bt_ptn where so_id_dt=b_so_id_dt;
select json_object('cs_phi' value cs_phi,'cs_bth' value cs_bth) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PTN_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd and nv<>'TNVC';
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_ptn where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd and nv<>'TNVC' order by so_hs)
        where sott between b_tu and b_den;
elsif b_klk='P' or FHT_MA_NSD_QU(b_ma_dvi,b_nsd,'BH','BT','X')<>'C' then
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong and nv<>'TNVC';
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_ptn  where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong and nv<>'TNVC' order by so_hs)
        where sott between b_tu and b_den;
else
    select count(*) into b_dong from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC';
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_ptn  where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC' order by so_hs)
        where sott between b_tu and b_den;
end if;
...
/
create or replace procedure PBH_BT_PTNVC_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_phong varchar2(10);
    b_ngay_ht number; b_klk varchar2(1); b_tu number; b_den number;
...
/
create or replace procedure PBH_BT_PTN_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hs:=FBH_BT_PTN_SO_HS(b_ma_dvi,b_so_id);
if b_so_hs is null then b_loi:='loi:Ho so da xoa:loi'; raise PROGRAM_ERROR; end if;
if b_klk ='N' then
    select count(*) into b_dong from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht  and nv<>'TNVC';
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd and nv<>'TNVC' order by so_hs)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC' order by so_hs)
        where sott between b_tu and b_den;
elsif b_klk='P' then
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong and nv<>'TNVC';
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong and nv<>'TNVC' order by so_hs)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC' order by so_hs)
    ...
    select count(*) into b_dong from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC';
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC' order by so_hd)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_ptn where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv<>'TNVC' order by so_hd)
        where sott between b_tu and b_den;
end if;
...
/
create or replace procedure PBH_BT_PTNVC_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_phong varchar2(10); b_so_hs varchar2(30);
    b_so_id number; b_ngay_ht number; b_klk varchar2(1); b_trangKt number; b_tu number; b_den number;
...
/
create or replace procedure PBH_BT_PTN_CT(
     b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
EXECUTE IMMEDIATE b_lenh into b_ma_dvi,b_so_id using b_oraIn;
b_loi:='loi:Ho so da xoa:loi';
select ma_dvi_ql,so_id_hd,ngay_xr into b_ma_dvi_ql,b_so_id_hd,b_ngay_xr from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_so_id_hdB:=FBH_PTN_SO_IDbt(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select lt,kbt into dt_lt,dt_kbt from bh_ptn_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id_hdB;
select json_object(so_hs,tien,tienHK,'ma_nn' value FBH_PTN_NNTT_TENl(ma_nn)) into dt_ct from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ma) order by bt) into dt_dk from bh_bt_ptn_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ma,ten,tien) order by bt) into dt_hk
    from bh_bt_ptn_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ten,ma_nt,tien) order by bt) into dt_tba from bh_bt_ptn_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
select count(*) into b_i1 from bh_bt_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_bt_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
select JSON_ARRAYAGG(json_object(loai,txt) returning clob) into dt_txt 
    from bh_bt_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai not in('dt_tba','dt_kbt');
...
/
create or replace procedure PBH_BT_PTN_GCNh(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
...
select max(ma_dvi),nvl(max(so_id),0) into b_ma_dvi,b_so_id from bh_bt_ptn where so_hd=b_so_hd and ngay_xr=b_ngay_xr;
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PTN_GCNg(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(2000); b_lenh varchar2(2000); b_i1 number;
    b_so_id_dt number; b_ngay_xr number; b_nt_tien varchar2(5);
...
b_loi:='loi:Nhap hop dong va ngay xay ra:loi';
FBH_PTN_SO_ID_DTf(b_so_id_dt,b_ma_dvi,b_so_idG,b_ngay_xr);
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
select nt_tien into b_nt_tien from bh_ptn where ma_dvi=b_ma_dvi and so_id=b_so_idG;
if b_nt_tien<>'VND' then b_tg:=FBH_TT_TRA_TGTT(b_ngay_xr,b_nt_tien); end if;   
select json_object(a.so_hd,b.gcn,a.ten,'ma_dvi_ql' value a.ma_dvi,b.dtuong,b.ngay_hl,b.ngay_kt,a.nt_tien,a.c_thue,
    'lhe_ten' value a.ten,'lhe_mobi' value a.mobi,'lhe_mail' value a.email,'tygia' value b_tg) into dt_ct
    from bh_ptn a,bh_ptn_dvi b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_idG and
    b.ma_dvi=b_ma_dvi and b.so_id=b_so_idG and b.so_id_dt=b_so_id_dt; 
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    ...
    order by bt returning clob) into dt_dk from bh_ptn_dk where
    ma_dvi=b_ma_dviN and so_id=b_so_idG and so_id_dt=b_so_id_dt and lh_bh<>'M';
select lt,kbt into dt_lt,dt_kbt from bh_ptn_kbt where ma_dvi=b_ma_dvi and so_id=b_so_idG and so_id_dt=b_so_id_dt;
dt_btlke:=FBH_BT_PTN_BTH_LKE(b_ma_dvi,b_so_idG,b_so_id_dt);
select count(*) into b_i1 from bh_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_idG and loai='dt_hk';
if b_i1=1 then
    select txt into dt_hk from bh_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_idG and loai='dt_hk';
end if;
...
/
create or replace procedure PBH_BT_PTN_GCNgdk(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(2000); b_lenh varchar2(2000);
    b_ngay_xr number; b_ma_dvi varchar2(10); 
    b_so_id number; b_so_idG number; b_so_id_dt number;
begin
-- Dan - Liet ke theo ngay
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
FBH_PTN_SO_ID_DTf(b_so_id_dt,b_ma_dvi,b_so_idG);
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
b_so_id:=FBH_PTN_SO_IDbt(b_ma_dvi,b_so_idG,b_ngay_xr);
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    'pt_bt' value 0,'ycau' value 0,'t_that' value 0,'tien' value 0,'nd' value '','ktru' value 0,
    ...
    order by bt returning clob) into b_oraOut from bh_ptn_dk where
    ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PTN_TESTr(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id_dt in out number,dt_ct clob,dt_dk clob,dt_hk clob,dt_tba clob,dt_kbt out clob,

    b_gcn out varchar2,b_ma_dvi_ql out varchar2,b_so_hd out varchar2,b_so_id_hd out number,
    b_ma_kh out varchar2,b_ten out nvarchar2,b_ma_nn out varchar2,b_tienHK out number,b_nv out varchar2,
...
AS
    ...
    b_ttrang varchar2(1); b_so_id_hdB number; b_ngay_xr number; b_ma_sp varchar2(10); b_ma_dviN varchar2(10);
    b_nt_tien varchar2(5); b_c_thue varchar2(1); b_tienH number; b_thueH number;
    b_tien number; b_thue number; dk_bt_con pht_type.a_num;
begin
...
FBH_PTN_SO_ID_DTf(b_so_id_dt,b_ma_dviN,b_so_id_hd);
if b_so_id_hd=0 then b_loi:='loi:Hop dong da xoa:loi'; return; end if;
b_so_id_hdB:=FBH_PTN_SO_IDbt(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select count(*) into b_i1 from bh_ptn where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and nt_tien=b_nt_tien and c_thue=b_c_thue;
...
select min(gcn) into b_gcn from bh_ptn_dvi where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt and b_ngay_xr between ngay_hoi and ngay_kt;
if b_gcn is null then b_loi:='loi:Ngay xay ra ngoai hieu luc bao hiem:loi'; return; end if;
select ma_kh,ten,nv,ma_sp into b_ma_kh,b_ten,b_nv,b_ma_sp from bh_ptn where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB;
...
if b_ttrang in('T','D') then
    ...
    if FBH_PTN_HLd(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ngay_xr)='K' then
    ...
    select kbt into dt_kbt from bh_ptn_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
    dt_kbt:=FKH_JS_BONH(dt_kbt);
    PBH_BT_PTN_KBT(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,dt_ct,dt_dk,dt_kbt,b_loi);
...
/
create or replace procedure PBH_BT_PTN_XOA_XOA
    (b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_nh boolean,b_loi out varchar2)
AS
    b_i1 number; r_hs bh_bt_ptn%rowtype;
Begin
-- Dan - Xoa boi thuong
select count(*) into b_i1 from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1=0 then b_loi:=''; return; end if;
b_loi:='loi:Loi xoa ho so:loi';
select * into r_hs from bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
if trim(r_hs.nsd) is not null and b_nsd<>r_hs.nsd then b_loi:='loi:Khong sua, xoa so lieu nguoi khac:loi'; return; end if;
if r_hs.ttrang in('G','T','D') then
    PBH_BT_GOC_XOA(b_ma_dvi,b_nsd,b_so_id,b_nh,b_loi);
    if b_loi is not null then return; end if;
end if;
delete bh_bt_ptn_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_ptn_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_ptn_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_ptn_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_ptn where ma_dvi=b_ma_dvi and so_id=b_so_id;
--nam: xoa du phong
delete bh_bt_ptn_duph where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_PTN_NH_NH(
...
-- Test rieng
    b_gcn varchar2,b_ma_dvi_ql varchar2,b_so_hd varchar2,b_so_id_hd number,
    b_ma_kh varchar2,b_ten nvarchar2,b_ma_nn varchar2,b_tienHK number,b_nv varchar2,
...
b_loi:='loi:Loi Table bh_bt_ptn:loi';
insert into bh_bt_ptn values(b_ma_dvi,b_so_id,b_ngay_ht,b_so_hs,b_ttrang,b_kieu_hs,b_so_hs_g,b_nv,b_gcn,
...
b_loi:='loi:Loi Table bh_bt_ptn_dk:loi';
for b_lp in 1..dk_ma.count loop
    insert into bh_bt_ptn_dk values(b_ma_dvi,b_so_id,b_lp,
...
if hk_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_ptn_HK:loi';
    for b_lp in 1..hk_ma_nt.count loop
        insert into bh_bt_ptn_hk values(
...
if tba_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_ptn_tba:loi';
    for b_lp in 1..tba_ma_nt.count loop
        insert into bh_bt_ptn_tba values(b_ma_dvi,b_so_id,b_lp,tba_ten(b_lp),tba_ma_nt(b_lp),tba_tien(b_lp));
    end loop;
end if;
insert into bh_bt_ptn_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_bt_ptn_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if dt_hk is not null then
    insert into bh_bt_ptn_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if dt_tba is not null then
    insert into bh_bt_ptn_txt values(b_ma_dvi,b_so_id,'dt_tba',dt_tba);
end if;
if dt_kbt is not null then
    insert into bh_bt_ptn_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
if dt_ttt is not null then
    insert into bh_bt_ptn_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
PBH_BT_HSBS_NH(b_ma_dvi,b_so_id,b_so_hs,b_ma_dvi_ql,b_so_hd,b_tien+b_thue,
    dt_ct,dt_dk,dt_hk,dt_tba,'',dt_kbt,b_loi);
if b_loi is not null then return; end if;
PBH_BT_PTN_GOC(b_ma_dvi,b_nsd,b_so_id,b_duph,b_loi);
if b_loi is not null then return; end if;
if b_ttrang='T' and b_duph='C' then
    b_i1:=PKH_NG_CSO(sysdate);
    insert into bh_bt_ptn_duph select a.*,b_i1 from bh_bt_ptn_txt a where ma_dvi=b_ma_dvi and so_id=b_so_id;
end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_PTN_NH(
...
-- Test rieng
    b_gcn varchar2(20); b_so_hd varchar2(20); b_ma_dvi_ql varchar2(10); b_so_id_hd number; b_so_id_dt number; 
    b_ma_kh varchar2(20);b_ten nvarchar2(500);b_ma_nn varchar2(200);b_tienHK number; b_nv varchar2(10);
...
if b_so_id<>0 then
    select count(*) into b_i1 from bh_bt_ptn where so_id=b_so_id;
...
if b_so_id=0 then
    ...
    select ma_dvi,ngay_ht into b_ma_dviC,b_ngay_htC from bh_bt_ptn where so_id=b_so_id for update nowait;
    if sql%rowcount=0 then raise PROGRAM_ERROR; end if;
    PBH_BT_PTN_XOA_XOA(b_ma_dviC,b_nsd,b_so_id,true,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
end if;
...
PBH_BT_PTN_TESTr(
    ...
    b_gcn,b_ma_dvi_ql,b_so_hd,b_so_id_hd,b_ma_kh,b_ten,b_ma_nn,b_tienHK,b_nv,
...
/
create or replace procedure PBH_BT_PTN_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_comm varchar2:='C')
AS
...
PBH_BT_PTN_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,false,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_PTN_GOC(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_duph varchar2,b_loi out varchar2)
AS
    b_so_id_hd number; b_so_idB number;
    b_so_id_dt number; b_ma_nt varchar2(5); b_ma_dt varchar2(100);
...
b_so_idB:=FBH_PTN_SO_IDbt(b_ma_dvi_ql,b_so_id_hd);
--select dtuong into b_ma_dt from bh_ptn_dvi where ma_dvi=b_ma_dvi_ql and so_id=b_so_idB and so_id_dt=b_so_id_dt;
insert into bh_bt_goc_temp1
    select lh_nv,sum(tien_bh),max(pt_bt),sum(t_that),sum(tien),sum(tien_qd),sum(thue),sum(thue_qd)
    from bh_bt_ptn_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_nv<>' ' group by lh_nv;
select 'PTN',ngay_ht,so_hs,ttrang,kieu_hs,so_hs_g,ma_dvi_ql,so_hd,ma_kh,ten,' ',' ',
    ngay_gui,ngay_xr,ngay_do,n_trinh,n_duyet,ngay_qd,nop,bphi,dung,traN,'bh_bt_ptn'
...
select ten,ma_nt,tien bulk collect
    ...
    from bh_bt_ptn_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_goc_temp1;
...
/
create or replace procedure PBH_BT_PTN_HSBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_hs_g <>' ' then
  ...
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_ptn where ma_dvi=b_ma_dvi and so_hs=b_so_hs_g union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_ptn where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs_g);
else 
  b_so_hs:=FBH_BT_HS_SOHS(b_ma_dvi,b_so_id);
  select JSON_ARRAYAGG(json_object(so_hs,'ma_dvi' value ma_dvi_ql,so_hd,tien,'ngay' value ngay_ht,so_id,'csot' value '') order by so_id desc returning clob) into cs_lke from 
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_ptn where ma_dvi=b_ma_dvi and so_hs=b_so_hs union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_ptn where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs);
end if;
...
/
create or replace procedure PBH_BT_PTN_TIMh(
   b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
for r_lp in (
  select distinct ma_dvi,so_id_d
    from bh_ptn where
        (b_ma_kh is null or b_ma_kh is null OR ma_kh = b_ma_kh) and b_ttrang in (' ',ttrang) and nv<>'TNH'
        and (upper(b_ten) = ' ' OR upper(ten) LIKE '%' || b_ten || '%')
        and (b_so_hd = ' ' OR so_hd LIKE '%' || b_so_hd || '%')
        and ngay_ht between b_ngayd and b_ngayc and ngay_kt > b_ngay and nv<>'TNVC'
    ) loop
    ...
    if FBH_PTN_TTRANG(b_ma_dviD,b_so_idC)='D' then
        ...
        insert into ket_qua(c1,c2,n1,n2,n3,n10,n11,c10)
            select so_hd,ten,ngay_hl,ngay_kt,ngay_cap,b_so_idD,0,b_nv from bh_ptn where ma_dvi=b_ma_dviD and so_id=b_so_idC;
        insert into ket_qua(c1,c2,n1,n2,n3,n10,n11,c10)
            select ' ','- '||dtuong,ngay_hl,ngay_kt,ngay_cap,b_so_idD,so_id_dt,'' from bh_ptn_dvi where ma_dvi=b_ma_dviD and so_id=b_so_idC order by bt;
    end if;
end loop;
...
/
create or replace procedure PBH_BT_PTNVC_TIMh(
   b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    ...
    if FBH_PTN_TTRANG(b_ma_dviD,b_so_idC)='D' then
        ...
        insert into ket_qua(c1,c2,n1,n2,n3,n10,n11,c10,c11)
            select so_hd,ten,ngay_hl,ngay_kt,ngay_cap,b_so_idD,0,nv,cmt
             from bh_ptn where ma_dvi=b_ma_dviD and so_id=b_so_idC;
    end if;
end loop;
select count(*) into b_dong from ket_qua where rownum<202;
select JSON_ARRAYAGG(json_object('so_hd' value c1,'ten' value c2,'ngay_hl' value n1,'ngay_kt' value n2,'ngay_cap' value n3,
...
/
create or replace procedure PBH_BT_PTNVC_TIM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select count(*) into b_dong
   from bh_bt_ptn where (b_ma_kh is null or b_ma_kh = ' ' or ma_kh = b_ma_kh)
        ...
        and ngay_ht between b_ngayd and b_ngayc and ngay_ht > b_ngay and nv='TNVC' and rownum<202;
...
/
create or replace procedure PBH_BT_PTN_TIM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(2000); b_ngay number:=PKH_NG_CSO(add_months(sysdate,-1));
    cs_ct clob:=''; cs_lke clob:='';
    b_ttrang  varchar2(1);
    b_ma_kh varchar2(20); b_cmt varchar2(20); b_mobi varchar2(20); b_email varchar2(100);
    b_ngayD number; b_ngayC number; b_so_hsT varchar2(20); b_so_hd varchar2(20);
...
b_ma_kh:=FBH_DTAC_MAf('','',b_cmt,b_mobi,b_email,'','');
select count(*) into b_dong
   from bh_bt_ptn where ma_dvi = b_ma_dvi and (b_ma_kh is null or b_ma_kh is null or ma_kh = b_ma_kh)
        and b_ttrang in (' ', ttrang) and (b_so_hsT = ' ' OR so_hs LIKE '%' || b_so_hsT || '%')
        and (b_so_hd = ' ' OR so_hd LIKE '%' || b_so_hd || '%') and (b_ten = ' ' OR upper(ten) LIKE '%' || b_ten || '%')
        and ngay_ht between b_ngayd and b_ngayc and ngay_ht > b_ngay and rownum<202 and nv<>'TNVC';
select JSON_ARRAYAGG(json_object(so_id,so_hs,'ngay' value ngay_xr,ttrang,so_hd,ten,ma_dvi returning clob) order by so_id desc returning clob)
    into cs_lke from (
        select a.*, rownum as sott
           from bh_bt_ptn a where ma_dvi = b_ma_dvi and (b_ma_kh is null or b_ma_kh is null or ma_kh = b_ma_kh)
              and b_ttrang in (' ', ttrang) and (b_so_hsT = ' ' OR so_hs LIKE '%' || b_so_hsT || '%')
              and (b_so_hd = ' ' OR so_hd LIKE '%' || b_so_hd || '%') and (b_ten = ' ' OR upper(ten) LIKE '%' || b_ten || '%')
              and ngay_ht between b_ngayd and b_ngayc and ngay_ht > b_ngay and rownum<202 and nv<>'TNVC'
    ) where sott between b_tu and b_den;
