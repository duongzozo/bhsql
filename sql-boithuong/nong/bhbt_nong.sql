create or replace function FBH_BT_NONG_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return varchar2
AS
    b_kq varchar2(100):=' '; b_i1 number; b_txt clob;
begin
-- Nam - Tra gia tri varchar2 trong txt
select count(*) into b_i1 from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
if b_i1=1 then
    select txt into b_txt from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
    b_kq:=FKH_JS_GTRIs(b_txt,b_tim);
end if;
return b_kq;
end;
/
create or replace function FBH_BT_NONG_TTRANG(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Nam - Tra tinh trang
select nvl(min(ttrang),'X') into b_kq from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_NONG_NV(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Nam - Tra nghiep vu
select nvl(min(ttrang),'G') into b_kq from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_BT_NONG_BTH_LKE(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return clob
as
    dt_bth clob;
begin
-- Nam - Luy ke boi thuong GCN
select JSON_ARRAYAGG(json_object(ma,bt_lke) returning clob) into dt_bth from
    (select ma,sum(tien) bt_lke from bh_bt_nong_dk where (ma_dvi,so_id) in
    (select ma_dvi,so_id from bh_bt_nong where ma_dvi_ql=b_ma_dvi and so_id_hd=b_so_id and
    so_id_dt=b_so_id_dt and ttrang='D') group by ma);
return dt_bth;
end;
/
create or replace procedure PBH_BT_NONG_MO
...
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_ttt
    from bh_kh_ttt where ps='BT' and nv='NONG';
select json_object('cs_kbt' value cs_kbt,'cs_ttt' value cs_ttt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace function FBH_BT_NONG_SO_HS(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(30);
begin
-- Nam - Tra so_id
select min(so_hs) into b_kq from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace procedure FBH_BT_NONG_SO_ID(b_so_hs varchar2,b_ma_dvi out varchar2,b_so_id out number)
AS
begin
-- Nam - Tra ma_dvi, so_id
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_bt_nong where so_hs=b_so_hs;
end;
/
create or replace procedure PBH_BT_NONG_SO_ID(
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','BT','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_BT_NONG_SO_ID(b_so_hs,b_ma_dvi,b_so_id);
...
/
create or replace procedure PBH_BT_NONG_FBPHI(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select ma_dvi_ql,so_id_hd,nv,so_id_dt into b_ma_dvi_hd,b_so_id_hd,b_nv,b_so_id_dt from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
select nvl(min(so_idp),0) into b_so_idP from bh_nong_dvi where ma_dvi=b_ma_dvi_hd and so_id=b_so_id_hd and so_id_dt=b_so_id_dt;
select json_object('so_idP' value b_so_idP,'nv' value b_nv) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_NONG_FHD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_ma_dvi varchar2(10); b_so_id number; b_nv varchar2(10);
...
EXECUTE IMMEDIATE b_lenh into b_ma_dvi,b_so_id using b_oraIn;
b_ma_dvi:=nvl(trim(b_ma_dvi),' '); b_so_id:=nvl(b_so_id,0);
select ma_dvi_ql,so_id_hd,so_id_dt,nv into b_ma_dvi_hd,b_so_id_hd,b_so_id_dt,b_nv from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_BT_NONG_LKE_GCN(
...
b_so_id_hd:=FBH_NONG_HD_SO_IDd(b_ma_dvi_ql,b_so_hd);
...
select JSON_ARRAYAGG(json_object(so_hs,ngay_mo,ttrang,tien,ma_dvi,so_id) order by ngay_mo desc)
    into cs_bth from bh_bt_nong where so_id_dt=b_so_id_dt;
select json_object('cs_phi' value cs_phi,'cs_bth' value cs_bth) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_NONG_GCN(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(2000); b_lenh varchar2(2000);
    b_so_id_dt number; b_ngay_xr number; b_nt_tien varchar2(5);
    b_ma_dvi varchar2(10); b_so_idG number; b_so_id number; b_tg number:=1; b_i1 number;
    dt_ct clob; dt_hk clob; dt_kbt clob; dt_btlke clob; dt_dk clob; dt_lt clob; dt_pvi clob;
begin
...
FBH_NONG_SO_ID_DTf(b_so_id_dt,b_ma_dvi,b_so_idG);
if b_so_idG=0 then raise PROGRAM_ERROR; end if;
b_so_id:=FBH_NONG_SO_IDb(b_ma_dvi,b_so_idG,b_ngay_xr);
select nt_tien into b_nt_tien from bh_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_nt_tien<>'VND' then b_tg:=FBH_TT_TRA_TGTT(b_ngay_xr,b_nt_tien); end if;
select json_object(a.so_hd,a.ten,'ma_dvi_ql' value a.ma_dvi,b.dvi,b.ngay_hl,b.ngay_kt,a.nt_tien,a.c_thue,
    'lhe_ten' value a.ten,'lhe_mobi' value a.mobi,'lhe_mail' value a.email,'tygia' value b_tg) into dt_ct
    from bh_nong a,bh_nong_dvi b where a.ma_dvi=b.ma_dvi and a.so_id=b.so_id and
    a.ma_dvi=b_ma_dvi and a.so_id=b_so_id and b.so_id_dt=b_so_id_dt;
select JSON_ARRAYAGG(json_object('chon' value '',MA,ten,'tien_bh' value tien,
    ...
    cap,tc,ma_ct,ma_dk,ma_dkC,kieu,lh_nv,t_suat,ptB,lkeP,lkeB,luy,nv,'ktruk' value ktru,pvi_ktru)
    order by bt returning clob) into dt_dk from bh_nong_dk where
    ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt and pvi_ma=' ' and nv='C';
select lt,kbt into dt_lt,dt_kbt from bh_nong_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
dt_btlke:=FBH_BT_NONG_BTH_LKE(b_ma_dvi,b_so_id,b_so_id_dt);
select JSON_ARRAYAGG(json_object('ten' value FBH_NONG_PVI_TEN(ma,nv),'chon' value '',ma,ktru) order by ma returning clob) into dt_pvi from
    (select distinct b.pvi_ma ma,b.pvi_ktru ktru,a.nv nv from bh_nong a, bh_nong_dk b where a.ma_dvi=b_ma_dvi and 
            a.so_id=b_so_id and b.so_id_dt=b_so_id_dt and a.ma_dvi=b.ma_dvi and a.so_id=b.so_id and b.pvi_ma<>' ');
-- an - lay dt_hk theo tung nv cua nong
select count(*) into b_i1 from bh_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hk';
if b_i1=1 then
    select txt into dt_hk from bh_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hk';
end if;
select json_object('so_id_dt' value b_so_id_dt,'dt_ct' value dt_ct,
    'dt_lt' value dt_lt,'dt_kbt' value dt_kbt,
    'dt_btlke' value dt_btlke,'dt_dk' value dt_dk,'dt_pvi' value dt_pvi,'dt_hk' value dt_hk returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_NONG_GCNd(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_ma_dvi varchar2(10); b_so_id number; b_so_id_dt number; 
begin
-- Nam - Tra so_id_dt qua GCN
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','NONG','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
FBH_NONG_SO_ID_GCN(b_oraIn,b_ma_dvi,b_so_id,b_so_id_dt);
select json_object('so_id_dt' value b_so_id_dt) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_NONG_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_nong where
    ...
    select count(*) into b_dong from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_nong  where
    ...
    select count(*) into b_dong from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_nong  where
...
/
create or replace procedure PBH_BT_NONG_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hs:=FBH_BT_NONG_SO_HS(b_ma_dvi,b_so_id);
if b_so_hs is null then b_loi:='loi:Ho so da xoa:loi'; raise PROGRAM_ERROR; end if;
if b_klk ='N' then
    select count(*) into b_dong from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hs)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs)
        where sott between b_tu and b_den;
elsif b_klk='P' or FHT_MA_NSD_QU(b_ma_dvi,b_nsd,'BH','BT','X')<>'C' then
    b_phong:=FHT_MA_NSD_PHONG(b_ma_dvi,b_nsd);
    select count(*) into b_dong from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and phong=b_phong order by so_hs)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hs)
    ...
    select count(*) into b_dong from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hs,rownum sott from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hd)
        where so_hs>=b_so_hs;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hs,nsd) obj,rownum sott from bh_bt_nong where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht order by so_hd)
        where sott between b_tu and b_den;
end if;
...
/
CREATE OR REPLACE procedure PBH_BT_NONG_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    ...
    dt_thoi clob:=''; dt_ttt clob:=''; dt_pvi clob:=''; dt_txt clob;
begin
-- Nam - Xem
b_loi:=FHT_MA_NSD_KTRA(b_ma_dviN,b_nsd,b_pas,'BH','NONG','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
EXECUTE IMMEDIATE b_lenh into b_ma_dvi,b_so_id using b_oraIn;
b_loi:='loi:Ho so da xoa:loi';
select ma_dvi_ql,so_id_hd,so_id_dt,ngay_xr into b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ngay_xr from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_so_id_hdB:=Fbh_nong_SO_IDb(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select lt,kbt into dt_lt,dt_kbt from bh_nong_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
select json_object(so_hs,tien,tienHK,gcn,'ma_nn' value FBH_NONG_NNTT_TENl(ma_nn) returning clob) into dt_ct from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ma) order by bt) into dt_dk from bh_bt_nong_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ma,ten,ma_nt,tien,thue,ttoan) order by bt) into dt_hk
    from bh_bt_nong_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
select JSON_ARRAYAGG(json_object(ten,ma_nt,tien) order by bt returning clob) into dt_tba from bh_bt_nong_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai not in('dt_tba','dt_kbt','dt_pvi');
select count(*) into b_i1 from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
select count(*) into b_i1 from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_pvi';
if b_i1<>0 then
    select txt into dt_pvi from bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_pvi';
end if;
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,'so_id_dt' value b_so_id_dt,
    'dt_ttt' value dt_ttt,'dt_pvi' value dt_pvi,'dt_lt' value dt_lt,'dt_kbt' value dt_kbt,'dt_thoi' value dt_thoi,'dt_ct' value dt_ct,'dt_dk' value dt_dk,
    'dt_hk' value dt_hk,'dt_tba' value dt_tba,'txt' value dt_txt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_NONG_TESTr(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id_dt number,
    dt_ct clob,dt_dk clob,dt_hk clob,dt_tba clob,dt_kbt out clob,

    b_nv out varchar2,b_gcn out varchar2,b_ma_dvi_ql out varchar2,b_so_hd out varchar2,b_so_id_hd out number,
...
AS
    ...
    b_tienH number; b_thueH number; b_tien number; b_thue number; b_ma_sp varchar2(10);
    dk_bt_con pht_type.a_num;
begin
-- Nam kiem tra thong tin nhap ho so boi thuong
...
if b_ma_nn is not null then
  if FBH_NONG_NNTT_HAN(b_ma_nn)<>'C' then b_loi:='loi:Ma nguyen nhan ton that'||b_ma_nn||' da het han:loi'; return; end if;
end if;
if b_ma_dvi_ql is null or b_so_hd is null then b_loi:='loi:Chon hop dong bao hiem:loi'; return; end if;
b_so_id_hd:=FBH_NONG_SO_ID(b_ma_dvi_ql,b_so_hd);
...
b_so_id_hdB:=Fbh_nong_SO_IDb(b_ma_dvi_ql,b_so_id_hd,b_ngay_xr);
select count(*) into b_i1 from bh_nong where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and nt_tien=b_nt_tien and c_thue=b_c_thue;
...
select min(gcn) into b_gcn from bh_nong_dvi where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and
...
select so_hd,ma_kh,ten,ma_sp,nv into b_so_hd,b_ma_kh,b_ten,b_ma_sp,b_nv from bh_nong where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB;
...
if b_tienH<>b_tien then
    b_loi:='loi:Chenh tong tien chi tiet va ho so:loi'; return;
end if;
b_tienHK:=0;
...
if b_ttrang in('T','D') then
    ...
    if FBH_NONG_HLd(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,b_ngay_xr)='K' then
    ...
    select kbt into dt_kbt from bh_nong_kbt where ma_dvi=b_ma_dvi_ql and so_id=b_so_id_hdB and so_id_dt=b_so_id_dt;
    dt_kbt:=FKH_JS_BONH(dt_kbt);
    PBH_BT_NONG_KBT(b_ma_dvi_ql,b_so_id_hd,b_so_id_dt,dt_ct,dt_dk,dt_kbt,b_loi);
...
/
create or replace procedure PBH_BT_NONG_XOA_XOA
    (b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_nh boolean,b_loi out varchar2)
AS 
    b_i1 number; r_hs bh_bt_nong%rowtype;
Begin
-- Nam - Xoa boi thuong
select count(*) into b_i1 from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1=0 then b_loi:=''; return; end if;
b_loi:='loi:Loi xoa ho so:loi';
select * into r_hs from bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
if b_loi is not null then return; end if;
delete bh_bt_nong_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_nong_hk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_nong_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_nong_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_nong where ma_dvi=b_ma_dvi and so_id=b_so_id;
--nam: xoa du phong
delete bh_bt_nong_duph where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_NONG_NH_NH(
...
-- Test rieng
    b_nv varchar2,b_gcn varchar2,b_ma_dvi_ql varchar2,b_so_hd varchar2,b_so_id_hd number,b_so_id_dt number,
...
b_loi:='loi:Loi Table bh_bt_nong:loi';
insert into bh_bt_nong values(b_ma_dvi,b_so_id,b_ngay_ht,b_so_hs,b_ttrang,b_kieu_hs,b_so_hs_g,b_nv,
...
b_loi:='loi:Loi Table bh_bt_nong_dk:loi';
for b_lp in 1..dk_ma.count loop
    insert into bh_bt_nong_dk values(b_ma_dvi,b_so_id,b_lp,
...
if hk_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_nong_HK:loi';
    for b_lp in 1..hk_ma_nt.count loop
        insert into bh_bt_nong_hk values(
...
if tba_ma_nt.count<>0 then
    b_loi:='loi:Loi Table bh_bt_nong_tba:loi';
    for b_lp in 1..tba_ma_nt.count loop
        insert into bh_bt_nong_tba values(b_ma_dvi,b_so_id,b_lp,tba_ten(b_lp),tba_ma_nt(b_lp),tba_tien(b_lp));
    end loop;
end if;
insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if dt_hk is not null then
    insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if dt_tba is not null then
    insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_tba',dt_tba);
end if;
if dt_kbt is not null then
    insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
if dt_ttt is not null then
    insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if dt_pvi is not null then
    insert into bh_bt_nong_txt values(b_ma_dvi,b_so_id,'dt_pvi',dt_pvi);
end if;
b_loi:='loi:day:loi';
PBH_BT_HSBS_NH(b_ma_dvi,b_so_id,b_so_hs,b_ma_dvi_ql,b_so_hd,b_tien+b_thue,
    dt_ct,dt_dk,dt_hk,dt_tba,'',dt_kbt,b_loi);
if b_loi is not null then return; end if;
PBH_BT_NONG_GOC(b_ma_dvi,b_nsd,b_so_id,b_duph,b_loi);
if b_loi is not null then return; end if;
if b_ttrang='T' and b_duph='C' then
    b_i1:=PKH_NG_CSO(sysdate);
    insert into bh_bt_nong_duph select a.*,b_i1 from bh_bt_nong_txt a where ma_dvi=b_ma_dvi and so_id=b_so_id;
end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_BT_NONG_NH(
...
-- Test rieng
    ...
    b_ma_kh varchar2(20); b_ten nvarchar2(500); b_tienHK number; b_ma_nn varchar2(500); b_nv varchar2(10);
    dk_ma pht_type.a_var; dk_ten pht_type.a_nvar; dk_tc pht_type.a_var; dk_ma_ct pht_type.a_var;
    dk_tien_bh pht_type.a_num; dk_pt_bt pht_type.a_num; dk_t_that pht_type.a_num;
    dk_tien pht_type.a_num; dk_thue pht_type.a_num; dk_tien_qd pht_type.a_num; dk_thue_qd pht_type.a_num;
    dk_cap pht_type.a_num; dk_ma_dk pht_type.a_var; dk_ma_dkC pht_type.a_var;
...
FKH_JS_NULL(dt_ct); FKH_JSa_NULL(dt_dk); FKH_JSa_NULL(dt_hk); FKH_JSa_NULL(dt_tba);
FKH_JSa_NULL(dt_ttt); FKH_JSa_NULL(dt_pvi);
if b_so_id<>0 then
    select count(*) into b_i1 from bh_bt_nong where so_id=b_so_id;
...
if b_so_id=0 then
    ...
    select ma_dvi,ngay_ht into b_ma_dviC,b_ngay_htC from bh_bt_nong where so_id=b_so_id for update nowait;
    if sql%rowcount=0 then raise PROGRAM_ERROR; end if;
    PBH_BT_NONG_XOA_XOA(b_ma_dviC,b_nsd,b_so_id,true,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
end if;
...
PBH_BT_NONG_TESTr(
    ...
    b_nv,b_gcn,b_ma_dvi_ql,b_so_hd,b_so_id_hd,b_ma_kh,b_ten,b_tienHK,b_ma_nn,
...
-- Test rieng
    b_nv,b_gcn,b_ma_dvi_ql,b_so_hd,b_so_id_hd,b_so_id_dt,b_ma_kh,b_ten,b_tienHK,b_ma_nn,
...
/
create or replace procedure PBH_BT_NONG_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_comm varchar2:='C')
AS
...
PBH_BT_NONG_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,false,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_BT_NONG_GOC(
...
b_so_idB:=FBH_NONG_SO_IDb(b_ma_dvi_ql,b_so_id_hd);
select loai into b_ma_dt from bh_nong_dvi where ma_dvi=b_ma_dvi_ql and so_id=b_so_idB and so_id_dt=b_so_id_dt;
insert into bh_bt_goc_temp1
    select lh_nv,sum(tien_bh),max(pt_bt),sum(t_that),sum(tien),sum(tien_qd),sum(thue),sum(thue_qd)
    from bh_bt_nong_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_nv<>' ' group by lh_nv;
select 'NONG',ngay_ht,so_hs,ttrang,kieu_hs,so_hs_g,ma_dvi_ql,so_hd,ma_kh,ten,' ',' ',
    ngay_gui,ngay_xr,ngay_do,n_trinh,n_duyet,ngay_qd,nop,bphi,dung,traN,'bh_bt_nong'
...
select ten,ma_nt,tien bulk collect into tba_ten,tba_ma_nt,tba_tien
    from bh_bt_nong_tba where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_bt_goc_temp1;
...
/
create or replace procedure PBH_BT_NONG_HSBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_hs_g <>' ' then
  ...
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_nong where ma_dvi=b_ma_dvi and so_hs=b_so_hs_g union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_nong where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs_g);
else 
  b_so_hs:=FBH_BT_HS_SOHS(b_ma_dvi,b_so_id);
  select JSON_ARRAYAGG(json_object(so_hs,'ma_dvi' value ma_dvi_ql,so_hd,tien,'ngay' value ngay_ht,so_id,'csot' value '') order by so_id desc returning clob) into cs_lke from 
  (select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_nong where ma_dvi=b_ma_dvi and so_hs=b_so_hs union
  select so_hs,ma_dvi_ql,so_hd,tien,ngay_ht,so_id from bh_bt_nong where ma_dvi=b_ma_dvi and so_hs_g=b_so_hs);
end if;
...
/
create or replace procedure PBH_BT_NONG_TIM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
select JSON_ARRAYAGG(json_object(so_hs,'ngay' value ngay_xr,ttrang,so_hd,ten,
    'dvi' value FBH_NONG_DVI(ma_dvi_ql,so_id_hd,so_id_dt,ngay_xr),ma_dvi,so_id) order by so_id desc returning clob)
    into cs_lke from (
...
/
create or replace procedure PBH_BT_NONG_TIMh(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(2000); b_ngay number:=PKH_NG_CSO(add_months(sysdate,-36)); cs_lke clob:='';
    b_so_idC number; b_so_idD number; b_ttrang  varchar2(1); b_nv varchar2(10);
...
for r_lp in (select distinct a.ma_dvi,a.so_id_d
    from bh_nong a,bh_nong_dvi b where
    ...
    if FBH_NONG_TTRANG(b_ma_dviD,b_so_idD)='D' then
        b_so_idC:=FBH_NONG_SO_IDc(b_ma_dviD,b_so_idD);
        b_so_hd:=FBH_NONG_SO_HDd(b_ma_dviD,b_so_idD); b_nv:=FBH_NONG_NV(b_ma_dviD,b_so_idD);
        insert into ket_qua(c1,c2,n1,n2,n3,n10,n11,c10)
            select b_so_hd,ten,ngay_hl,ngay_kt,ngay_cap,b_so_idD,0,b_nv from bh_nong where ma_dvi=b_ma_dviD and so_id=b_so_idC;
        insert into ket_qua(c1,c2,n1,n2,n3,n10,n11,c10)
            select ' ','- '||dvi,ngay_hl,ngay_kt,ngay_cap,b_so_idD,so_id_dt,'' from bh_nong_dvi where ma_dvi=b_ma_dviD and so_id=b_so_idC order by bt;
    end if;
end loop;
