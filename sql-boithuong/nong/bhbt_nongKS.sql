create or replace procedure PBH_BT_NONG_KBT(
...
begin
-- Nam - Kiem soat dieu kien rieng
b_lenh:=FKH_JS_LENH('ma,lh_nv,tien_bh,t_that,tien,luy');
EXECUTE IMMEDIATE b_lenh bulk collect into dk_ma,dk_lh_nv,dk_tien_bh,dk_t_that,dk_bth,dk_luy using dt_dk;
b_lenh:=FKH_JS_LENH('ma,kbt');
EXECUTE IMMEDIATE b_lenh bulk collect into kbt_ma,kbt_kbt using dt_kbt;
select ma,sum(tien) bt_lke bulk collect into dkL_ma,dkL_tien from bh_bt_nong_dk
    where (ma_dvi,so_id) in (select ma_dvi,so_id from bh_bt_nong where so_id_dt=b_so_id_dt and ttrang='D') group by ma;
for b_lp in 1..dk_ma.count loop
    dk_lke(b_lp):=0;
    b_v:=FKH_ARR_VTRI(dkL_ma,dk_ma(b_lp));
    if b_v<>0 then dk_lke(b_lp):=dkL_tien(b_v); end if;
end loop;
FBH_NONG_DT_NG(b_ma_dvi,b_so_id,b_so_id_dt,b_ngay_hl,b_ngay_kt);
        ...
        if b_c is not null then
            ...
            for b_lp1 in 1..kma_ma.count loop
                if kma_ma(b_lp1) in('MVU','GVU','KVU') then
                    b_loi:=FBH_BT_KBT(kma_ma(b_lp1),dk_tien_bh(b_lp),dk_t_that(b_lp),dk_bth(b_lp),kma_nd(b_lp1),b_kho);
                else
					b_s:='PBH_BT_NONG_KBT_'||kma_ma(b_lp1);
					...
					execute immediate b_lenh using b_ma_dvi,b_so_id,b_so_id_dt,dk_tien_bh(b_lp),dk_lke(b_lp),
						dk_t_that(b_lp),dk_bth(b_lp),kma_nd(b_lp1),dt_ct,out b_loi;
...
if b_v<>0 then
    ...
    if b_c is not null then
        ...
        for b_lp1 in 1..kma_ma.count loop
            if kma_ma(b_lp1) in('MVU','GVU','KVU') then
				b_loi:=FBH_BT_KBT(kma_ma(b_lp1),b_tien_bh,b_t_that,b_tien,kma_nd(b_lp1),b_kho);
            else
				b_s:='PBH_BT_NONG_KBT_'||kma_ma(b_lp1);
				select count(*) into b_i1 from USER_PROCEDURES where OBJECT_NAME=b_s;
				if b_i1=0 then b_loi:='loi:Chua tao ham '||b_s||':loi'; return; end if;
...
/
create or replace procedure PBH_BT_NONG_KBT_GVU(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_tien_bh number,b_lke number,
...
/
create or replace procedure PBH_BT_NONG_KBT_KVU(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_tien_bh number,b_lke number,
    b_t_that number,b_bth number,b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
...
/
create or replace procedure PBH_BT_NONG_KBT_MVU(
    b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_tien_bh number,b_lke number,
    b_t_that number,b_bth number,b_nd varchar2,dt_ct clob,b_loi out varchar2)
as
    b_ndN number:=PKH_LOC_CHU_SO(b_nd,'F');
begin
-- Nam - Muc mien thuong/vu
