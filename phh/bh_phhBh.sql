create or replace procedure PBH_PHHBH_CT(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_lan number;
    dt_ct clob; dt_dkbs clob; dt_lt clob; dt_dk clob; dt_hk clob; dt_them clob; 
    ds_ct clob; ds_dk clob; ds_dkbs clob:=''; ds_pvi clob;
    ds_lt clob:=''; ds_dkth clob; ds_kbt clob:=''; ds_ttt clob:=''; ds_gchu clob:=''; dt_kytt clob;
...
select count(*) into b_i1 from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_hk';
if b_i1=1 then
    select txt into dt_hk from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_hk';
end if;
select count(*) into b_i1 from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_them';
if b_i1=1 then
    select txt into dt_them from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_them';
end if;
...
select count(*) into b_i1 from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_gchu';
if b_i1=1 then
    select txt into ds_gchu from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_gchu';
end if;
...
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,
    'dt_ct' value dt_ct,'dt_dk' value dt_dk,'dt_dkbs' value dt_dkbs,'dt_lt' value dt_lt,'dt_hk' value dt_hk,'dt_them' value dt_them,
...
/
create or replace procedure PBH_PHHBH_NH_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,b_lan out number,
    
    dt_ct in out clob,dt_dk clob,dt_dkbs clob,dt_lt clob,dt_hk clob,dt_them clob,
    ds_ct clob,ds_dk clob,ds_dkbs clob,ds_pvi clob,ds_lt clob,ds_dkth clob,ds_kbt clob,ds_ttt clob,ds_gchu clob,dt_kytt clob,
-- Chung
...
-- Rieng
    ...
    dvi_dk_lut pht_type.a_var,dvi_hs_lut pht_type.a_num,dvi_mrr pht_type.a_var,
...
AS
    b_i1 number; b_ttrang_bg varchar2(1); b_tien number;
begin
-- Dan - Nhap
...
for b_lp in 1..dvi_so_id.count loop
    insert into bh_phhB_dvi values(b_ma_dvi,b_so_id,dvi_so_id(b_lp),
        dvi_dvi(b_lp),dvi_mrr(b_lp),dvi_ma_dt(b_lp),dvi_ddiem(b_lp),dvi_dk_lut(b_lp),dvi_hs_lut(b_lp),
...
for b_lp in 1..dk_so_id.count loop
    if dk_lh_nv(b_lp)<>' ' then
        ...
        insert into bh_phhB_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),dk_ma(b_lp),dk_ten(b_lp),dk_kieu(b_lp),
...
if dt_hk is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_hk',dt_hk);
end if;
if dt_them is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_them',dt_them);
end if;
insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_ct',ds_ct);
insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_dk',ds_dk);
insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_pvi',ds_pvi);
if ds_dkbs is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_dkbs',ds_dkbs);
end if;
if ds_lt is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_lt',ds_lt);
end if;
if ds_dkth is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_dkth',ds_dkth);
end if;
if ds_kbt is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_kbt',ds_kbt);
end if;
if ds_ttt is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_ttt',ds_ttt);
end if;
if ds_gchu is not null then
    insert into bh_phhB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_gchu',ds_gchu);
end if;
...
insert into bh_phhB_ls (ma_dvi,so_id,lan,loai,txt) select ma_dvi,so_id,lan,loai,txt
    from bh_phhB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id  and lan=b_lan;
...
/
create or replace procedure PBH_PHHBH_NH(
...
AS
    ...
    ds_ct clob; ds_dk clob; ds_dkbs clob; ds_pvi clob; ds_lt clob; ds_dkth clob; ds_kbt clob; ds_ttt clob; ds_gchu clob; dt_kytt clob; dt_cdt clob;
-- Chung
...
-- Rieng
    ...
    dvi_gio_kt pht_type.a_var; dvi_ngay_kt pht_type.a_num; dvi_ngay_cap pht_type.a_num; 
    dvi_phi pht_type.a_num; dvi_thue pht_type.a_num; dvi_ttoan pht_type.a_num; 
    dvi_giam pht_type.a_num; dvi_so_idP pht_type.a_num; dvi_tlbt pht_type.a_num;
...
b_lenh:=FKH_JS_LENHc('dt_ct,dt_dk,dt_dkbs,dt_lt,dt_hk,dt_them,ds_ct,ds_dk,ds_dkbs,ds_pvi,ds_lt,ds_dkth,ds_kbt,ds_ttt,ds_gchu,dt_kytt');
EXECUTE IMMEDIATE b_lenh into dt_ct,dt_dk,dt_dkbs,dt_lt,dt_hk,dt_them,ds_ct,ds_dk,ds_dkbs,ds_pvi,ds_lt,ds_dkth,ds_kbt,ds_ttt,ds_gchu,dt_kytt using b_oraIn;
...
PBH_PHHBH_NH_NH(
    ...
    dt_ct,dt_dk,dt_dkbs,dt_lt,dt_hk,dt_them,
    ds_ct,ds_dk,ds_dkbs,ds_pvi,ds_lt,ds_dkth,ds_kbt,ds_ttt,ds_gchu,dt_kytt,
-- Chung
...
-- Rieng
    ...
    dvi_so_id,dvi_kieu_gcn,dvi_gcn,dvi_gcnG,dvi_dvi,dvi_ma_dt,dvi_kvuc,dvi_lvuc,dvi_ddiem,
    dvi_dk_lut,dvi_hs_lut,dvi_mrr,dvi_cdt,dvi_tdx,dvi_tdy,dvi_bk,dvi_gio_hl,dvi_ngay_hl,
    dvi_gio_kt,dvi_ngay_kt,dvi_ngay_cap,dvi_phi,dvi_thue,dvi_ttoan,dvi_giam,dvi_so_idP,
    dk_so_id,dk_ma,dk_ten,dk_tc,dk_ma_ct,dk_kieu,dk_lkeM,dk_lkeP,dk_lkeB,dk_luy,dk_ktru,
    dk_ma_dk,dk_ma_dkC,dk_lh_nv,dk_t_suat,dk_cap,dk_lbh,dk_nv,
    dk_tien,dk_pt,dk_phi,dk_thue,dk_ttoan,dk_ptB,dk_phiB,dk_ptG,dk_phiG,dk_pvi_ma,dk_pvi_tc,dk_pvi_ktru,
    lt_so_id,lt_lt,lt_kbt,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
