...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_phh_sp a,(select distinct ma_sp from bh_phh_phi where nhom in('G','T') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_cdich from
    bh_ma_cdich a,(select distinct cdich from bh_phh_phi where nhom in('G','T') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten returning clob) into cs_goi from
    bh_phh_goi a,(select distinct goi from bh_phh_phi where nhom in('G','T') and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select json_object('cs_sp' value cs_sp,'cs_cdich' value cs_cdich,'cs_goi' value cs_goi,
    'cs_khd' value cs_khd,'cs_kbt' value cs_kbt,'cs_ttt' value cs_ttt returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PHHG_CT(
...
select json_object(so_hd,ma_kh) into dt_ct from bh_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
select txt into dt_pvi from bh_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_pvi';
...
select count(*) into b_i1 from bh_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_kbt';
if b_i1=1 then
    select txt into dt_kbt from bh_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_kbt';
end if;
select count(*) into b_i1 from bh_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1=1 then
    select txt into dt_ttt from bh_phh_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly FBH_PHHG_KHO:loi'; end if;
end;
/
create or replace procedure FBH_PHHG_PHI(
...
AS
    b_lenh varchar2(1000); b_i1 number; b_i2 number; b_iX number; b_so_idD number; b_so_idG number:=0;
    b_kt number:=0; b_ktL number:=0; b_maHH varchar2(5); b_pt number; b_phi number; b_dk varchar2(1);
    b_nhom varchar2(1); b_ma_sp varchar2(10); b_cdich varchar2(10); b_goi varchar2(10); b_tg number:=1;
    b_ngay_hl number; b_ngay_kt number; b_ngay_cap number; b_capD number; b_capC number;
    b_ngay_hlC number; b_ngay_ktC number; b_ngay_capC number; b_ktru varchar2(500);
    b_mrr varchar2(500); b_kho number; b_c_thue varchar2(1); b_hs number;
    b_nt_tien varchar2(5); b_nt_phi varchar2(5); b_tp number:=0; b_tien number;
    b_kieu_hd varchar2(1); b_so_hdG varchar2(20); b_pt_hang number;
    b_so_idB number; b_ttoan number; b_ttoanC number; b_tygia number;
    ...
    a_ptC pht_type.a_num; a_phiX pht_type.a_num;
    ...
    pvi_maX pht_type.a_var; pvi_tcX pht_type.a_var; pvi_tenX pht_type.a_var;
    pvi_ptTSX pht_type.a_num; pvi_ptKHX pht_type.a_num; pvi_ktruX pht_type.a_var;
    pvi_ptTSbX pht_type.a_num; pvi_ptKHbX pht_type.a_num;
    pvi_ptTScX pht_type.a_num; pvi_ptKHcX pht_type.a_num;
    pvi_loaiX pht_type.a_var; pvi_ma_ctX pht_type.a_var;

    bs_ma pht_type.a_var; bs_tien pht_type.a_num; bs_ptB pht_type.a_num; bs_pt pht_type.a_num;
begin
-- Dan - Tinh phi
b_lenh:=FKH_JS_LENH('kieu_hd,so_hd_g,nhom,ma_sp,cdich,goi,mrr,ngay_hl,ngay_kt,ngay_cap,nt_tien,nt_phi,tygia,c_thue,pt_hang');
EXECUTE IMMEDIATE b_lenh into b_kieu_hd,b_so_hdG,b_nhom,b_ma_sp,b_cdich,b_goi,b_mrr,
...
b_lenh:=FKH_JS_LENH('ma,ten,pttsb,ptts,ptkhb,ptkh,ktru,tc,loai,ma_ct');
EXECUTE IMMEDIATE b_lenh bulk collect into pvi_ma,pvi_ten,
    pvi_ptTSb,pvi_ptTS,pvi_ptKHb,pvi_ptKH,pvi_ktru,pvi_tc,pvi_loai,pvi_ma_ct using dt_pvi;
...
for b_lp in 1..pvi_ma.count loop
    pvi_loai(b_lp):=nvl(trim(pvi_loai(b_lp)),'C');
end loop;
for b_lp in 1..pvi_ma.count loop
    if pvi_loai(b_lp)='C' then
...
for b_lp in 1..pvi_ma.count loop
    ...
    if (pvi_ptTSb(b_lp)<100 and abs(pvi_ptTS(b_lp))< 100) or (pvi_ptTSb(b_lp)>100 and abs(pvi_ptTS(b_lp))>100) then
        pvi_ptTS(b_lp):=pvi_ptTSb(b_lp)-pvi_ptTS(b_lp);
    elsif pvi_ptTSb(b_lp)>100 then
        pvi_ptTS(b_lp):=pvi_ptTSb(b_lp)-ROUND(pvi_ptTSb(b_lp)*pvi_ptTS(b_lp)/100,b_tp);
    else
        pvi_ptTSc(b_lp):=pvi_ptTS(b_lp); pvi_ptTS(b_lp):=pvi_ptTSb(b_lp);
    end if;
    if (pvi_ptKHb(b_lp)<100 and abs(pvi_ptKH(b_lp))< 100) or (pvi_ptKHb(b_lp)>100 and abs(pvi_ptKH(b_lp))>100) then
        pvi_ptKH(b_lp):=pvi_ptKHb(b_lp)-pvi_ptKH(b_lp);
    elsif pvi_ptKHb(b_lp)>100 then
        pvi_ptKH(b_lp):=pvi_ptKHb(b_lp)-ROUND(pvi_ptKHb(b_lp)*pvi_ptKH(b_lp)/100,b_tp);
    else
        pvi_ptKHc(b_lp):=pvi_ptKH(b_lp); pvi_ptKH(b_lp):=pvi_ptKHb(b_lp);
    end if;
end loop;
for b_lp in 1..pvi_ma.count loop
    b_pvi_ptTS:=b_pvi_ptTS+pvi_ptTS(b_lp);
    b_pvi_ptTSb:=b_pvi_ptTSb+pvi_ptTSb(b_lp);
    b_pvi_ptTSc:=b_pvi_ptTSc+pvi_ptTSc(b_lp);
    b_pvi_ptKH:=b_pvi_ptKH+pvi_ptKH(b_lp);
    b_pvi_ptKHb:=b_pvi_ptKHb+pvi_ptKHb(b_lp);
...
for b_lp_dk in 1..dk_ma.count loop
    ...
    a_tien(b_kt):=dk_tien(b_lp_dk); a_pt(b_kt):=0; a_phi(b_kt):=0;
    ...
    if a_tc(b_kt)='C' then
        for b_lp_pvi in 1..pvi_ma.count loop
            ...
            a_t_suat(b_kt):=a_t_suat(b_ktL); a_cap(b_kt):=a_cap(b_ktL)+1;
            a_lbh(b_kt):=a_lbh(b_ktL); a_nv(b_kt):=a_nv(b_ktL);
            a_tien(b_kt):=0; b_tien:=a_tien(b_ktL);
            ...
            if a_lkeP(b_kt)='K' then
                 a_pt(b_kt):=0; a_phi(b_kt):=0; a_ptB(b_kt):=0; a_phiB(b_kt):=0; a_ptC(b_kt):=0;
            else
                if a_lbh(b_kt) in('TS','TB','HH') then
                    a_pt(b_kt):=pvi_ptTS(b_lp_pvi); a_ptB(b_kt):=pvi_ptTSb(b_lp_pvi); a_ptC(b_kt):=pvi_ptTSc(b_lp_pvi);
                else
                    a_pt(b_kt):=pvi_ptKH(b_lp_pvi); a_ptB(b_kt):=pvi_ptKHb(b_lp_pvi); a_ptC(b_kt):=pvi_ptKHc(b_lp_pvi);
                end if;
                if a_pt(b_kt)>100 then a_phi(b_kt):=a_pt(b_kt);
                else a_phi(b_kt):=b_tien*a_pt(b_kt)/100;
                end if;
                a_phi(b_kt):=ROUND(a_phi(b_kt)*b_kho*b_tg,b_tp)-a_ptC(b_kt);
                if a_ptB(b_kt)>100 then a_phiB(b_kt):=a_pt(b_kt);
                else a_phiB(b_kt):=b_tien*a_ptB(b_kt)/100;
                end if;
                a_phiB(b_kt):=ROUND(a_phiB(b_kt)*b_kho*b_tg,b_tp);
                if a_phiB(b_kt)<0 then a_phiB(b_kt):=0; end if;
                if b_pt_hang<>100 and b_maHH='HH' then
                    a_pt(b_kt):=round(a_pt(b_kt)*b_pt_hang/100,2);
                    a_phi(b_kt):=round(a_phi(b_kt)*b_pt_hang/100,b_tp);
                    a_ptB(b_kt):=round(a_ptB(b_kt)*b_pt_hang/100,2);
    ...
    elsif a_lkeP(b_kt)='G' and a_tien(b_kt)<>0 then
        if a_lbh(b_kt) in('TS','TB','HH') then
            a_pt(b_kt):=b_pvi_ptTS; a_ptB(b_kt):=b_pvi_ptTSb; a_ptC(b_kt):=b_pvi_ptTSc;
        else
            a_pt(b_kt):=b_pvi_ptKH; a_ptB(b_kt):=b_pvi_ptKHb; a_ptC(b_kt):=b_pvi_ptKHc;
        end if;
        b_tien:=a_tien(b_kt);
        if a_pt(b_kt)>100 then a_phi(b_kt):=a_pt(b_kt);
        else a_phi(b_kt):=b_tien*a_pt(b_kt)/100;
...
for b_lp_bs in 1..bs_ma.count loop
    ...
    if b_i1=1 then
        ...
        a_lkeM(b_kt):='G'; a_lkeP(b_kt):='G'; a_lkeB(b_kt):='G'; a_luy(b_kt):='K'; a_ktru(b_kt):=' ';
        a_ma_dk(b_kt):=' '; a_ma_dkC(b_kt):=' '; a_cap(b_kt):=0; a_lbh(b_kt):=' ';
    end if;
    ...
    if (a_ptB(b_kt)<100 and abs(a_pt(b_kt))< 100) or (a_ptB(b_kt)>100 and abs(a_pt(b_kt))>100) then
        ...
        a_pt(b_kt):=ROUND(a_ptB(b_kt)*a_pt(b_kt)/100,2);
    else
...
for b_lp in 1..dk_ma.count loop
    ...
    for b_lp1 in 1..a_ma.count loop
        if (a_lkeP(b_lp1)='G' and a_ma(b_lp1)=dk_ma(b_lp)) or (a_tc(b_lp1)='C' and
            instr(a_ma(b_lp1),'>')>0 and instr(a_ma(b_lp1),dk_ma(b_lp))=1) then
...
if b_so_idG<>0 then
    ...
    select so_id,ngay_hl,ngay_kt,ngay_cap bulk collect into a_so_idG,a_ngay_hlG,a_ngay_ktG,a_ngay_capG from bh_phh where
    ...
    if a_so_idG.count<>0 then
        ...
        for b_lpG in 1..a_so_idG.count loop
            ...
            select ma,phi,phiB bulk collect into dk_maG,dk_phiG,dk_phiB from bh_phh_dk where
                ma_dvi=b_ma_dvi and so_id=a_so_idG(b_lpG) and tc='C';
...
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly FBH_PHHG_PHI:loi'; end if;
end;
/
create or replace procedure FBH_PHHG_PHIb(b_tp number,
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly FBH_PHHG_PHIb:loi'; end if;
end;
/
create or replace procedure PBH_PHHG_PHI(
...
FBH_PHHG_PHI(b_ma_dvi,dt_ct,dt_dk,dt_dkbs,dt_pvi,b_so_idP,
...
/
create or replace procedure PBH_PHHG_TESTr(
    b_ma_dvi varchar2,b_nsd varchar2,dt_ct in out clob,dt_dk clob,dt_dkbs clob,dt_pvi clob,
    b_ng_huong out nvarchar2,b_ma_sp out varchar2,b_cdich out varchar2,b_goi out varchar2,
    b_dvi out nvarchar2,b_ddiem out nvarchar2,b_kvuc out varchar2,
    b_tdx out number,b_tdy out number,b_bk out number,b_ma_dt out varchar2,b_lvuc out varchar2,
    b_mrr out varchar2,b_cdt out varchar2,b_dk_lut out varchar2,b_hs_lut out number,
    b_nt_tien out varchar2,b_nt_phi out varchar2,b_c_thue out varchar2,b_tlbt out number,b_so_idP out number,
...
begin
-- Dan - Nhap
b_lenh:='ttrang,ngay_hl,ngay_kt,ma_sp,cdich,goi,';
b_lenh:=b_lenh||'dvi,ddiem,tdx,tdy,ma_dt,lvuc,mrr,cdt,dk_lut,hs_lut,';
b_lenh:=b_lenh||'nt_tien,nt_phi,tygia,c_thue,thue,ttoan,tlbt,so_idp';
b_lenh:=FKH_JS_LENH(b_lenh);
EXECUTE IMMEDIATE b_lenh into
    b_ttrang,b_ngay_hl,b_ngay_kt,b_ma_sp,b_cdich,b_goi,
    b_dvi,b_ddiem,b_tdx,b_tdy,b_ma_dt,b_lvuc,b_mrr,b_cdt,b_dk_lut,b_hs_lut,
    b_nt_tien,b_nt_phi,b_tygia,b_c_thue,b_thueH,b_ttoanH,b_tlbt,b_so_idPn using dt_ct;
if b_dvi=' ' or b_ddiem=' ' then b_loi:='loi:Nhap dia diem, dia chi bao hiem:loi'; return; end if;
if nvl(b_tdx,0)=0 then b_loi:='loi:Chua lay duoc toa do :loi'; return; end if;
b_kvuc:=' '; b_bk:=25; b_dk_lut:=nvl(trim(b_dk_lut),'K');
if b_ma_sp<>' ' and FBH_PHH_SP_HAN(b_ma_sp)<>'C' then b_loi:='loi:Sai ma san pham:loi'; return; end if;
if b_cdich<>' ' and FBH_MA_CDICH_HAN(b_cdich)<>'C' then b_loi:='loi:Het chien dich:loi'; return; end if;
if b_goi<>' ' and FBH_PHH_GOI_HAN(b_goi)<>'C' then b_loi:='loi:Ma goi het han:loi'; return; end if;
b_loi:='loi:Sai ma doi tuong:loi';
if b_ma_dt is null then return; end if;
b_ma_dt:=PKH_MA_TENl(b_ma_dt);
if FBH_PHH_DTUONG_HAN(b_ma_dt)<>'C' then return; end if;
b_loi:='loi:Sai linh vuc:loi';
if b_lvuc is null then return; end if;
b_lvuc:=PKH_MA_TENl(b_lvuc);
if FBH_MA_LVUC_HAN(b_lvuc)<>'C' then return; end if;
b_loi:='loi:Sai muc rui ro:loi';
if b_mrr is null then return; end if;
b_mrr:=PKH_MA_TENl(b_mrr);
...
if b_ttrang in('T','D') then
    ...
    if b_i1<>0 then
        ...
        PBH_PHHG_KHD(dt_ct,dt_dk,dt_khd,b_loi);
        if b_loi is not null then return; end if;
    end if;
end if;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_PHHG_TESTr:loi'; end if;
end;
/
create or replace procedure PBH_PHHG_NH_NH(
...
-- Rieng
    ...
    b_dvi nvarchar2,b_ddiem nvarchar2,b_kvuc varchar2,b_tdx number,b_tdy number,b_bk number,
    b_ma_dt varchar2,b_lvuc varchar2,b_mrr varchar2,b_cdt varchar2,
    b_dk_lut varchar2,b_hs_lut number,b_tlbt number,
...
for b_lp in 1..dk_lh_nv.count loop
    ...
    insert into bh_phh_dk values(b_ma_dvi,b_so_id,b_so_idD,b_lp,
...
insert into bh_phh_dvi values(b_ma_dvi,b_so_id,b_so_idD,0,b_kieu_hd,b_so_hd,b_so_hd_g,
    b_dvi,b_ma_dt,b_kvuc,b_lvuc,b_ddiem,b_dk_lut,b_hs_lut,b_mrr,b_cdt,b_tdx,b_tdy,b_bk,
    b_gio_hl,b_ngay_hl,b_gio_kt,b_ngay_kt,b_ngay_cap,
    b_so_idP,b_giam,b_phi,b_thue,b_ttoan,b_tlbt);
...
insert into bh_phh values(
    ...
    b_nt_tien,b_nt_phi,b_c_thue,b_ma_sp,b_cdich,b_goi,1,b_tien,
...
insert into bh_phh_txt values(b_ma_dvi,b_so_id,'dt_pvi',dt_pvi);
if dt_dkbs is not null then
    insert into bh_phh_txt values(b_ma_dvi,b_so_id,'dt_dkbs',dt_dkbs);
end if;
if dt_ttt is not null then
    insert into bh_phh_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if dt_lt is not null then
    insert into bh_phh_txt values(b_ma_dvi,b_so_id,'dt_lt',dt_lt);
end if;
if dt_kbt is not null then
    insert into bh_phh_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
if REGEXP_COUNT(b_ddiem,',')>2 then
    insert into bh_phh_ttu values(b_ma_dvi,b_so_idD,b_so_idD,b_dvi,b_tdx,b_tdy,b_bk,b_ngay_hl,b_ngay_kt);
end if;
if b_ttrang in('T','D') then
    select JSON_ARRAYAGG(json_object(
        ma,ten,tc,ma_ct,tien,pt,phi,cap,ma_dk,ma_dkC,lh_nv,t_suat,thue,ttoan,ptB,ptG,phiG,lkeM,lkeP,lkeB,luy,pvi_ma,pvi_ktru,pvi_tc)
        order by bt returning clob) into b_txt
        from bh_phh_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
    insert into bh_phh_kbt values(b_ma_dvi,b_so_id,b_so_idD,b_txt,dt_lt,dt_kbt);
    select json_object('ma_dvi' value b_ma_dvi,'nsd' value b_nsd,'so_id' value b_so_id,'so_hd' value b_so_hd,
        ...
        'dvi_ksoat' value b_ma_dvi,'ksoat' value b_nsd,'bangg' value 'bh_phh',
...
if b_ttrang='D' then
    ...
    if b_loi is not null then return; end if;
    if b_ma_ke<>' ' then
        insert into bh_hd_goc_ttindt values(b_ma_dvi,b_so_idD,b_so_idD,'PHH',b_dvi,b_ma_kh,b_ngay_kt,b_ddiem,b_ma_ke);
    end if;
end if;
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_PHHG_NH_NH:loi'; end if;
end;
/
create or replace procedure PBH_PHHG_NH(
...
-- Rieng
	...
	b_cdt varchar2(5); b_dk_lut varchar2(1); b_hs_lut number; b_tlbt number;
    dk_ma pht_type.a_var; dk_ten pht_type.a_nvar; dk_tc pht_type.a_var;
    dk_ma_ct pht_type.a_var; dk_kieu pht_type.a_var; 
    dk_lkeM pht_type.a_var; dk_lkeP pht_type.a_var; dk_lkeB pht_type.a_var;
    dk_luy pht_type.a_var; dk_ktru pht_type.a_var;
...
if b_so_id<>0 then
    ...
    if b_i1<>0 then
        select ngay_ht,kieu_hd,ttrang into b_ngay_htC,b_kieu_hd,b_ttrang from bh_phh
        ...
        if b_ttrang in('T','D') then b_kieu_hd:='N'; else b_kieu_hd:='X'; end if;
...
PBH_PHHG_NH_NH(
    ...
    tt_ngay,tt_tien,
-- Rieng
    b_ng_huong,b_ma_sp,b_cdich,b_goi,b_so_idP,b_dvi,b_ddiem,b_kvuc,
    b_tdx,b_tdy,b_bk,b_ma_dt,b_lvuc,b_mrr,b_cdt,b_dk_lut,b_hs_lut,b_tlbt,
    dk_ma,dk_ten,dk_tc,dk_ma_ct,dk_kieu,dk_lkeM,dk_lkeP,dk_lkeB,
    dk_luy,dk_ktru,dk_ma_dk,dk_ma_dkC,dk_lh_nv,dk_t_suat, 
    dk_cap,dk_lbh,dk_nv,dk_tien,dk_pt,dk_phi,dk_thue,dk_ttoan,dk_ptB,dk_phiB,dk_ptG,dk_phiG,
    dk_pvi_ma,dk_pvi_tc,dk_pvi_ktru,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
