...
begin
-- Dan - Phan tich phuc hoi
delete bh_hd_goc_ttpt where ma_dvi=b_ma_dvi and so_id_tt=b_so_id_ps;
select nvl(min(ngay_ht),0),min(nt_phi) into b_ngay_ht,b_nt_phi from bh_phhP where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
...
select lh_nv,t_suat,ma_dt,sum(phi),sum(thue),sum(phi_qd),sum(thue_qd) BULK COLLECT
    ...
    from bh_phhP_dk where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps group by lh_nv,t_suat,ma_dt;
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_PHHP_PT:loi'; end if;
end;
/
create or replace PROCEDURE PBH_PHHP_PTDT(
    b_ma_dvi varchar2,b_so_id number,b_so_id_ps number,b_loi out varchar2)
AS
    b_i1 number; b_tp number:=0; b_hs number; b_bt number;
    b_nv varchar2(10):='PHH'; b_so_idD number; b_so_idB number; b_nt_phi varchar2(5);
...
select count(*) into b_i1 from bh_phhP where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
...
if b_i1=0 then b_loi:=''; return; end if;
select so_id_d,nt_phi into b_so_idD,b_nt_phi from bh_phh where ma_dvi=b_ma_dvi and so_id=b_so_id;
select count(*) into b_i1 from bh_phh_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idD and so_id_dt<>b_so_idD;
if b_i1=0 then
...
b_loi:='';
exception when others then if b_loi is null then b_loi:='loi:Loi xu ly PBH_PHHP_PTDT:loi'; end if;
end;
/
create or replace procedure PBH_PHHP_TH_ID(
...
PBH_PHHP_PT(b_ma_dvi,b_so_id,b_so_id_ps,b_loi);
if b_loi is not null then return; end if;
PBH_PHHP_PTDT(b_ma_dvi,b_so_id,b_so_id_ps,b_loi);
if b_loi is not null then return; end if;
...
/
create or replace procedure PBH_PHHP_LKE(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_so_hd varchar2(20); b_tu number; b_den number; b_so_idB number;
...
--nam: check hop dong da duyet thi di tiep
b_so_idB:=FBH_PHH_HD_SO_IDc(b_ma_dvi,b_so_hd,'C');
if b_so_idB=0 then b_loi:='loi:Hop dong da xoa hoac chua duyet:loi'; raise PROGRAM_ERROR; end if;
select json_object(nt_tien,nt_phi) into cs_ct from bh_phh where ma_dvi=b_ma_dvi and so_id=b_so_idB;
select count(*) into b_dong from bh_phhP where ma_dvi=b_ma_dvi and so_id=b_so_idB;
PKH_LKE_TRANG(b_dong,b_tu,b_den);
select JSON_ARRAYAGG(json_object(ngay_ht) order by ngay_ht desc returning clob) into cs_lke
    from bh_phhP where ma_dvi=b_ma_dvi and so_id=b_so_idB;
...
/
create or replace procedure PBH_PHHP_LKE_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_hd=' ' then b_loi:='loi:Nhap so hop dong:loi'; raise PROGRAM_ERROR; end if;
b_so_id:=FBH_PHH_HD_SO_IDd(b_ma_dvi,b_so_hd);
if b_so_id=0 then b_loi:='loi:Hop dong da xoa:loi'; raise PROGRAM_ERROR; end if;
select count(*) into b_dong from bh_phhP where ma_dvi=b_ma_dvi and so_id=b_so_id;
select nvl(min(sott),b_dong) into b_tu from
    (select ngay_ht,rownum sott from bh_phhP where ma_dvi=b_ma_dvi and so_id=b_so_id order by ngay_ht)
...
/
create or replace procedure PBH_PHHP_CT(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hd:=nvl(trim(b_so_hd),' '); b_ngay_ht:=nvl(b_ngay_ht,0);
select nvl(min(so_id_ps),0) into b_so_id_ps from bh_phhP where ma_dvi=b_ma_dvi and so_hd=b_so_hd and ngay_ht=b_ngay_ht;
if b_so_id_ps=0 then b_loi:='loi:Phuc hoi da xoa:loi'; raise PROGRAM_ERROR; end if;
select txt into dt_ct from bh_phhP_txt where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps and loai='dt_ct';
select txt into dt_hd from bh_phhP_txt where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps and loai='dt_hd';
select json_object('dt_ct' value dt_ct,'dt_hd' value dt_hd returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PHHP_THEM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_i1 number; b_so_hd varchar2(20); b_so_id number;
    b_dvi nvarchar2(500); b_lan number; b_bth number; b_so_idB number;
begin
-- Dan - Xem
...
b_so_id:=FBH_PHH_HD_SO_IDd(b_ma_dvi,b_so_hd);
if b_so_id=0 then b_loi:='loi:Hop dong da xoa:loi'; raise PROGRAM_ERROR; end if;
for r_lp in (select so_id_dt,max(ngay_qd) ngay_qd from bh_bt_phh
    where ma_dvi_ql=b_ma_dvi and so_id_hd=b_so_id and ttrang='D' group by so_id_dt) loop
    select count(*) into b_i1 from bh_phhP_dvi where
        ma_dvi=b_ma_dvi and so_id=b_so_id and ngay_ht>=r_lp.ngay_qd and so_id_dt=r_lp.so_id_dt;
    if b_i1<>0 then continue; end if;
    b_so_idB:=FBH_PHH_SO_IDb(b_ma_dvi,b_so_id);
    select nvl(sum(b.tien),0) into b_bth from bh_bt_phh a,bh_bt_phh_dk b where
    ...
    select min(dvi) into b_dvi from bh_phh_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=r_lp.so_id_dt;
    select count(*) into b_lan from bh_phhP_dvi where
...
/
create or replace procedure PBH_PHHP_XOA_XOA(
...
begin
-- Dan - Xoa
b_loi:='loi:Loi xu ly PBH_PHHP_XOA_XOA:loi';
select count(*) into b_i1 from bh_phhP where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
if b_i1=0 then b_loi:=''; return; end if;
select so_id_kt,nsd,so_id,ngay_ht into b_so_idK,b_nsdC,b_so_id,b_ngay_ht
    from bh_phhP where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
...
if b_nsdC<>b_nsd then b_loi:='loi:Khong sua, xoa phuc hoi nguoi khac nhap:loi'; return; end if;
select count(*) into b_i1 from bh_phhP where ma_dvi=b_ma_dvi and so_id=b_so_id and ngay_ht>b_ngay_ht;
if b_i1<>0 then b_loi:='loi:Khong sua, xoa phuc hoi da co phuc hoi:loi'; return; end if;
select count(*) into b_i1 from bh_phhP_dvi a,bh_bt_phh b where
...
delete bh_phhP_dk where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
delete bh_phhP_txt where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
delete bh_phhP_dvi where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
delete bh_phhP where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_PHHP_NH_NH(
...
begin
-- Dan - Nhap
b_loi:='loi:Loi xu ly PBH_PHHP_NH_NH:loi';
b_so_idB:=FBH_PHH_SO_IDb(b_ma_dvi,b_so_id);
...
for b_lp in 1..a_so_id_dt.count loop
    ...
    select nvl(sum(phi),0) into b_phiT from bh_phh_dk where
        ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=a_so_id_dt(b_lp) and lh_nv<>' ' and FBH_MA_LHNV_LOAI(lh_nv)='V';
    if b_phiT=0 then  continue; end if;
    b_hs:=a_phi(b_lp)/b_phiT; b_lh_nv:=' '; b_phiM:=0; b_phiT:=a_phi(b_lp); b_thueT:=a_thue(b_lp);
    for r_lp in(select lh_nv,t_suat,sum(phi) phi,sum(thue) thue from bh_phh_dk where
        ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=a_so_id_dt(b_lp) and
        phi<>0 and lh_nv<>' ' and FBH_MA_LHNV_LOAI(lh_nv)='V' group by lh_nv,t_suat) loop
        ...
        insert into bh_phhP_dk values(b_ma_dvi,b_so_id_ps,b_so_id,a_so_id_dt(b_lp),b_ngay_ht,
    ...
    if b_phiT<>0 or b_thueT<>0 then
        ...
        update bh_phhP_dk set phi=phi+b_phiT,thue=thue+b_thueT,phi_qd=phi_qd+b_phiT_qd,thue_qd=thue_qd+b_thueT_qd
...
select sum(phi),sum(thue),sum(phi_qd),sum(thue_qd) into b_phiT,b_thueT,b_phiT_qd,b_thueT_qd
    from bh_phhP_dk where ma_dvi=b_ma_dvi and so_id_ps=b_so_id_ps;
insert into bh_phhP values(
...
forall b_lp in 1..a_so_id_dt.count
    insert into bh_phhP_dvi values(b_ma_dvi,b_so_id_ps,b_so_id,a_so_id_dt(b_lp),b_ngay_ht,
...
insert into bh_phhP_txt values(b_ma_dvi,b_so_id_ps,b_so_id,b_ngay_ht,'dt_ct',dt_ct);
insert into bh_phhP_txt values(b_ma_dvi,b_so_id_ps,b_so_id,b_ngay_ht,'dt_hd',dt_hd);
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_PHHP_NH(
...
AS
    ...
    b_ma_kh varchar2(20); b_ten nvarchar2(500); b_ma_thue varchar2(50); b_dchi nvarchar2(1000);
    a_so_id_dt pht_type.a_num; a_dvi pht_type.a_nvar; a_ma_dt pht_type.a_var;
...
if b_so_id_ps<>0 then
    PBH_PHHP_XOA_XOA(b_ma_dvi,b_nsd,b_so_id_ps,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
else
...
select nt_tien,nt_phi into b_nt_tien,b_nt_phi from bh_phh where ma_dvi=b_ma_dvi and so_id=b_so_idB;
for b_lp in 1..a_so_id_dt.count loop
    b_loi:='loi:Dia diem dong '||to_char(b_lp)||' da xoa:loi';
    select dvi,ma_dt into a_dvi(b_lp),a_ma_dt(b_lp) from bh_phh_dvi
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_PHHP_TH_ID(b_ma_dvi,'T',b_so_id,b_so_id_ps,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
if b_ma_kh='VANGLAI' then
    ...
    select cmt,dchi into b_ma_thue,b_dchi from bh_dtac_ma where ma=b_ma_kh;
end if;
...
/
create or replace procedure PBH_PHHP_XOA(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob)
AS
