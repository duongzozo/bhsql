create or replace function FBH_PTNCH_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return nvarchar2
AS
    b_kq nvarchar2(500):=' '; b_i1 number; b_id number; b_txt clob;
    a_ds_ct pht_type.a_clob;
begin
-- nam - Tra gia tri varchar2 trong txt
select count(*) into b_i1 from bh_ptnch_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
if b_i1=1 then
	select txt into b_txt from bh_ptnch_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
	PKH_JS_BONH(b_txt); b_kq:=FKH_JS_GTRIs(b_txt,b_tim);
end if;
return b_kq;
end;
/
create or replace function FBH_PTNCH_TXTn(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return number
AS
...
/
create or replace  function FBH_PTNCH_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Nam - Tra so hop dong
select min(so_hd) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace  function FBH_PTNCH_SO_IDd(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Nam - Tra so id dau
...
/
create or replace function FBH_PTNCH_SO_IDc(b_ma_dvi varchar2,b_so_id number,b_ttrang varchar2:='K') return number
as
    b_kq number:=0; b_so_idD number:=b_so_id;
begin
-- Nam - Tra so id cuoi
b_so_idD:=FBH_PTNCH_SO_IDd(b_ma_dvi,b_so_id);
if b_ttrang<>'C' then
    select nvl(max(so_id),0) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
else
    select nvl(max(so_id),0) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D';
end if;
return b_kq;
end;
/
create or replace function FBH_PTNCH_KIEU_HDd(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(1); b_so_idD number:=FBH_PTNCH_SO_IDd(b_ma_dvi,b_so_id);
begin
-- Nam - Tra so hop dong
select nvl(min(kieu_hd),' ') into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_id=b_so_idD;
return b_kq;
end;
/
create or replace function FBH_PTNCH_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2,b_nv varchar2:='') return number
as
    b_kq number;
begin
-- Nam - Tra so id
if nvl(b_nv,' ')=' ' then 
    select nvl(min(so_id),0) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
else 
    select nvl(min(so_id),0) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv;
end if;
return b_kq;
end;
/
create or replace function FBH_PTNCH_SO_IDt(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return number
as
    b_kq number:=0; b_so_idD number;
begin
-- Nam - Tra so id 
b_so_idD:=FBH_PTNCH_SO_ID(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang in('T','D') and ngay_ht<=b_ngay;
return b_kq;
end;
/
create or replace function FBH_PTNCH_SO_IDb(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return number
as
    b_kq number:=0; b_so_idD number;
begin
-- Nam - Tra so id 
b_so_idD:=FBH_PTNCH_SO_ID(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_ptnch where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D' and ngay_ht<=b_ngay;
return b_kq;
end;
/
create or replace procedure PBH_PTNCH_SO_ID(
...
b_so_id:=FBH_PTNCH_SO_ID(b_ma_dvi,b_so_hd,b_nv);
if b_so_id=0 then b_loi:='loi:So hop dong da xoa:loi'; raise PROGRAM_ERROR; end if;
select json_object('ma_dvi' value b_ma_dvi,'so_id' value b_so_id,'nv' value b_nv) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure FBH_PTNCH_BPHI_DKm(
...
b_loi:='loi:Loi xu ly FBH_PTNCH_BPHI_DKm:loi';
select count(*),nvl(max(tien),0) into b_i1,b_tienM from bh_ptnch_phi_dk where
...
select nvl(min(bt),0) into b_bt from bh_ptnch_phi_dk
    where so_id=b_so_id and ma=b_ma and lh_bh=b_nv and tien=b_tienM;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_PTNCH_BPHI_DKm(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    ...
    b_nv varchar2(1); b_nhom varchar2(1); b_ma_sp varchar2(10); b_cdich varchar2(10); b_ghan varchar2(1); b_ngay_hl number;
begin
-- Nam - Lay %phi theo khoang muc trach nhiem
delete temp_1; commit;
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('nhom,ma_sp,cdich,ghan_m,ngay_hl,nv,ma,tien');
EXECUTE IMMEDIATE b_lenh into b_nhom,b_ma_sp,b_cdich,b_ghan,b_ngay_hl,b_nv,b_ma,b_tien using b_oraIn;
...
FBH_PTNCH_BPHI_DKm(b_so_id,b_ma,b_tien,b_nv,b_bt,b_loi);
for r_lp in (select * from bh_ptnch_phi_dk where so_id=b_so_id and bt>=b_bt order by bt) loop
...
/
create or replace procedure PBH_PTNCH_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_phong varchar2(10);
    b_ngay_ht number; b_klk varchar2(1); b_tu number; b_den number; b_nv varchar2(10); b_nhom varchar2(1);
    b_dong number; cs_lke clob;
begin
-- Nam - Liet ke theo ngay
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('ngay_ht,klk,tu,den,nv,nhom');
EXECUTE IMMEDIATE b_lenh into b_ngay_ht,b_klk,b_tu,b_den,b_nv,b_nhom using b_oraIn;
if b_klk='N' then
    select count(*) into b_dong from bh_ptnch where ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and
        ngay_ht=b_ngay_ht and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_ptnch where
            ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hd)
    ...
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_ptnch  where
            ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and ngay_ht=b_ngay_ht and phong=b_phong  order by so_hd)
        where sott between b_tu and b_den;
else
    select count(*) into b_dong from bh_ptnch where ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and ngay_ht=b_ngay_ht;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_ptnch  where
            ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and ngay_ht=b_ngay_ht order by so_hd)
        where sott between b_tu and b_den;
end if;
...
/
create or replace procedure PBH_PTNCH_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_phong varchar2(10); b_so_hd varchar2(20);
    b_so_id number; b_ngay_ht number; b_klk varchar2(1); b_nv varchar2(10); b_nhom varchar2(1);
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','X');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('so_id,ngay_ht,klk,trangkt,nv,nhom');
EXECUTE IMMEDIATE b_lenh into b_so_id,b_ngay_ht,b_klk,b_trangKt,b_nv,b_nhom using b_oraIn;
b_so_hd:=FBH_PTNCH_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ptnch where
            ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
    ...
    select count(*) into b_dong from bh_ptnch where ma_dvi=b_ma_dvi and
        ngay_ht=b_ngay_ht ;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ptnch where
            ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and ngay_ht=b_ngay_ht   order by so_hd)
...
/
create or replace procedure PBH_PTNCH_XOA_XOA(
...
begin
-- Nam - Xoa
b_loi:='loi:Loi xu ly PBH_PTNCH_XOA_XOA:loi';
...
select so_id_kt,ttrang,ksoat,nsd into b_so_idK,b_ttrang,b_ksoat,b_nsdC
    from bh_ptnch where so_id=b_so_id;
...
b_loi:='loi:Loi xoa Table bh_ptnch:loi';
delete bh_hd_goc_ttindt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnch_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnch_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnch_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnch_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnch_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnch where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_PTNCH_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_comm varchar2:='C')
AS
...
PBH_PTNCH_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCH_PHIGr(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_i1 number; b_i2 number; b_iX number; b_lenh varchar2(1000);
    dt_ct clob; dt_dk clob; dt_dkbs clob;
    b_dthu number; b_tlph number; b_tp number:=0; b_nt_phi varchar2(5); b_nt_tien varchar2(5);
    b_so_hdG varchar2(20); b_ngay_hl number; b_ngay_kt number; b_ngay_cap number;
    b_tygia number:=1; b_kho number:=1; b_c_thue varchar2(1); b_tien_dt number;
    ...
    dk_cap pht_type.a_num;dk_ppt pht_type.a_num;
    ...
    dkbs_cap pht_type.a_num;dkbs_ppt pht_type.a_num;
...
b_lenh:=FKH_JS_LENH('so_id_dt,so_hd_g,dthu,tlph,ngay_hl,ngay_kt,ngay_cap,nt_phi,nt_tien,tygia,c_thue');
EXECUTE IMMEDIATE b_lenh into b_so_id_dt,b_so_hdG,b_dthu,b_tlph,b_ngay_hl,b_ngay_kt,b_ngay_cap,b_nt_phi,b_nt_tien,b_tygia,b_c_thue using dt_ct;
b_so_id_dt:=nvl(b_so_id_dt,0);
if b_so_hdG<>' ' then
    b_so_idG:=FBH_PTNCH_SO_ID(b_ma_dvi,b_so_hdG);
    if b_so_idG=0 then b_loi:='loi:GCN goc da xoa:loi'; return; end if;
end if;
if b_tygia is null or b_tygia=0 then b_tygia:=1; end if;
b_lenh:=FKH_JS_LENH('ma,ten,tien,ptb,ppt,pp,pt,phi,cap,tc,ma_ct,ma_dk,ma_dkc,kieu,lh_nv,t_suat,phib,lkem,lkep,lkeb,ptk,luy,bt');
EXECUTE IMMEDIATE b_lenh bulk collect into dk_ma,dk_ten,dk_tien,dk_ptB,dk_ppt,dk_pp,dk_pt,dk_phi,dk_cap,dk_tc,dk_ma_ct,
...
if trim(dt_dkbs) is not null then
  EXECUTE IMMEDIATE b_lenh bulk collect into dkbs_ma,dkbs_ten,dkbs_tien,dkbs_ptB,dkbs_ppt,dkbs_pp,dkbs_pt,dkbs_phi,dkbs_cap,dkbs_tc,dkbs_ma_ct,
...
if b_tlph<100 then b_tien_dt:=ROUND(b_tlph * b_dthu / 100,0);
else b_tien_dt:=ROUND(b_tlph * b_dthu,0);
end if;
...
for b_lp_dk in 1..dk_ma.count loop
  ...
  if dk_lkeP(b_lp_dk) not in ('T','N','K') then
    if dk_tien(b_lp_dk)<>0 then
        dk_ppt(b_lp_dk):=ROUND(b_tien_dt/dk_tien(b_lp_dk) * 100,4);
       if dk_ptk(b_lp_dk)<>'P' then
          if b_nt_phi<>'VND' and b_nt_phi<>b_nt_tien then
             dk_phiB(b_lp_dk):=ROUND((dk_ptB(b_lp_dk) + dk_ppt(b_lp_dk)) / b_tygia *b_kho,b_tp);
          elsif b_nt_tien<>'VND' and b_nt_phi<>b_nt_tien then
             dk_phiB(b_lp_dk):=ROUND((dk_ptB(b_lp_dk) + dk_ppt(b_lp_dk)) * b_tygia *b_kho,b_tp);
          else
             dk_phiB(b_lp_dk):=ROUND((dk_ptB(b_lp_dk) + dk_ppt(b_lp_dk)) *b_kho,b_tp);
          end if;
       elsif dk_ptk(b_lp_dk)<>'T' and dk_tien(b_lp_dk)<>0 and dk_ptB(b_lp_dk)<>0 then
          if b_nt_phi<>'VND' and b_nt_phi<>b_nt_tien then
             dk_phiB(b_lp_dk):=ROUND((dk_ptB(b_lp_dk) + dk_ppt(b_lp_dk))* dk_tien(b_lp_dk) / b_tygia *b_kho/ 100,b_tp);
          elsif b_nt_tien<>'VND' and b_nt_phi<>b_nt_tien then
             dk_phiB(b_lp_dk):=ROUND((dk_ptB(b_lp_dk) + dk_ppt(b_lp_dk))* dk_tien(b_lp_dk) * b_tygia *b_kho/ 100,b_tp);
          else
             dk_phiB(b_lp_dk):=ROUND((dk_ptB(b_lp_dk) + dk_ppt(b_lp_dk))* dk_tien(b_lp_dk) *b_kho/ 100,b_tp);
          end if;
       else dk_phiB(b_lp_dk):=0;
       end if;
       dk_ppt(b_lp_dk):=dk_ppt(b_lp_dk);
      end if;
      if dk_pp(b_lp_dk) = 'DG' and dk_pt(b_lp_dk)>0 then dk_phi(b_lp_dk):=ROUND(dk_pt(b_lp_dk),b_tp);
           ...
           dk_phi(b_lp_dk):=ROUND((dk_pt(b_lp_dk) + dk_ppt(b_lp_dk)) * dk_tien(b_lp_dk) *b_kho/ 100,b_tp);
        ...
        if dk_pp(b_lp_dk) = 'GG' then
             ...
             dk_phi(b_lp_dk):= dk_phi(b_lp_dk) - ROUND((dk_pt(b_lp_dk) + dk_ppt(b_lp_dk)) * dk_tien(b_lp_dk) *b_kho/ 100,b_tp);
...
if b_so_idG<>0 and b_ngay_hl<b_ngay_cap then
    select ngay_hl,ngay_kt into b_ngay_hlC,b_ngay_ktC from bh_ptnch_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idG and b_so_id_dt in(0,so_id_dt);
    ...
    select ma,tien,pt,phi bulk collect into dk_maG,dk_tienG,dk_ptG,dk_phiG from bh_ptnch_dk
        where ma_dvi=b_ma_dvi and so_id=b_so_idG and b_so_id_dt in(0,so_id_dt) and lh_bh<>'M' order by bt;
    select ma,tien,pt,phi bulk collect into dkbs_maG,dkbs_tienG,dkbs_ptG,dkbs_phiG from bh_ptnch_dk
...
for b_lp in 1..dk_ma.count loop
    ...
    'pp' value dk_pp(b_lp),'ppt' value dk_ppt(b_lp),'ptK' value dk_ptk(b_lp),'bt' value dk_bt(b_lp),
...
for b_lp in 1..dkbs_ma.count loop
    ...
    'pp' value dkbs_pp(b_lp),'ppt' value '','ptK' value dkbs_ptk(b_lp),'bt' value dkbs_bt(b_lp),
...
/
create or replace procedure PBH_PTNCH_HDBS(
...
select nvl(min(so_id_d),0) into b_so_idD from bh_ptnch where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_so_idD<>0 then
    select count(*) into b_dong from bh_ptnch where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
    select JSON_ARRAYAGG(json_object(ngay_ht,so_hd,ttoan,ttrang,ma_dvi,so_id,
           'ma_sdbs' value PKH_TEN_TENl(FBH_PTNCH_TXT(ma_dvi,so_id,'ma_sdbs'))
    ) order by so_id desc returning clob)
        into cs_lke from bh_ptnch where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
end if;
select json_object('dong' value b_dong,'cs_lke' value cs_lke) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
