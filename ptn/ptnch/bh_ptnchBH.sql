create or replace procedure PBH_PTNCHBH_CT(
...
select count(*) into b_i1 from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_hk';
if b_i1=1 then
    select txt into dt_hk from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_hk';
end if;
select txt into ds_ct from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_ct';
select txt into ds_dk from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_dk';
select count(*) into b_i1 from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_dkbs';
if b_i1=1 then
    select txt into ds_dkbs from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_dkbs';
end if;
select count(*) into b_i1 from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_lt';
if b_i1=1 then
    select txt into ds_lt from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_lt';
end if;
select count(*) into b_i1 from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_kbt';
if b_i1=1 then
    select txt into ds_kbt from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_kbt';
end if;
select count(*) into b_i1 from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_ttt';
if b_i1=1 then
    select txt into ds_ttt from bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_ttt';
end if;
...
/
create or replace procedure PBH_PTNCHBH_CTbg(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_lan number;
...
/
create or replace procedure PBH_PTNCHBH_NH_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_so_id number,
    dt_ct in out clob,dt_hk in out clob,ds_ct clob,ds_dk clob,ds_dkbs clob,ds_lt clob,ds_kbt clob,dt_kytt clob,ds_ttt clob,
    ...
    b_nt_tien varchar2,b_nt_phi varchar2,b_phong varchar2,b_lan out number,b_ma_sp varchar2,b_nv varchar2,
    dvi_so_id pht_type.a_num,dvi_ngay_hl pht_type.a_num,dvi_ngay_kt pht_type.a_num,
...
insert into bh_ptnB values(
    b_ma_dvi,b_so_id,b_lan,b_so_hd,b_ngay_ht,b_nv,'H',b_ttrang,b_kieu_hd,b_so_hd_g,
...
for b_lp in 1..dvi_so_id.count loop
    insert into bh_ptnB_dvi values(b_ma_dvi,b_so_id,dvi_so_id(b_lp),b_ten,b_ma_sp,dvi_ngay_hl(b_lp),dvi_ngay_kt(b_lp));
end loop;
PKH_JS_THAYa(dt_ct,'so_hd,ma_kh',b_so_hd||','||b_ma_kh);
insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_ct',dt_ct);
insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_hk',dt_hk);
insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_ct',ds_ct);
insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_dk',ds_dk);
insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_dkbs',ds_dkbs);
if ds_ttt is not null then
  insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_ttt',ds_ttt);
end if;
if ds_lt is not null then
    insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_lt',ds_lt);
end if;
if ds_kbt is not null then
    insert into bh_ptnB_txt values(b_ma_dvi,b_so_id,b_lan,'ds_kbt',ds_kbt);
end if;
...
/
create or replace procedure PBH_PTNCHBH_NH(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(1000); b_lenh varchar2(2000); b_lan number;
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_BG_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,'bh_ptnch',dt_ct,dt_kytt,
...
PBH_PTNCHBH_NH_NH(
    ...
    b_nt_tien,b_nt_phi,b_phong,b_lan,b_ma_sp,b_nv,
    dvi_so_id,dvi_ngay_hl,dvi_ngay_kt,
