create or replace function FBH_PTNB_NHOM(
    b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Nam - Tra nghiep vu
select min(nhom) into b_kq from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_PTNB_NV(
    b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Nam - Tra nghiep vu bao hiem
select min(nv) into b_kq from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_PTNB_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Nam - Tra so hop dong
select min(so_hd) into b_kq from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_PTNB_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2,b_nv varchar2:='',b_nhom varchar2:='') return number
as
    b_kq number;
begin
-- Nam - Tra so id
if nvl(b_nhom,' ')=' ' then 
    select nvl(min(so_id),0) into b_kq from bh_ptnB where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
else 
    select nvl(min(so_id),0) into b_kq from bh_ptnB where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv and nhom=b_nhom;
end if;
return b_kq;
end;
/
create or replace procedure PBH_PTNB_LKE
...
if b_klk='N' then
    select count(*) into b_dong from bh_ptnB where ma_dvi=b_ma_dvi and
        ngay_ht=b_ngay_ht and nv=b_nv and nhom=b_nhom and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_ptnB where
    ...
    select count(*) into b_dong from bh_ptnB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nhom=b_nhom;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_ptnB  where
...
/
create or replace procedure PBH_PTNB_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hd:=FBH_PTNB_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ptnB where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nhom=b_nhom and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
    ...
    select count(*) into b_dong from bh_ptnB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nhom=b_nhom;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ptnB where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nhom=b_nhom order by so_hd)
        where so_hd>=b_so_hd;
    PKH_LKE_VTRI(b_trangKt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,lan,nsd) obj,rownum sott from bh_ptnB  where
...
/
create or replace procedure PBH_PTNB_SO_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_so_id number; b_so_hd varchar2(20); b_nv varchar2(10); b_nhom varchar2(10);
...
b_so_id:=FBH_PTNB_SO_ID(b_ma_dvi,b_so_hd,b_nv,b_nhom);
...
/
create or replace procedure PBH_PTNB_TIM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if trim(b_cmt||b_mobi||b_email) is not null then
    ...
    for r_lp in (select so_id,max(lan) lan from bh_ptnB where ma_dvi=b_ma_dvi and ma_kh=b_ma_kh and nv=b_nv and nhom=b_nhom and
        ...
        insert into ket_qua(c1,c2,c3,n1,n10,n11,n12,n13)
            ...
            from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan;
    end loop;
else
    for r_lp in (select so_id,max(lan) lan from bh_ptnB where ma_dvi=b_ma_dvi and nv=b_nv and nhom=b_nhom and
        ngay_ht between b_ngayD and b_ngayC and phong=b_phong and (b_so_hd = ' ' OR so_hd LIKE '%' || b_so_hd || '%')
        and (b_ten = ' ' OR upper(ten) LIKE '%' || b_ten || '%') and b_ttrang in (' ',ttrang) group by so_id) loop
        b_so_id:=r_lp.so_id; b_lan:=r_lp.lan;
        insert into ket_qua(c1,c2,c3,n1,n10,n11,n12,n13)
...
/
create or replace procedure PBH_PTNB_XOA_XOA(
...
begin
-- Nam - Xoa
b_loi:='loi:Loi xu ly PBH_PTNB_XOA_XOA:loi';
select count(*) into b_i1 from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1>0 then
  select nsd into b_nsdC from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
b_loi:='loi:Loi xoa Table bh_ptnB:loi';
if b_lan = 0 then
    delete bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
else
    delete bh_ptnB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan;
end if;
delete bh_ptnB_ds where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnB_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id;
PBH_BAO_NV_XOA(b_ma_dvi,b_nsd,b_so_id,b_nh,b_loi);
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_PTNB_XOA(
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_PTNB_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',0,b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
commit;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNB_HDBS(
...
if b_so_id<>0 then
    select JSON_ARRAYAGG(json_object( ma_dvi,so_id,lan,'ngay_ht' value FKH_JS_GTRIn(txt,'ngay_ht') , 'ttoan' value FKH_JS_GTRIn(txt,'ttoan'),
                              ...
                              from bh_ptnB_ls where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
end if;
select json_object('dong' value b_dong,'cs_lke' value cs_lke) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNB_CHUYEN_HD(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_id<>0 and b_lan>0 then
    select so_id,max(lan),ttrang into b_so_id,b_i1,b_ttrang from bh_ptnB where ma_dvi=b_ma_dvi and so_id=b_so_id group by so_id,ttrang;
