create or replace function FBH_PTNCC_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return nvarchar2
AS
    b_kq nvarchar2(500):=' '; b_i1 number; b_id number; b_txt clob;
    a_ds_ct pht_type.a_clob;
begin
-- nam - Tra gia tri varchar2 trong txt
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
if b_i1=1 then
...
/
create or replace function FBH_PTNCC_TXTn(b_ma_dvi varchar2,b_so_id number,b_tim varchar2) return number
AS
...
/
create or replace  function FBH_PTNCC_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Nam - Tra so hop dong
select min(so_hd) into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace  function FBH_PTNCC_SO_IDd(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Nam - Tra so id dau
...
/
create or replace function FBH_PTNCC_SO_IDc(b_ma_dvi varchar2,b_so_id number,b_ttrang varchar2:='K') return number
as
    b_kq number:=0; b_so_idD number:=b_so_id;
begin
-- Nam - Tra so id cuoi
b_so_idD:=FBH_PTNCC_SO_IDd(b_ma_dvi,b_so_id);
if b_ttrang<>'C' then
    select nvl(max(so_id),0) into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
else
    select nvl(max(so_id),0) into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D';
end if;
return b_kq;
end;
/
create or replace function FBH_PTNCC_KIEU_HDd(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(1); b_so_idD number:=FBH_PTNCC_SO_IDd(b_ma_dvi,b_so_id);
begin
-- Nam - Tra so hop dong
select nvl(min(kieu_hd),' ') into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_id=b_so_idD;
return b_kq;
end;
/
create or replace function FBH_PTNCC_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2,b_nv varchar2:='') return number
as
    b_kq number;
begin
-- Nam - Tra so id
if nvl(b_nv,' ')=' ' then 
    ...
    select nvl(min(so_id),0) into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv;
end if;
return b_kq;
end;
/
create or replace function FBH_PTNCC_SO_IDt(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return number
as
    b_kq number:=0; b_so_idD number;
begin
-- Nam - Tra so id 
b_so_idD:=FBH_PTNCC_SO_ID(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang in('T','D') and ngay_ht<=b_ngay;
return b_kq;
end;
/
create or replace function FBH_PTNCC_SO_IDb(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return number
as
    b_kq number:=0; b_so_idD number;
begin
-- Nam - Tra so id 
b_so_idD:=FBH_PTNCC_SO_ID(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_ptncc where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D' and ngay_ht<=b_ngay;
return b_kq;
end;
/
create or replace procedure PBH_PTNCC_SO_ID(
...
b_so_id:=FBH_PTNCC_SO_ID(b_ma_dvi,b_so_hd,b_nv);
...
/
create or replace procedure FBH_PTNCC_BPHI_DKm(
...
b_loi:='loi:Loi xu ly FBH_PTNCC_BPHI_DKm:loi';
select count(*),nvl(max(tien),0) into b_i1,b_tienM from bh_ptncc_phi_dk where
...
select nvl(min(bt),0) into b_bt from bh_ptncc_phi_dk
    where so_id=b_so_id and ma=b_ma and lh_bh=b_nv and tien=b_tienM;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_PTNCC_BPHI_DKm(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    ...
    b_nv varchar2(1); b_nhom varchar2(1); b_ma_sp varchar2(10); b_cdich varchar2(10); b_lvuc varchar2(500); b_ghan varchar2(1); b_ngay_hl number;
begin
-- Nam - Lay %phi theo khoang muc trach nhiem
delete temp_1; commit;
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('nhom,ma_sp,cdich,lvuc,ghan_m,ngay_hl,nv,ma,tien');
EXECUTE IMMEDIATE b_lenh into b_nhom,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_hl,b_nv,b_ma,b_tien using b_oraIn;
b_ma_sp:=NVL(trim(b_ma_sp),' '); b_cdich:=NVL(trim(b_cdich),' '); b_lvuc:=PKH_MA_TENl(b_lvuc);
...
FBH_PTNCC_BPHI_DKm(b_so_id,b_ma,b_tien,b_nv,b_bt,b_loi);
for r_lp in (select * from bh_ptncc_phi_dk where so_id=b_so_id and bt>=b_bt order by bt) loop
...
/
create or replace procedure PBH_PTNCC_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
    select count(*) into b_dong from bh_ptncc where ma_dvi=b_ma_dvi and nv=b_nv and
        ngay_ht=b_ngay_ht and nsd=b_nsd;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_ptncc where
    ...
    select count(*) into b_dong from bh_ptncc where ma_dvi=b_ma_dvi and nv=b_nv and ngay_ht=b_ngay_ht;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_ptncc  where
...
/
create or replace procedure PBH_PTNCC_LKE_ID
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hd:=FBH_PTNCC_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ptncc where
            ma_dvi=b_ma_dvi and nv=b_nv and ngay_ht=b_ngay_ht and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
    ...
    select count(*) into b_dong from bh_ptncc where ma_dvi=b_ma_dvi and
        ngay_ht=b_ngay_ht ;
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_ptncc where
            ma_dvi=b_ma_dvi and nv=b_nv and ngay_ht=b_ngay_ht   order by so_hd)
...
/
create or replace procedure PBH_PTNCC_XOA_XOA(
...
begin
-- Nam - Xoa
b_loi:='loi:Loi xu ly PBH_PTNCC_XOA_XOA:loi';
select count(*) into b_i1 from bh_ptncc where so_id=b_so_id;
if b_i1=0 then b_loi:=''; return; end if;
select so_id_kt,ttrang,ksoat,nsd into b_so_idK,b_ttrang,b_ksoat,b_nsdC
    from bh_ptncc where so_id=b_so_id;
...
b_loi:='loi:Loi xoa Table bh_ptncc:loi';
delete bh_hd_goc_ttindt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptncc_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptncc_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptncc_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_ptncc where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_PTNCC_XOA(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_comm varchar2:='C')
AS
...
PBH_PTNCC_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_PHIGr(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_idG<>0 and b_ngay_hl<b_ngay_cap then
    select ngay_hl,ngay_kt into b_ngay_hlC,b_ngay_ktC from bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idG and b_so_id_dt in(0,so_id_dt);
    ...
    select ma,tien,pt,phi bulk collect into dk_maG,dk_tienG,dk_ptG,dk_phiG from bh_ptncc_dk
        where ma_dvi=b_ma_dvi and so_id=b_so_idG and b_so_id_dt in(0,so_id_dt) and lh_bh<>'M' order by bt;
    select ma,tien,pt,phi bulk collect into dkbs_maG,dkbs_tienG,dkbs_ptG,dkbs_phiG from bh_ptncc_dk
...
/
create or replace procedure PBH_PTNCC_HDBS(
...
select nvl(min(so_id_d),0) into b_so_idD from bh_ptncc where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_so_idD<>0 then
    select count(*) into b_dong from bh_ptncc where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
    select JSON_ARRAYAGG(json_object(ngay_ht,so_hd,ttoan,ttrang,ma_dvi,so_id,
           'ma_sdbs' value PKH_TEN_TENl(FBH_PTNCC_TXT(ma_dvi,so_id,'ma_sdbs'))
    ) order by so_id desc returning clob)
        into cs_lke from bh_ptncc where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
end if;
select json_object('dong' value b_dong,'cs_lke' value cs_lke) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
