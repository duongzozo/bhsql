create or replace procedure PBH_PTNCCH_MO(
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_ptncc_sp a,(select distinct ma_sp from bh_ptncc_phi where nhom='H' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
  where a.ma=b.ma_sp and FBH_PTNCC_SP_HAN(a.ma)='C';
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_cdich from
    bh_ma_cdich a,(select distinct cdich from bh_ptncc_phi where nhom='H' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
/
create or replace procedure PBH_PTNCCH_MOd(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraOut out clob)
AS
    b_loi varchar2(1000); b_ngay number:=PKH_NG_CSO(sysdate);
...
/
create or replace procedure PBH_PTNCCH_CT(
...
select json_object(so_hd,ma_kh) into dt_ct from bh_ptncc where ma_dvi=b_ma_dvi and so_id=b_so_id;
select JSON_ARRAYAGG(json_object(ngay,tien) order by ngay) into dt_kytt from bh_ptncc_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hk';
if b_i1=1 then
    select txt into dt_hk from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hk';
end if;
select txt into ds_ct from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ct';
select txt into ds_dk from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dk';
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
if b_i1=1 then
    select txt into ds_dkbs from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_dkbs';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
if b_i1=1 then
    select txt into ds_lt from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_lt';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
if b_i1=1 then
    select txt into ds_kbt from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_kbt';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
if b_i1=1 then
    select txt into ds_ttt from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='ds_ttt';
end if;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
...
/
create or replace procedure PBH_PTNCCH_NH_NH(
    ...
    b_ma_sp varchar2,b_cdich varchar2,
    dvi_so_id pht_type.a_num, dvi_kieu_gcn pht_type.a_var, dvi_gcn pht_type.a_var, dvi_gcnG pht_type.a_var,
    dvi_lvuc pht_type.a_var, dvi_dtuong pht_type.a_nvar, dvi_ngay_hoi pht_type.a_num,
...
begin
-- Nam - Nhap
b_loi:='loi:Loi Table bh_ptncc:loi';
...
for b_lp in 1..dk_lh_nv.count loop
    ...
    insert into bh_ptncc_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),b_lp,
...
for b_lp in 1..dvi_so_id.count loop
  insert into bh_ptncc_dvi values(
        b_ma_dvi,b_so_id,dvi_so_id(b_lp),b_lp,dvi_kieu_gcn(b_lp),dvi_gcn(b_lp),dvi_gcnG(b_lp),dvi_lvuc(b_lp),
...
insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_ct',dt_ct);
insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'ds_ct',ds_ct);
insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'ds_dk',ds_dk);
if ds_dkbs is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'ds_dkbs',ds_dkbs);
end if;
if dt_hk is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_hk',dt_hk);
end if;
if dt_ttt is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if ds_lt is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'ds_lt',ds_lt);
end if;
if ds_kbt is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'ds_kbt',ds_kbt);
end if;
for b_lp in 1..tt_ngay.count loop
    insert into bh_ptncc_tt values(b_ma_dvi,b_so_id,tt_ngay(b_lp),tt_tien(b_lp));
end loop;
-- Di tiep
if b_kieu_hd<>'U' and b_ttrang in('T','D') then
  ...
  insert into bh_ptn values(
        b_ma_dvi,b_so_id,b_so_hd,b_ngay_ht,'TNCC','H',b_ttrang,b_kieu_hd,b_so_hd_g,b_kieu_kt,b_ma_kt,b_kieu_gt,b_ma_gt,b_ma_cb,
...
/
create or replace procedure PBH_PTNCCH_NH(
...
if b_so_id<>0 then
    ...
    if b_i1<>0 then
        select ngay_ht,kieu_hd,ttrang into b_ngay_htC,b_kieu_hd,b_ttrang from bh_ptncc
        ...
        PBH_PTNCC_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,b_kieu_hd,b_loi);
        if b_loi is not null then raise PROGRAM_ERROR; end if;
    end if;
else
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
PBH_HD_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,b_ngay_htC,'bh_ptncc',dt_ct,dt_kytt,
...
PBH_PTNCCH_TESTr(
    ...
    b_ma_sp,b_cdich,
    dvi_so_id,dvi_kieu_gcn,dvi_gcn,dvi_gcnG,dvi_lvuc,
...
/
create or replace procedure PBH_PTNCCH_TESTr(
    ...
    b_ma_sp out varchar2,b_cdich out varchar2,
    
    dvi_so_id out pht_type.a_num, dvi_kieu_gcn out pht_type.a_var, dvi_gcn out pht_type.a_var, dvi_gcnG out pht_type.a_var, 
    dvi_lvuc out pht_type.a_var, dvi_dtuong out pht_type.a_nvar, dvi_ngay_hoi out pht_type.a_num, 
...
AS
    ...
    b_lvuc varchar2(250); b_dtuong nvarchar2(500); b_ngay_hoi number; b_gio_hl varchar2(50); 
...
b_lenh:=FKH_JS_LENH('ttrang,kieu_hd,gio_hl,ngay_hl,gio_kt,ngay_kt,ngay_cap,
    ma_sp,cdich,thue,ttoan,nt_tien,nt_phi,tygia,c_thue,loai_khh,cmth,mobih,emailh,tenh,dchih');
EXECUTE IMMEDIATE b_lenh into b_ttrang,b_kieu_hd,b_gio_hlH,b_ngay_hlH,b_gio_ktH,b_ngay_ktH,b_ngay_capH,b_ma_sp,b_cdich,
    b_thueH,b_ttoanH,b_nt_tien,b_nt_phi,b_tygia,b_c_thue,b_loai_khH,b_cmtH,b_mobiH,b_emailH,b_tenH,b_dchiH using dt_ct;
if b_ma_sp<>' ' and FBH_PTNCC_SP_HAN(b_ma_sp)<>'C' then b_loi:='loi:Sai ma san pham:loi'; return; end if;
...
for ds_lp in 1..a_ds_ct.count loop
    ...
    dt_ct,a_ds_ct(ds_lp),a_ds_dk(ds_lp),a_ds_dkbs(ds_lp),
        ...
        b_so_id_dt,b_kieu_gcn,b_gcn,b_gcnG,b_lvuc,b_dtuong,
    ...
    dvi_lvuc(ds_lp):=b_lvuc; dvi_dtuong(ds_lp):=b_dtuong; dvi_ngay_hoi(ds_lp):=b_ngay_hoi; dvi_ngay_hl(ds_lp):=b_ngay_hl;
...
if b_kieu_hd in('B','S') and b_so_idG<>0 then
    for r_lp in (select so_id_dt,dtuong from bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id=b_so_idG) loop
...
/
create or replace procedure PBH_PTNCCH_TESTd(
    ...
    b_lvuc out varchar2,b_dtuong out nvarchar2,
...
begin
-- Nam - Nhap
b_loi:='loi:Loi xu ly PBH_PTNCCH_TESTd:loi';
b_lenh:=FKH_JS_LENH('so_id_dt,gcn,gcn_g,ngay_hl,ngay_kt,ngay_hoi,ghan_m,gio_hl,gio_kt,lvuc,dtuong,so_idp,cdtc,cdtf,cdtx');
EXECUTE IMMEDIATE b_lenh into
    b_so_id_dt,b_gcn,b_gcnG,b_ngay_hl,b_ngay_kt,b_ngay_hoi,b_ghan_m,b_gio_hl,b_gio_kt,b_lvuc,b_dtuong,b_so_idPn,b_cdtC,b_cdtF,b_cdtX using dt_ct;
...
if b_so_id_dt<100000 then
    ...
    select count(*) into b_i1 from bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcnG;
...
if b_gcn=' ' or instr(b_gcn,'.')=2 then
    ...
    if b_kieu_gcn<>'G' then
        select count(*) into b_i1 from bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        if(b_i1>0) then
           select max(REGEXP_SUBSTR(gcn, 'B([0-9]+)', 1, 1, NULL, 1)) into b_i1 from bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and kieu_gcn=b_kieu_hd;
        end if;
        b_gcn:=b_gcn||'/'||b_kieu_hd||to_char(b_i1+1);
    end if;
else
    select nvl(max(ngay_cap),b_ngay_capH) into b_ngay_cap from bh_ptncc_dvi where ma_dvi=b_ma_dvi and so_id_dt=b_so_id_dt and gcn=b_gcn;
end if;
...
b_so_idP:=FBH_PTNCC_BPHI_SO_IDh('H',b_ma_sp,b_cdich,b_lvuc,b_ghan_m,b_ngay_hl);
