create or replace procedure PBH_PTNCCG_MO(
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_ptncc_sp a,(select distinct ma_sp from bh_ptncc_phi where nhom='G' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_cdich from
    bh_ma_cdich a,(select distinct cdich from bh_ptncc_phi where nhom='G' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_lvuc from
    bh_ma_lvuc a,(select distinct lvuc from bh_ptncc_phi where nhom='G' and ngay_bd<=b_ngay and ngay_kt>b_ngay) b
...
/
create or replace procedure PBH_PTNCCG_CT(
...
select JSON_ARRAYAGG(json_object(ma,ten) order by bt returning clob) into dt_dk
    from bh_ptncc_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_bh<>'M';
select JSON_ARRAYAGG(json_object(ma,ten) order by bt returning clob) into dt_dkbs
    from bh_ptncc_dk where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_bh='M';
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_lt';
if b_i1<>0 then
    select txt into dt_lt from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_lt';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_khd';
if b_i1<>0 then
    select txt into dt_khd from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_khd';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_kbt';
if b_i1<>0 then
    select txt into dt_kbt from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_kbt';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hu';
if b_i1<>0 then
    select txt into dt_hu from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_hu';
end if;
select count(*) into b_i1 from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_ptncc_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai in('dt_ct','dt_dk','dt_dkbs');
...
/
create or replace procedure PBH_PTNCCG_NH_NH(
    ...
    b_so_idP number,b_ma_sp varchar2,b_cdich varchar2,b_lvuc varchar2,b_dtuong nvarchar2,
    b_dthu number, b_ngay_hoi number,
...
for b_lp in 1..dk_lh_nv.count loop
    ...
    insert into bh_ptncc_dk values(b_ma_dvi,b_so_id,b_so_id,b_lp,
...
insert into bh_ptncc values(
        b_ma_dvi,b_so_id,b_so_hd,b_ngay_ht,'G',b_ttrang,b_kieu_hd,b_so_hd_g,b_kieu_kt,b_ma_kt,b_kieu_gt,
...
insert into bh_ptncc_dvi values(
        b_ma_dvi,b_so_id,b_so_id,0,b_kieu_hd,b_so_hd,b_so_hd_g,b_lvuc,b_dtuong,b_gio_hl,b_ngay_hl,b_gio_kt,b_ngay_kt,b_ngay_cap,b_ngay_hoi,b_so_idP,b_phi,b_thue,b_ttoan);
...
insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if dt_dkbs is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_dkbs',dt_dkbs);
end if;
if dt_ttt is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if dt_lt is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_lt',dt_lt);
end if;
if dt_kbt is not null then
    insert into bh_ptncc_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
...
if b_ttrang in('T','D') then
    ...
    insert into bh_ptncc_kbt values(b_ma_dvi,b_so_id,b_so_id,b_txt,dt_lt,dt_kbt);
    insert into bh_ptn values(
          b_ma_dvi,b_so_id,b_so_hd,b_ngay_ht,'TNCC','G',b_ttrang,b_kieu_hd,b_so_hd_g,b_kieu_kt,b_ma_kt,b_kieu_gt,b_ma_gt,b_ma_cb,
...
/
create or replace procedure PBH_PTNCCG_NH(
...
/
create or replace procedure PBH_PTNCCG_TESTr(
...
begin
-- Nam - Nhap
b_lenh:=FKH_JS_LENH('ttrang,loai_kh,ma_sp,cdich,lvuc,ghan_m,dtuong,dthu,thue,ttoan,nt_tien,nt_phi,tygia,c_thue,ngay_hl,ngay_kt,ngay_hoi');
EXECUTE IMMEDIATE b_lenh into b_ttrang,b_loai_khM,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_dtuong,b_dthu,
...
b_so_idP:=FBH_PTNCC_BPHI_SO_IDh('G',b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_hl);
