/* Ma san pham */
create or replace function FBH_PTNCC_SP_HAN(b_ma varchar2,b_dk varchar2:='C') return varchar2
AS
    b_kq varchar2(1):='K'; b_i1 number; b_ngay number:=PKH_NG_CSO(sysdate);
begin
-- Nam - Kiem tra con hieu luc
select count(*) into b_i1 from bh_ptncc_sp where ma=b_ma and tc in('C',b_dk) and b_ngay between ngay_bd and ngay_kt;
if b_i1<>0 then b_kq:='C'; end if;
return b_kq;
end;
/
create or replace procedure PBH_PTNCC_SP_LKE(
...
if b_tim is null then
    select count(*) into b_dong from bh_ptncc_sp;
    ...
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select json_object(*) obj,rownum sott from
            ...
            (select * from bh_ptncc_sp order by ma) a
            start with ma_ct=' ' CONNECT BY prior ma=ma_ct))
        where sott between b_tu and b_den;
else
    select count(*) into b_dong from bh_ptncc_sp where upper(ten) like b_tim;
    PKH_LKE_TRANG(b_dong,b_tu,b_den);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select obj,rownum sott from
            (select ma,ten,json_object(a.*,'xep' value ma) obj from bh_ptncc_sp a)
...
/
create or replace procedure PBH_PTNCC_SP_MA(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_tim is null then
    ...
    select nvl(min(sott),0) into b_tu from (select ma,ma_ct,rownum sott from
        (select * from bh_ptncc_sp order by ma) a start with ma_ct=' ' CONNECT BY prior ma=ma_ct)
        where ma=b_ma;
    if b_tu=0 then
    ...
    select nvl(min(sott),b_dong) into b_tu from (
        select ma,rownum sott from bh_ptncc_sp where upper(ten) like b_tim order by ma)
        where ma>=b_ma;
    PKH_LKE_VTRI(b_hangkt,b_tu,b_den,b_trang);
    select JSON_ARRAYAGG(obj) into cs_lke from
        (select obj,rownum sott from
...
/
create or replace procedure PBH_PTNCC_SP_CT(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','MA','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
select json_object(ma,txt) into cs_ct from bh_ptncc_sp where ma=b_ma;
select json_object('cs_ct' value cs_ct) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_SP_NH
...
if trim(b_ma_ct) is null then
    ...
    select 0 into b_i1 from bh_ptncc_sp where ma=b_ma_ct and tc='T';
end if;
if b_ngay_bd in(0,30000101) then b_ngay_bd:=PKH_NG_CSO(sysdate); end if;
if b_ngay_kt=0 then b_ngay_kt:=30000101; end if;
b_loi:='';
delete bh_ptncc_sp where ma=b_ma;
insert into bh_ptncc_sp values(b_ma_dvi,b_ma,b_ten,b_tc,b_ma_ct,b_ngay_bd,b_ngay_kt,b_nsd,b_oraIn);
commit;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_SP_XOA
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob)
AS
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_ma is null then b_loi:='loi:Nhap ma:loi'; raise PROGRAM_ERROR; end if;
select count(*) into b_i1 from bh_ptncc_sp where ma_ct=b_ma;
if b_i1<>0 then
...
/
create or replace function FBH_PTNCC_SP_TEN(b_ma varchar2) return nvarchar2
AS
    b_kq nvarchar2(200);
begin
-- Nam
select min(ten) into b_kq from bh_ptncc_sp where ma=b_ma;
return b_kq;
end;
/
/* bieu phi trach nhiem */
create or replace function FBH_PTNCC_BPHI_SO_ID
...
select max(lvuc) into b_lvucM from bh_ptncc_phi where ma_sp=b_ma_sp and cdich=b_cdich and lvuc=b_lvuc and b_ngay between ngay_bd and ngay_kt;
if b_lvucM is not null then
    select nvl(max(so_id),0) into b_so_id from bh_ptncc_phi where 
    ma_sp=b_ma_sp and cdich=b_cdich and lvuc=b_lvuc and b_ngay between ngay_bd and ngay_kt;
end if;
return b_so_id;
end;
/
create or replace function FBH_PTNCC_BPHI_SO_IDh(
    b_nhom varchar2,b_ma_sp varchar2,b_cdich varchar2,b_lvuc varchar2,b_ghan varchar2,b_ngay_hl number:=0) return number
AS
    b_so_id number:=0; b_lvucM varchar2(10); b_ngay number:=b_ngay_hl;
begin
-- Nam - Tra so ID con han su dung
if b_ngay in(0,30000101) then b_ngay:=PKH_NG_CSO(sysdate); end if;
select max(lvuc) into b_lvucM from bh_ptncc_phi where 
       nhom=b_nhom and ma_sp=b_ma_sp and cdich in (' ',b_cdich) and lvuc in (' ',b_lvuc) and ghan in (' ',b_ghan) and b_ngay between ngay_bd and ngay_kt;
if b_lvucM is not null then
    select nvl(max(so_id),0) into b_so_id from bh_ptncc_phi where 
           nhom=b_nhom and ma_sp=b_ma_sp and cdich in (' ',b_cdich) and  lvuc in (' ',b_lvuc) and ghan in (' ',b_ghan) and b_ngay between ngay_bd and ngay_kt;
end if;
...
/
create or replace procedure PBH_PTNCC_BPHI_MO(
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','MA','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
select JSON_ARRAYAGG(json_object(ma,ten returning clob) order by ten returning clob) into cs_sp from bh_ptncc_sp where FBH_PTNCC_SP_HAN(ma)='C' and ma<>'HANG';  
...
/
create or replace procedure PBH_PTNCC_BPHI_SO_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if trim(b_ma_sp) is null or trim(b_lvuc) is null then
    ...
    b_cdich:=nvl(b_cdich,' ');
    b_so_id:=FBH_PTNCC_BPHI_SO_ID(b_ma_sp,b_cdich,b_lvuc);
end if;
select json_object('so_id' value b_so_id) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_BPHI_LKE
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
    b_ma_sp varchar2(500); b_lvuc varchar2(500);
    b_tu number; b_den number; b_dong number; cs_lke clob;
begin
-- Nam
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('ma_sp,lvuc,tu,den');
EXECUTE IMMEDIATE b_lenh into b_ma_sp,b_lvuc,b_tu,b_den using b_oraIn;
b_ma_sp:=PKH_MA_TENl(b_ma_sp); b_lvuc:=PKH_MA_TENl(b_lvuc);
b_ma_sp:=nvl(b_ma_sp,' '); b_lvuc:=nvl(b_lvuc,' '); 
select count(*) into b_dong from bh_ptncc_phi where b_ma_sp in(' ',ma_sp) and b_lvuc in(' ',lvuc);
PKH_LKE_TRANG(b_dong,b_tu,b_den);
select JSON_ARRAYAGG(json_object(so_id,nhom,'ma_sp' value FBH_PTNCC_SP_TEN(ma_sp),'cdich' value FBH_MA_CDICH_TEN(cdich),
       'lvuc' value FBH_MA_LVUC_TEN(lvuc),tlph,ghan,ngay_bd,ngay_kt) returning clob) into cs_lke from
    (select so_id,nhom,ma_sp,cdich,lvuc,tlph,ghan,ngay_bd,ngay_kt,rownum sott from bh_ptncc_phi
    where b_ma_sp in(' ',ma_sp) and b_lvuc in(' ',lvuc)
    order by ma_sp,lvuc,nhom,ngay_bd)
    where sott between b_tu and b_den;
select json_object('dong' value b_dong,'cs_lke' value cs_lke returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_BPHI_MA
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000);
...
select count(*) into b_dong from bh_ptncc_phi;
select nvl(min(sott),b_dong) into b_tu from
    (select a.*,rownum sott from bh_ptncc_phi a order by ma_sp,cdich,lvuc,tlph)
...
select JSON_ARRAYAGG(obj returning clob) into cs_lke from
    (select json_object(a.*) obj,rownum sott from bh_ptncc_phi a order by ma_sp,cdich,lvuc,tlph)
    where sott between b_tu and b_den;
select json_object('trang' value b_trang,'dong' value b_dong,'cs_lke' value cs_lke returning clob) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_BPHI_CT
    (b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
...
select JSON_ARRAYAGG(json_object(so_id,nsd)) into dt_ct from bh_ptncc_phi where so_id=b_so_id;
select txt into dt_dk from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_dk';
select count(*) into b_i1 from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_dkbs';
if b_i1<>0 then
    select txt into dt_dkbs from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_dkbs';
end if;
select JSON_ARRAYAGG(json_object(ma_lt) order by bt) into dt_lt from bh_ptncc_phi_lt where so_id=b_so_id;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt from bh_ptncc_phi_txt where so_id=b_so_id and loai in('dt_ct','dt_lt');
select count(*) into b_i1 from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_khd';
if b_i1<>0 then
    select txt into dt_khd from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_khd';
end if;
select count(*) into b_i1 from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_kbt';
if b_i1<>0 then
    select txt into dt_kbt from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_kbt';
end if;
...
/
create or replace procedure PBH_PTNCC_BPHI_CTs(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(1000); b_lenh varchar2(1000); dt_ct clob:=FKH_JS_GTRIc(b_oraIn,'dt_ct');
    b_nhom varchar2(1); b_lvuc varchar2(500); b_ma_sp varchar2(20); b_cdich varchar2(20); 
    b_ghan varchar2(1); b_ngay_hl number; b_so_id number;
begin
-- Nam - Tra so_id bieu phi theo dieu kien
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('nhom,ma_sp,cdich,lvuc,ghan_m,ngay_hl');
EXECUTE IMMEDIATE b_lenh into b_nhom,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_hl using dt_ct;
b_lvuc:=PKH_MA_TENl(b_lvuc);
b_cdich:=nvl(b_cdich,' '); b_ma_sp:=nvl(b_ma_sp,' '); b_ghan:=nvl(b_ghan,' ');
b_so_id:=FBH_PTNCC_BPHI_SO_IDh(b_nhom,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_hl);
select json_object('so_id' value b_so_id) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_PTNCC_BPHI_CTd(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
insert into temp_1(c1,n1) select ma,min(bt) from bh_ptncc_phi_dk where so_id=b_so_id and lh_bh<>'M' group by ma;
select JSON_ARRAYAGG(json_object(
    ...
    from bh_ptncc_phi_dk a,temp_1 b where a.so_id=b_so_id and b.n1=a.bt and lh_bh<>'M' order by a.bt; 
insert into temp_1(c1,n1) select ma,min(bt) from bh_ptncc_phi_dk where so_id=b_so_id and lh_bh='M' group by ma;
select JSON_ARRAYAGG(json_object(
    a.ma,ten,
    'tien' value case when b_nt_tien<>'VND' then round(tien/b_tygia,2) else tien end,
...
select JSON_ARRAYAGG(json_object('ma_lt' value ma_lt) order by bt returning clob)
    into cs_lt from bh_ptncc_phi_lt where so_id=b_so_id;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into cs_txt
    from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_lt';
select count(*) into b_i1 from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_khd';
if b_i1<>0 then
    select txt into dt_khd from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_khd';
end if;
select count(*) into b_i1 from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_kbt';
if b_i1<>0 then
    select txt into cs_kbt from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_kbt';
end if;
select count(tlph) into b_i1 from bh_ptncc_phi where so_id=b_so_id;
if b_i1<>0 then
    select tlph into b_tlph from bh_ptncc_phi where so_id=b_so_id;
end if;
...
b_lenh:=FKH_JS_LENH('nhom,ma_sp,cdich,lvuc,tlph,ghan,ngay_bd,ngay_kt');
EXECUTE IMMEDIATE b_lenh into b_nhom,b_ma_sp,b_cdich,b_lvuc,b_tl_phi,b_ghan,b_ngay_bd,b_ngay_kt using b_dt_ct;
if trim(b_ma_sp) is null then
    b_loi:='loi:Nhap san pham:loi'; raise PROGRAM_ERROR;
elsif FBH_PTNCC_SP_HAN(b_ma_sp)<>'C' then
...
b_lvuc:=PKH_MA_TENl(b_lvuc);
if b_lvuc<>' ' then
    if FBH_MA_LVUC_HAN(b_lvuc)<>'C' then
    b_loi:='loi:Ma linh vuc '||b_lvuc||' da het su dung:loi'; raise PROGRAM_ERROR;
    end if;
end if;
b_ma_sp:=nvl(b_ma_sp,' '); b_cdich:=nvl(b_cdich,' '); b_tl_phi:=nvl(b_tl_phi,0);
...
b_so_id:=FBH_PTNCC_BPHI_SO_IDh(b_nhom,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_bd);
if b_so_id<>0 then
    delete bh_ptncc_phi_txt where so_id=b_so_id;
    delete bh_ptncc_phi_lt where so_id=b_so_id;
    delete bh_ptncc_phi_dk where so_id=b_so_id;
    delete bh_ptncc_phi where so_id=b_so_id;
end if;
if b_so_id=0 then
    PHT_ID_MOI(b_so_id,b_loi);
    if b_loi is not null then raise PROGRAM_ERROR; end if;
end if;
b_loi:='loi:Loi Table bh_ptncc_phi:loi';
insert into bh_ptncc_phi values(b_ma_dvi,b_so_id,b_nhom,b_ma_sp,b_lvuc,b_cdich,b_tl_phi,b_ghan,b_ngay_bd,b_ngay_kt,b_nsd);
insert into bh_ptncc_phi_txt values(b_ma_dvi,b_so_id,'dt_ct',b_dt_ct);
insert into bh_ptncc_phi_txt values(b_ma_dvi,b_so_id,'dt_dk',b_dt_dk);
if trim(b_dt_dkbs) is not null then
    insert into bh_ptncc_phi_txt values(b_ma_dvi,b_so_id,'dt_dkbs',b_dt_dkbs);
end if;
for b_lp in 1..dk_ma.count loop
   insert into bh_ptncc_phi_dk values(b_ma_dvi,b_so_id,b_lp,dk_ma(b_lp),dk_ten(b_lp),dk_tc(b_lp),dk_ma_ct(b_lp),
...
for r_lp in(select ma,ma_ct,so_id,bt,level from  (select t.so_id,t.bt,t.ma,t.ma_ct from bh_ptncc_phi_dk t where t.so_id = b_so_id) t start with t.ma_ct=' '  CONNECT BY prior t.ma=t.ma_ct) loop
    update bh_ptncc_phi_dk set cap=r_lp.level where so_id=r_lp.so_id and bt=r_lp.bt;
end loop;
if lt_ma_lt.count<>0 then
    for b_lp in 1..lt_ma_lt.count loop
        insert into bh_ptncc_phi_lt values(b_ma_dvi,b_so_id,b_lp,lt_ma_lt(b_lp),lt_ten(b_lp));
    end loop;
    insert into bh_ptncc_phi_txt values(b_ma_dvi,b_so_id,'dt_lt',b_dt_lt);
end if;
if trim(b_dt_khd) is not null then
    insert into bh_ptncc_phi_txt values(b_ma_dvi,b_so_id,'dt_khd',b_dt_khd);
end if;
if trim(b_dt_kbt) is not null then
    insert into bh_ptncc_phi_txt values(b_ma_dvi,b_so_id,'dt_kbt',b_dt_kbt);
end if;
commit;
select json_object('so_id' value b_so_id) into b_oraOut from dual;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
...
/
create or replace procedure PBH_PTNCC_BPHI_XOA(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS 
...
/
create or replace procedure PBH_PTNCC_BPHId_THEM(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(1000); b_lenh varchar2(1000); b_i1 number;
    b_lvuc varchar2(500);b_nhom varchar2(1); b_ma_sp varchar2(20); b_cdich varchar2(20); b_ngay_hl number; b_so_id number; b_vu varchar2(10);
...
b_loi:=FHT_MA_NSD_KTRA(b_ma_dvi,b_nsd,b_pas,'BH','PTN','');
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_lenh:=FKH_JS_LENH('nhom,ma_sp,cdich,lvuc,ghan_m,ngay_hl,vu,nt_tien,nt_phi,tygia');
EXECUTE IMMEDIATE b_lenh into b_nhom,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_hl,b_vu,b_nt_tien,b_nt_phi,b_tygia using b_oraIn;
b_cdich:=nvl(b_cdich,' '); b_lvuc:=PKH_MA_TENl(b_lvuc); b_ghan:=nvl(b_ghan,' ');
b_so_id:=FBH_PTNCC_BPHI_SO_IDh(b_nhom,b_ma_sp,b_cdich,b_lvuc,b_ghan,b_ngay_hl);
...
if b_vu='dkbs' then
    select JSON_ARRAYAGG(json_object(ma,ten,'tien' value case when b_nt_tien='VND' then tien else round(tien/b_tygia,2) end,
        ...
        (select ma,ten,tien,pt,ma_ct,tc,phi,cap,lh_nv,luy,ktru,lkeM,lkeP,bt
                from bh_ptncc_phi_dk where so_id=b_so_id and lh_bh='M' union
    ...
    if b_vu='dk' then
        select JSON_ARRAYAGG(json_object(ma,ten,'tien' value case when b_nt_tien='VND' then tien else round(tien/b_tygia,2) end,
            ...
            into b_oraOut from bh_ptncc_phi_dk where so_id=b_so_id and lh_bh<>'M' order by bt;
    elsif b_vu='lt' then
      select count(*) into b_i1 from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_lt';
        if b_i1 >0 then
             select txt into b_dk_lt from bh_ptncc_phi_txt where so_id=b_so_id and loai='dt_lt';
