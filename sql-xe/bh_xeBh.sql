create or replace procedure PBH_XEBH_CT(
...
select max(lan) into b_lan from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_lan > 0 then
...
/
create or replace procedure PBH_XEBH_NH_NH(
...
for b_lp in 1..ds_so_id.count loop
    ...
    insert into bh_xeB_ds values(b_ma_dvi,b_so_id,ds_so_id(b_lp),b_bien,ds_loai_xe(b_lp),ds_nam_sx(b_lp),ds_md_sd(b_lp),ds_ma_sp(b_lp),ds_loai_xe(b_lp));
end loop;
for b_lp in 1..dk_so_id.count loop
    if dk_lh_nv(b_lp)<>' ' then
        if dk_kieu(b_lp)='T' then b_tien:=b_tien+dk_tien(b_lp); end if;
        insert into bh_xeB_dk values(b_ma_dvi,b_so_id,dk_so_id(b_lp),dk_ma(b_lp),dk_ten(b_lp),
...
insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_ds',dt_ds);
...
insert into bh_xeB_ls(ma_dvi,so_id,lan,loai,txt) select ma_dvi,so_id,lan,loai,txt
    from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan;
...
/
create or replace procedure PBH_XEBH_NH(
