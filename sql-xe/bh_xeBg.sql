create or replace procedure PBH_XEBG_CT(
...
select nvl(max(lan),0) into b_lan from bh_xeB where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_lan=0 then b_loi:='loi:Bao gia da xoa:loi'; raise PROGRAM_ERROR; end if;
select json_object(
    'nhom_xe' value FBH_XE_NHOM_TENl(FKH_JS_GTRIs(FKH_JS_BONH(txt),'nhom_xe')),'loai_xe' value FBH_XE_LOAI_TENl(FKH_JS_GTRIs(FKH_JS_BONH(txt),'loai_xe')),
    'hang' value FBH_XE_HANG_TENl(FKH_JS_GTRIs(FKH_JS_BONH(txt),'hang')),'hieu' value FBH_XE_HIEU_TENl(FKH_JS_GTRIs(FKH_JS_BONH(txt),'hieu')),
    'pban' value FBH_XE_PB_TENl(FKH_JS_GTRIs(FKH_JS_BONH(txt),'hang'),FKH_JS_GTRIs(FKH_JS_BONH(txt),'hieu'),FKH_JS_GTRIs(FKH_JS_BONH(txt),'pban')),
    'dong' value FBH_XE_DONG_TENl(FKH_JS_GTRIs(FKH_JS_BONH(txt),'dong'))returning clob) 
    into dt_ct from bh_xeB_txt where so_id=b_so_id and lan=b_lan and loai='dt_ct';
select txt into dt_dk from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dk';
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dkbs';
if b_i1<>0 then
    select txt into dt_dkbs from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_dkbs';
end if;
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_lt';
if b_i1<>0 then
    select txt into dt_lt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_lt';
end if;
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kbt';
if b_i1<>0 then
    select txt into dt_kbt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kbt';
end if;
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_hu';
if b_i1<>0 then
    select txt into dt_hu from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_hu';
end if;
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_ttt';
end if;
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kytt';
if b_i1<>0 then
    select txt into dt_kytt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kytt';
end if;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai in('dt_ct');
...
/
create or replace procedure PBH_XEBG_CTbg(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
    b_loi varchar2(100); b_lenh varchar2(1000); b_i1 number;
    b_ma_dvi varchar2(10); b_so_id number; b_lan number;
...
select count(*) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_ttt';
if b_i1=1 then
    select txt into dt_ttt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='ds_ttt';
end if;
if b_i1<>0 then
    select txt into dt_kytt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan and loai='dt_kytt';
end if;
...
/
create or replace procedure PBH_XEBG_NH_NH(
...
begin
-- viet anh - Nhap
b_loi:='loi:Loi Table bh_xeB:loi';
b_lan:=FKH_JS_GTRIn(dt_ct,'lan');
select nvl(max(lan),0) into b_i1 from bh_xeB where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_lan <> 0 and b_lan < b_i1 then b_loi:='loi:Khong duoc sua bao gia cu:loi'; return; end if;
if b_lan <> b_i1 then
  select nvl(ttrang,' ') into b_ttrang_bg from bh_xeB where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_i1;
...
PBH_XEB_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'N',b_lan,b_loi);
if b_loi is not null then return; end if;
b_bien:=nvl(trim(b_bien_xe),b_so_khung);
insert into bh_xeB_ds values(b_ma_dvi,b_so_id,b_so_id,b_bien,b_loai_xe,b_nam_sx,b_md_sd,b_ma_sp,b_loai_xe);
for b_lp in 1..dk_lh_nv.count loop
    if dk_lh_nv(b_lp)<>' ' then
        if dk_kieu(b_lp)='T' then b_tien:=b_tien+dk_tien(b_lp); end if;
        insert into bh_xeB_dk values(b_ma_dvi,b_so_id,b_so_id,dk_ma(b_lp),dk_ten(b_lp),dk_kieu(b_lp),dk_lh_nv(b_lp),dk_tien(b_lp),dk_phi(b_lp),dk_ptG(b_lp));
    end if;
end loop;
insert into bh_xeB values(
...
insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_ct',dt_ct);
insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_dk',dt_dk);
if trim(dt_dkbs) is not null then
    insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_dkbs',dt_dkbs);
end if;
if trim(dt_lt) is not null then
    insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_lt',dt_lt);
end if;
if trim(dt_kbt) is not null then
    insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_kbt',dt_kbt);
end if;
if trim(dt_hu) is not null then
    insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_hu',dt_hu);
end if;
if trim(dt_ttt) is not null then
    insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_ttt',dt_ttt);
end if;
if trim(dt_kytt) is not null then
    insert into bh_xeB_txt values(b_ma_dvi,b_so_id,b_lan,'dt_kytt',dt_kytt);
end if;
delete from bh_xeB_ls where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_lan;
insert into bh_xeB_ls (ma_dvi,so_id,lan,loai,txt) select ma_dvi,so_id,lan,loai,txt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id  and lan=b_lan;
PBH_BAO_NV_NH(b_ma_dvi,b_nsd,b_so_id,b_so_hd,'XE',b_ttrang,b_phong,b_ma_kh,b_ten,
    b_ngay_ht,b_ngay_hl,b_ngay_kt,b_nt_tien,b_tien,b_nt_phi,b_phi,b_loi);
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_XEBG_NH(
...
PBH_BG_JS_TEST(
    b_ma_dvi,b_nsd,b_so_id,'bh_xe',dt_ct,dt_kytt,
