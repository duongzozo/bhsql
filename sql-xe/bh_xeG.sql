create or replace procedure PBH_XEG_MO(
...
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_sp from
    bh_xe_sp a,(select distinct ma_sp from bh_xe_phi where ngay_bd<=b_ngay and ngay_kt>b_ngay) b
    where a.ma=b.ma_sp and FBH_XE_sp_HAN(a.ma)='C';
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten) into cs_cdich from
    bh_ma_cdich a,(select distinct cdich from bh_xe_phi where ngay_bd<=b_ngay and ngay_kt>b_ngay) b
    where a.ma=b.cdich and FBH_MA_NV_CO(a.nv,'XE')='C' and FBH_MA_CDICH_HAN(a.ma)='C';
select JSON_ARRAYAGG(json_object(a.ma,a.ten) order by a.ten returning clob) into cs_goi from
    bh_xe_goi a,(select distinct goi from bh_xe_phi where ngay_bd<=b_ngay and ngay_kt>b_ngay) b
    where a.ma=b.goi and FBH_XE_GOI_HAN(a.ma)='C';
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_khd
    from bh_kh_ttt where ps='KHD' and nv='XE';
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_kbt
    from bh_kh_ttt where ps='KBT' and nv='XE';
select JSON_ARRAYAGG(json_object(ma,ten,loai,bb,ktra) order by bt returning clob) into cs_ttt
    from bh_kh_ttt where ps='HD' and nv='XE';
select JSON_ARRAYAGG(json_object('ma' value FKH_SO_Fm(pt),'ten' value FKH_SO_Fm(muc)) order by muc) into cs_ktru
    from bh_xe_ktru where ngay_kt>b_ngay order by muc;
...
/
create or replace procedure PBH_XEG_CT(
...
select json_object(
    'nhom_xe' value FBH_XE_NHOM_TENl(nhom_xe),'loai_xe' value FBH_XE_LOAI_TENl(loai_xe),
    ...
    into dt_ct from bh_xe_ds where so_id=b_so_id;
...
select JSON_ARRAYAGG(json_object(ma,ten) order by bt returning clob) into dt_dk from bh_xe_dk
    where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_bh <> 'M';
select JSON_ARRAYAGG(json_object(ma,ten) order by bt returning clob) into dt_dkbs from bh_xe_dk
    where ma_dvi=b_ma_dvi and so_id=b_so_id and lh_bh = 'M';
select count(*) into b_i1 from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_lt';
if b_i1<>0 then
    select txt into dt_lt from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_lt';
end if;
select count(*) into b_i1 from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_kbt';
if b_i1<>0 then
    select txt into dt_kbt from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_kbt';
end if;
...
select count(*) into b_i1 from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
if b_i1<>0 then
    select txt into dt_ttt from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ttt';
end if;
select JSON_ARRAYAGG(json_object(loai,txt returning clob) returning clob) into dt_txt
    from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai in('dt_ct','dt_dk','dt_dkbs');
...
/
create or replace procedure PBH_XEG_KBT(
...
begin
-- Dan - Update kbt
b_loi:='loi:Loi xu ly PBH_XEG_KBT:loi';
...
for b_lp in 1..kbt_ma.count loop
    if trim(kbt_kbt(b_lp)) is not null and FBH_XE_BPHI_DK_LOAI(b_so_id,kbt_ma(b_lp))='TV' and
		FBH_XE_BPHI_DK_LHBH(b_so_id,kbt_ma(b_lp))='C' then
...
/
create or replace procedure PBH_XEG_TESTr(
...
if b_md_sd<>' ' and FBH_XE_MDSD_HAN(PKH_MA_TENl(b_md_sd))<>'C' then b_loi:='loi:Sai muc dich su dung:loi'; return; end if;
if b_ma_sp<>' ' and FBH_XE_SP_HAN(b_ma_sp)<>'C' then b_loi:='loi:Sai ma san pham:loi'; return; end if;
if b_cdich<>' ' and FBH_MA_CDICH_HAN(b_cdich)<>'C' then b_loi:='loi:Het chien dich:loi'; return; end if;
if b_goi<>' ' and FBH_XE_GOI_HAN(b_goi)<>'C' then b_loi:='loi:Ma goi het han:loi'; return; end if;
...
if b_hang<>' ' and FBH_XE_HANG_HAN(b_hang)<>'C' then b_loi:='loi:Sai hang xe:loi'; return; end if;
if b_hang<>' ' and b_hieu<>' ' and FBH_XE_HIEU_HAN(b_hang,b_hieu)<>'C' then b_loi:='loi:Sai hieu xe:loi'; return; end if;
if b_hang<>' ' and b_hieu<>' ' and b_pban<>' ' and FBH_XE_PB_HAN(b_hang,b_hieu,b_pban)<>'C' then
    b_loi:='loi:Sai phien ban xe:loi'; return;
end if;
if b_dong<>' ' and FBH_XE_DONG_HAN(b_dong)<>'C' then b_loi:='loi:Sai dong xe:loi'; return; end if;
if b_dco not in(' ','X','D','E','H') then b_loi:='loi:Sai loai dong co:loi'; return; end if;
if b_loai_xe<>' ' and FBH_XE_LOAI_HAN(b_loai_xe)<>'C' then b_loi:='loi:Sai loai xe:loi'; return; end if;
if b_nhom_xe<>' ' and FBH_XE_NHOM_HAN(b_nhom_xe)<>'C' then b_loi:='loi:Sai nhom xe:loi'; return; end if;
...
for b_lp in 1..dk_maX.count loop
    ...
    dk_nv(b_kt):=dk_nvX(b_lp); dk_kieu(b_kt):=''; dk_ma(b_kt):=dk_maX(b_lp); dk_ten(b_kt):=dk_tenX(b_lp);
...
for b_lp1 in 1..3 loop
    if a_nv(b_lp1)<>' ' then
        FBH_XE_BPHI_SO_ID('G',a_nv(b_lp1),'C',b_md_sd,b_ma_sp,b_cdich,b_goi,b_loai_xe,
        ...
        for b_lp in 1..dk_ma.count loop
            if dk_nv(b_lp)=a_nv(b_lp1) then
                if dk_lkeP(b_lp)= 'M' then
                    ...
                    FBH_XE_BPHI_DKm(b_so_idP,dk_ma(b_lp),dk_tien(b_lp),dk_lh_bh(b_lp),dk_ptB(b_lp),dk_phiB(b_lp),b_loi);
                    if b_loi is not null then return; end if;
                end if;
            end if;
        end loop;
...
if b_xe_id<>0 then
    for r_lp in(select distinct nv_bh from bh_xe_ds where xe_id=b_xe_id and so_id_dt<>b_so_idD and
        FKH_GIAO(b_ngay_hl,b_ngay_kt,ngay_hl,ngay_kt)='C' and FBH_XE_TTRANG(ma_dvi,so_id,'C')='D') loop
...
if b_so_hdL='E' and b_kieu_hd<>'S' and instr(b_nv_bh,'B')<>0 and b_ttrang='D' then
    ...
    if FBH_XE_SO_ID(b_ma_dvi,b_so_hd) > 0 then b_loi:='loi:So an chi da su dung '||b_so_hd||':loi'; return; end if;
...
if b_ttrang in('T','D') then
    ...
    select count(*) into b_i1 from bh_xe_phi_txt where so_id=b_so_idP and loai='dt_khd';
    if b_i1<>0 then
        select txt into dt_khd from bh_xe_phi_txt where so_id=b_so_idP and loai='dt_khd';
        b_i1:=length(dt_khd)-2;
        dt_khd:=substr(dt_khd,2,b_i1);
        PBH_XEG_KHD(dt_ct,dt_dk,dt_khd,b_loi);
        if b_loi is not null then return; end if;
    end if;
end if;
b_loi:='';
...
/
create or replace procedure PBH_XEG_NH_NH(
...
for b_lp in 1..dk_lh_nv.count loop
    ...
    insert into bh_xe_dk values(b_ma_dvi,b_so_id,b_so_idD,dk_bt(b_lp),
...
insert into bh_xe_ds values(b_ma_dvi,b_so_id,b_so_idD,0,b_kieu_hd,b_so_hd,b_so_hd_g,
...
delete bh_xe_ID where xe_id=b_xe_id;
insert into bh_xe_ID values(b_xe_id,b_tenC,b_cmtC,b_mobiC,b_emailC,b_dchiC,
    b_bien_xe,b_so_khung,b_so_may,b_hang,b_hieu,b_pban,
    b_loai_xe,b_nhom_xe,b_dong,b_dco,b_ttai,b_so_cn,b_nam_sx,b_gia);
...
insert into bh_xe_txt values(b_ma_dvi,b_so_id,'dt_dk',dt_dk);
if trim(dt_lt) is not null then
insert into bh_xe_txt values(b_ma_dvi,b_so_id,'dt_lt',dt_lt);
end if;
if trim(dt_kbt) is not null then
insert into bh_xe_txt values(b_ma_dvi,b_so_id,'dt_kbt',dt_kbt);
end if;
if trim(dt_dkbs) is not null then
    insert into bh_xe_txt values(b_ma_dvi,b_so_id,'dt_dkbs',dt_dkbs);
end if;
...
if dt_ttt is not null then
    insert into bh_xe_txt values(b_ma_dvi,b_so_id,'dt_ttt',dt_ttt);
end if;
if b_ttrang in('T','D') then
  if b_ktru<>0 then
    PBH_XEG_KBT(b_nv_bh,b_so_idP,b_ktru,dt_kbt,b_loi);
      if b_loi is not null then return; end if;
  end if;
    ...
    insert into bh_xe_kbt values(b_ma_dvi,b_so_id,b_so_idD,b_txt,dt_lt,dt_kbt);
...
if b_ttrang='D' then
   if b_so_hdL='P' then
    ...
    insert into bh_hd_goc_ttindt values(
        b_ma_dvi,b_so_idD,b_so_idD,'XE',b_bien_xe||' -- '||b_so_khung,b_ma_kh,b_ngay_kt,' ',b_ma_ke);
end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_XEG_NH(
...
if b_so_id<>0 then
    ...
    if b_i1<>0 then
        select ngay_ht,kieu_hd,ttrang into b_ngay_htC,b_kieu_hd,b_ttrang from bh_xe
...
/
create or replace procedure FBH_XE_BPHI_CTs(
...
if b_loi is not null then raise PROGRAM_ERROR; end if;
b_loi:='loi:Loi xu ly FBH_XE_BPHI_CTs:loi';
