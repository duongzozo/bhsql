create or replace function FBH_XE_TXT(b_ma_dvi varchar2,b_so_id number,b_tim varchar2,b_so_id_dt number:=0) return nvarchar2
AS
...
if b_so_id_dt in(0,b_so_id) then
    select count(*) into b_i1 from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
    if b_i1=1 then
        select txt into b_txt from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
    else
        select nvl(max(lan),0) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
        if b_i1<>0 then
            select txt into b_txt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_i1 and loai='dt_ct';
        end if;
    end if;
    ...
    if b_i1=1 then
        select txt into b_txt from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ds';
    else
        select nvl(max(lan),0) into b_i1 from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ds';
        if b_i1<>0 then
            select txt into b_txt from bh_xeB_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and lan=b_i1 and loai='dt_ds';
        end if;
    end if;
    if length(b_txt)<>0 then
...
/
create or replace function FBH_XE_TXTn(b_ma_dvi varchar2,b_so_id number,b_tim varchar2,b_so_id_dt number:=0) return number
AS
...
/
create or replace function FBH_XE_SO_IDp(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return varchar2
as
    b_kq varchar2(100);
begin
-- Dan - Tra so_idP
select min(so_idP) into b_kq from bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_XE_BIEN(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number,b_ngayN number:=30000101) return varchar2
AS
...
select min(nvl(trim(bien_xe),so_khung)) into b_kq from
    bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_XE_TUOI(b_nam_sx number) return number
AS
...
/
create or replace function FBH_XE_TUOIt(b_thang_sx number,b_nam_sx number) return number
AS
...
/
create or replace function FBH_XETSO_SO_ID(b_bien_xe varchar2,b_so_khung varchar2) return number
AS
    b_kq number:=0;
begin
-- Dan - Tra nghiep vu
...
/
create or replace procedure PBH_XETSO_BIEN(
...
b_so_id:=FBH_XETSO_SO_ID(b_bien_xe,b_so_khung);
if b_so_id<>0 then
    select json_object(ten,cmt,mobi,email,dchi,bien_xe,so_khung,so_may,
        'hang' value FBH_XE_HANG_TENl(hang), 'hieu' value FBH_XE_HIEU_TENl(hieu),'pban' value FBH_XE_PB_TENl(hang,hieu,pban),
        'loai_xe' value FBH_XE_LOAI_TENl(loai_xe),'nhom_xe' value FBH_XE_NHOM_TENl(nhom_xe),
        'dong' value FBH_XE_DONG_TENl(dong),dco,ttai,so_cn,nam_sx,gia)
        into cs_ct from bh_xe_ID where xe_id=b_so_id;
end if;
select json_object('cs_ct' value cs_ct) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace function FBH_XE_NV(b_ma_dvi varchar2,b_so_id number) return varchar2
AS
    b_kq varchar2(1);
begin
-- Dan - Tra nghiep vu
select min(nv) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace procedure PBH_XE_GCN(b_ma_dvi varchar2,b_so_id number,b_loi out varchar2)
AS
    b_i1 number; b_i2 number:=0; ds_gcn pht_type.a_var; 
begin
-- Nam- Tra so GCN
b_loi:='loi:Loi xy ly PBH_XE_GCN:loi';
     select count(*) into b_i1 from bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_id;
     if b_i1<>0 then
        for rec in ( select gcn from bh_xe_ds where ma_dvi = b_ma_dvi and so_id=b_so_id) loop
            b_i2:=b_i2+1;
            ds_gcn(b_i2) := rec.gcn;
        end loop;
        for ds_lp in 1..ds_gcn.count loop
           ...
           update bh_xe_ds set gcn=ds_gcn(ds_lp) where ma_dvi=b_ma_dvi and so_id=b_so_id;
        end loop;
     end if;
b_loi:='';
...
/
create or replace procedure FBH_XE_DT_NG(
...
b_so_idB:=FBH_XE_HD_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select nvl(min(ngay_hl),0),nvl(min(ngay_kt),0) into b_ngay_hl,b_ngay_kt
    from bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=b_so_id_dt;
if b_ngay_hl=0 then
    select nvl(min(ngay_hl),0),nvl(min(ngay_kt),0) into b_ngay_hl,b_ngay_kt from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_idB;
end if;
end;
/
create or replace function FBH_XE_HL(
...
b_so_idB:=FBH_XE_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select ngay_hl,ngay_kt into b_ngay_hl,b_ngay_kt from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_idB;
if b_ngayd<=b_ngay_kt and b_ngay_hl<=b_ngayc then b_kq:='C'; end if;
return b_kq;
end;
/
create or replace function FBH_XE_HLd(
...
b_so_idB:=FBH_XE_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
if b_so_idB<>0 then
    select ngay_hl,ngay_kt into b_ngay_hl,b_ngay_kt from bh_xe_ds  where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id_dt=b_so_id_dt;
    if b_ngay between b_ngay_hl and b_ngay_kt then b_kq:='C'; end if;
end if;
return b_kq;
end;
/
create or replace function FBH_XE_KIEU_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Dan - Tra so hop dong
select nvl(min(kieu_hd),' ') into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_XE_KIEU_HDd(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(1); b_so_idD number:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
begin
-- Dan - Tra kieu hop dong qua so_id
select nvl(min(kieu_hd),' ') into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_idD;
return b_kq;
end;
/
create or replace function FBH_XE_SO_HD(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50);
begin
-- Dan - Tra so hop dong
select min(so_hd) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_XE_SO_HDd(b_ma_dvi varchar2,b_so_id number) return varchar2
as
    b_kq varchar2(50); b_so_idD number;
begin
-- Dan - Tra so hop dong dau
b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
b_kq:=FBH_XE_SO_HD(b_ma_dvi,b_so_idD);
return b_kq;
end;
/
create or replace function FBH_XE_SO_ID(b_ma_dvi varchar2,b_so_hd varchar2,b_nv varchar2:='') return number
as
    b_kq number;
begin
-- Dan - Tra so id
if nvl(b_nv,' ')=' ' then 
    select nvl(min(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
else 
    select nvl(min(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_hd=b_so_hd and nv=b_nv;
end if;
return b_kq;
end;
/
create or replace function FBH_XE_ID_ID_DT(b_ma_dvi varchar2,b_so_id number,b_gcn varchar2) return number
as
    b_kq number;
begin
-- Dan - Tra so id
select nvl(min(so_id_dt),0) into b_kq from bh_xe_ds where ma_dvi=b_ma_dvi and gcn=b_gcn;
return b_kq;
end;
/
create or replace function FBH_XE_ID_GCN(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return varchar2
as
    b_kq varchar2(20);
begin
-- Dan - Tra gcn
select min(gcn) into b_kq from bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_XE_HD_ID_DT(b_ma_dvi varchar2,b_so_hd number,b_gcn varchar2) return number
as
    b_kq number:=0; b_so_id number;
begin
-- Dan - Tra so id DT
b_so_id:=FBH_XE_SO_ID(b_ma_dvi,b_so_hd);
if b_so_id<>0 then b_kq:=FBH_XE_ID_ID_DT(b_ma_dvi,b_so_id,b_gcn); end if;
return b_kq;
end;
/
create or replace function FBH_XE_SO_IDd(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Dan - Tra so id dau
select nvl(min(so_id_d),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_XE_HD_SO_IDd(b_ma_dvi varchar2,b_so_hd varchar2) return number
as
    b_kq number;
begin
-- Dan - Tra so id dau
select nvl(min(so_id_d),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_hd=b_so_hd;
return b_kq;
end;
/
create or replace function FBH_XE_SO_GCNd(b_ma_dvi varchar2,b_so_id number,b_so_id_dt number) return number
as
    b_kq number; b_so_idD number;
begin
-- Dan - Tra so GCN dau
b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
select min(gcn) into b_kq from bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_idD and so_id_dt=b_so_id_dt;
return b_kq;
end;
/
create or replace function FBH_XE_SO_IDc(b_ma_dvi varchar2,b_so_id number,b_ttrang varchar2:='K') return number
as
...
b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
if b_ttrang<>'C' then
    select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
else
    select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D';
end if;
return b_kq;
end;
/
create or replace function FBH_XE_HD_SO_IDc(b_ma_dvi varchar2,b_so_hd varchar2,b_ttrang varchar2:='K') return number
as
    b_kq number:=0; b_so_idD number;
begin
-- Dan - Tra so id cuoi
b_so_idD:=FBH_XE_HD_SO_IDd(b_ma_dvi,b_so_hd);
if b_ttrang<>'C' then
    select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
else
    select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D';
...
/
create or replace function FBH_XE_SO_IDt(
...
-- Dan - Tra so id cuoi
b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang in('T','D') and ngay_ht<=b_ngay;
return b_kq;
end;
/
create or replace function FBH_XE_SO_IDb(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return number
as
    b_kq number:=0; b_so_idD number;
begin
-- Dan - Tra so id cuoi
b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D' and ngay_cap<=b_ngay;
return b_kq;
end;
/
create or replace function FBH_XE_HD_SO_IDb(
...
-- Dan - Tra so id cuoi
b_so_idD:=FBH_XE_HD_SO_IDd(b_ma_dvi,b_so_hd);
select nvl(max(so_id),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD and ttrang='D' and ngay_ht<=b_ngay;
return b_kq;
end;
/
create or replace procedure FBH_XE_SO_ID_DT(
    b_so_id_dt number,b_ma_dvi out varchar2,b_so_id out number,b_ngay number:=30000101)
as
begin
-- Dan - Tra so id cuoi qua so_id_dt
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_xe_ds where so_id_dt=b_so_id_dt;
if b_so_id<>0 then b_so_id:=FBH_XE_SO_IDb(b_ma_dvi,b_so_id,b_ngay); end if;
end;
/
create or replace procedure FBH_XE_SO_ID_DTf(
...
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_xe_ds where so_id_dt=b_so_id_dt;
if b_so_id=0 then
    select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_xe where so_id=b_so_id_dt;
    if b_so_id<>0 then
        b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
        select count(*) into b_i1 from bh_xe_ds where so_id_dt=b_so_idD;
...
/
create or replace procedure FBH_XE_SO_ID_GCN(
    b_gcn varchar2,b_ma_dvi out varchar2,b_so_id out number,b_so_id_dt out number,b_ngay number:=30000101)
as
begin
-- Dan - Tra so id cuoi qua so_id_dt
select count(*) into b_so_id from bh_xe_ds where gcn=b_gcn;
if b_so_id<>1 then
	b_ma_dvi:=''; b_so_id:=0; b_so_id_dt:=0;
else
	select ma_dvi,so_id,so_id_dt into b_ma_dvi,b_so_id,b_so_id_dt from bh_xe_ds where gcn=b_gcn;
	b_so_id:=FBH_XE_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
end if;
end;
/
create or replace procedure FBH_XE_SO_ID_DTd(
    b_so_id_dt number,b_ma_dvi out varchar2,b_so_id out number)
as
begin
-- Dan - Tra so id dau qua so_id_dt
select min(ma_dvi),nvl(min(so_id),0) into b_ma_dvi,b_so_id from bh_xe_ds where so_id_dt=b_so_id_dt;
if b_so_id<>0 then b_so_id:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id); end if;
end;
/
create or replace procedure FBH_XE_HD_SO_ID_DT(
    b_gcn varchar2,b_ngay number,b_ma_dvi out varchar2,b_so_id out number,b_so_id_dt out number)
as
begin
-- Dan - Tra so id qua GCN
select nvl(min(so_id_dt),0) into b_so_id_dt from bh_xe_ds where gcn=b_gcn;
if b_so_id_dt<>0 then
    FBH_XE_SO_ID_DT(b_so_id_dt,b_ma_dvi,b_so_id,b_ngay);
else
    b_so_id:=0; b_ma_dvi:='';
end if;
end;
/
create or replace procedure FBH_XE_HD_SO_ID_DTd(
    b_gcn varchar2,b_ma_dvi out varchar2,b_so_id out number,b_so_id_dt out number)
as
begin
-- Dan - Tra so id dau qua GCN
select min(ma_dvi),nvl(min(so_id),0),nvl(min(so_id_dt),0) into b_ma_dvi,b_so_id,b_so_id_dt from bh_xe_ds where gcn=b_gcn;
if b_so_id<>0 then b_so_id:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id); end if;
end;
/
create or replace function FBH_XE_NT_TIEN(
...
b_so_idB:=FBH_XE_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select nvl(min(nt_tien),'VND') into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_idB;
return b_kq;
end;
/
create or replace function FBH_XE_NT_PHI(
    b_ma_dvi varchar2,b_so_id number,b_ngay number:=30000101) return varchar2
as
    b_kq varchar2(5); b_so_idB number;
begin
...
b_so_idB:=FBH_XE_SO_IDb(b_ma_dvi,b_so_id,b_ngay);
select nvl(min(nt_phi),'VND') into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_idB;
return b_kq;
end;
/
create or replace function FBH_XE_MA_DT(
...
begin
-- Dan - Tra so id cuoi
b_so_idB:=FBH_XE_SO_IDt(b_ma_dvi,b_so_id,b_ngay);
if b_so_idB<>0 then
    select nvl(min(loai_xe),' ') into b_kq from bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_idB and so_id=b_so_id_dt;
else
    select nvl(min(loai_xe),' ') into b_kq from bh_xeB_ds where ma_dvi=b_ma_dvi and so_id=b_so_id and so_id=b_so_id_dt;
end if;
return b_kq;
end;
/
create or replace function FBH_XE_CAP(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Dan - Tra ngay cap
select min(ngay_cap) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_XE_NGAY(b_ma_dvi varchar2,b_so_id number) return number
as
    b_kq number;
begin
-- Dan - Tra nhap
select nvl(min(ngay_ht),0) into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
return b_kq;
end;
/
create or replace function FBH_XE_TTRANG(
    b_ma_dvi varchar2,b_so_id number,b_dk varchar2:='K',b_ngay number:=30000101) return varchar2
as
    b_kq varchar2(1); b_i1 number;
begin
-- Dan - Tra ngay cap
select nvl(min(ttrang),'X') into b_kq from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
...
/
create or replace procedure PBH_XE_SO_ID_KTRA(b_ma_dvi varchar2,b_so_id_g number,b_loi out varchar2)
AS
    b_i1 number;
begin
-- Dan - Kiem tra SO_ID moi nhap
if b_so_id_g=0 then b_loi:=''; return; end if;
select count(*) into b_i1 from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id_g and ttrang<>'D';
if b_i1<>0 then b_loi:='loi:Hop dong cu da xoa hoac chua ky:loi'; return; end if;
select count(*) into b_i1 from bh_xe where ma_dvi=b_ma_dvi and so_id_g=b_so_id_g and kieu_hd<>'K';
...
--create or replace procedure PBH_XE_BPHI_CTm(
create or replace procedure FBH_XE_BPHI_DKm(
...
b_loi:='loi:Loi xu ly FBH_XE_BPHI_DKm:loi';
select count(*),nvl(max(tien),0) into b_i1,b_tienM from bh_xe_phi_dk where
...
select nvl(max(pt),0),nvl(max(phi),0) into b_pt,b_phi from bh_xe_phi_dk
    where so_id=b_so_id and ma=b_ma and lh_bh=b_lh_bh and tien=b_tienM;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
CREATE OR REPLACE procedure PBH_XE_BS(
...
select JSON_ARRAYAGG(json_object(ngay_ht,so_hd,gcn) order by ngay_ht desc,so_hd desc,gcn desc returning clob) into cs_lke from
        (select distinct a.ngay_ht,a.so_hd,b.gcn from bh_xe a,bh_xe_ds b
...
/
create or replace procedure PBH_XE_LKE(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_klk='N' then
	...
	if b_klk='P' then
		...
		select count(*) into b_dong from bh_xe where ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv;
		PKH_LKE_TRANG(b_dong,b_tu,b_den);
		select JSON_ARRAYAGG(obj returning clob) into cs_lke from
			(select json_object(ma_dvi,so_id,so_hd,nsd) obj,rownum sott from bh_xe  where
...
/
create or replace procedure PBH_XE_LKE_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_hd:=FBH_XE_SO_HD(b_ma_dvi,b_so_id);
...
if b_klk ='N' then
    ...
    select nvl(min(sott),b_dong) into b_tu from
        (select so_hd,rownum sott from bh_xe where
            ma_dvi=b_ma_dvi and ngay_ht=b_ngay_ht and nv=b_nv and nsd=b_nsd order by so_hd)
        where so_hd>=b_so_hd;
...
/
create or replace procedure PBH_XE_SO_ID(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_id:=FBH_XE_SO_ID(b_ma_dvi,b_so_hd,b_nv);
...
/
create or replace procedure PBH_XE_XOA_XOA(
...
AS
    ...
    b_ttrang varchar2(1); b_ksoat varchar2(20); b_so_hdL varchar2(20);
begin
-- Dan - Xoa
b_loi:='loi:Loi xu ly PBH_XE_XOA_XOA:loi';
select count(*) into b_i1 from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
if b_i1=0 then b_loi:=''; return; end if;
select so_id_d,so_id_kt,ttrang,ksoat,nsd,so_hdl into b_so_idD,b_so_idK,b_ttrang,b_ksoat,b_nsdC,b_so_hdL
    from bh_xe where so_id=b_so_id;
...
b_loi:='loi:Loi xoa Table bh_xe:loi';
delete bh_hd_goc_ttindt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_xe_kbt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_xe_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_xe_dk where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_xe_ds where ma_dvi=b_ma_dvi and so_id=b_so_id;
delete bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_XE_XOA(
...
PBH_XE_XOA_XOA(b_ma_dvi,b_nsd,b_so_id,'X',b_loi);
if b_loi is not null then raise PROGRAM_ERROR; end if;
if b_comm='C' then commit; end if;
exception when others then rollback; if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_XE_HDBS(
    b_ma_dviN varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
b_so_idD:=FBH_XE_SO_IDd(b_ma_dvi,b_so_id);
if b_so_idD<>0 then
    select count(*) into b_dong from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
    select JSON_ARRAYAGG(json_object(ngay_ht,so_hd,ttoan,ttrang,ma_dvi,so_id,
           'ma_sdbs' value PKH_TEN_TENl(FBH_XE_MA_SDBS(so_id))
    ) order by so_id desc returning clob)
        into cs_lke from bh_xe where ma_dvi=b_ma_dvi and so_id_d=b_so_idD;
end if;
select json_object('dong' value b_dong,'cs_lke' value cs_lke) into b_oraOut from dual;
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_XE_NV(b_ma_dvi varchar2,b_so_id number,
...
select nt_tien,nt_phi,nv into b_nt_tien,b_nt_phi,b_nv from bh_xe where ma_dvi=b_ma_dvi and so_id=b_so_id;
select a.loai_xe,b_nt_tien,b_nt_phi,b.lh_nv,b.t_suat,sum(b.tien),sum(b.phi),sum(b.thue),sum(b.ttoan),min(b.pt),max(b.ptG)
    ...
    from bh_xe_ds a,bh_xe_dk b where a.ma_dvi=b_ma_dvi and a.so_id=b_so_id and
...
select ngay,b_nt_phi,tien bulk collect into tt_ngay,tt_ma_nt,tt_tien from bh_xe_tt where ma_dvi=b_ma_dvi and so_id=b_so_id;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else null; end if;
end;
/
create or replace procedure PBH_XE_DON(b_ma_dvi varchar2,b_so_id number,b_nv varchar2,b_loi out varchar2)
AS
    ...
    a_mau_ac pht_type.a_var;a_loai_ac pht_type.a_var;a_gcn_m pht_type.a_var; a_gcn_c pht_type.a_var; a_gcn pht_type.a_var;  r_hd bh_xe%rowtype;
begin
...
if r_hd.nv='G' then
  select txt into a_ds_ct(1) from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ct';
  b_lenh:=FKH_JS_LENH('loai_ac,mau_ac,gcn');
  EXECUTE IMMEDIATE b_lenh into a_loai_ac(1),a_mau_ac(1),a_gcn(1) using a_ds_ct(1);
else
  select count(1) into b_i1 from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ds';
  if b_i1 > 0 then
     select FKH_JS_BONH(txt) into dt_ds_txt from bh_xe_txt where ma_dvi=b_ma_dvi and so_id=b_so_id and loai='dt_ds';
  end if;
...
/
create or replace procedure PBH_XE_VACH(b_nv varchar2,b_gcn out varchar2,b_loi out varchar2)
AS
    b_i1 number;
begin
...
b_gcn:=' ';
select count(*) into b_i1 from bh_xe_vach where nv=b_nv;
if b_i1<>0 then
    select stt into b_i1 from bh_xe_vach where nv=b_nv for update wait 10;
    if b_i1<>0 then
        b_i1:=b_i1+1;
        update bh_xe_vach set stt=b_i1 where nv=b_nv;
...
/
create or replace procedure PBH_XEG_PHIb(
    b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_oraIn clob,b_oraOut out clob)
AS
...
if b_so_hdG<>' ' then
    b_so_idG:=FBH_xe_SO_ID(b_ma_dvi,b_so_hdG);
    if b_so_idG=0 then b_loi:='loi:Hop dong goc da xoa:loi'; return; end if;
end if;
FBH_XE_PHI(dt_ct,dt_dk,dt_dkbs,
...
if b_so_idG<>0 and b_ngay_hl<b_ngay_cap then
    select ngay_hl,ngay_kt into b_ngay_hlC,b_ngay_ktC from bh_xe_ds
    ...
    select ma,tien,pt,phi bulk collect into dk_maG,dk_tienG,dk_ptG,dk_phiG from bh_xe_dk
    ...
    for b_i3 in 1..dk_ma.count loop
        select count(*) into b_i4 from bh_xe_dk where ma_dvi=b_ma_dvi and so_id=b_so_idG and b_so_id_dt in (0,so_id_dt) 
    ...
    select ma,tien,pt,phi bulk collect into dkbs_maG,dkbs_tienG,dkbs_ptG,dkbs_phiG from bh_xe_dk
...
FBH_XEG_PHIt(b_tp,dk_ma,dk_ma_ct,dk_lkeP,dk_cap,dk_phi,dk_thue,dk_ttoan,b_loi);
if b_loi is not null then return; end if;
FBH_XEG_PHIt(b_tp,dkbs_ma,dkbs_ma_ct,dkbs_lkeP,dkbs_cap,dkbs_phi,dkbs_thue,dkbs_ttoan,b_loi);
if b_loi is not null then return; end if;
b_oraOut:='';
for b_lp in 1..dk_ma.count loop
...
/
create or replace procedure FBH_XE_PHI(
    dt_ct clob,dt_dk clob,dt_dkbs clob,
...
begin
-- viet anh - Tinh phi
b_loi:='loi:Loi xu ly FBH_XE_PHI:loi';
...
for b_lp_dk in 1..dk_ma.count loop
  ...
  -- lay ptB
  PBH_XE_BPHI_PRORATA_PT(dt_ct,dk_nv(b_lp_dk),'C',dk_ma(b_lp_dk),dk_tien(b_lp_dk),b_nt_tien,b_nt_phi,dk_ptB(b_lp_dk),b_loi);
  if b_loi is not null then return; end if;
...
FBH_XEG_PHIb(b_tp,dk_ma,dk_ma_ct,dk_lkeP,dk_lkeM,dk_cap,dk_kieu,dk_tien,dk_phi,b_loi);
if b_loi is not null then return; end if;
FBH_XEG_PHIb(b_tp,dkbs_ma,dkbs_ma_ct,dkbs_lkeP,dkbs_lkeM,dkbs_cap,dkbs_kieu,dkbs_tien,dkbs_phi,b_loi);
if b_loi is not null then return; end if;
b_loi:='';
exception when others then if b_loi is null then raise PROGRAM_ERROR; else raise_application_error(-20105,b_loi); end if;
end;
/
create or replace procedure PBH_XE_BPHI_PRORATA_PT(
...
b_loi:='loi:Loi xu ly PBH_XE_BPHI_PRORATA_PT:loi';
...
FBH_XE_BPHI_SO_ID('T',b_nv,'C',b_md_sd,b_ma_sp,b_cdich,b_goi,b_loai_xe,b_nhom_xe,b_dong,b_dco,b_ttai,b_so_cn,b_tuoi,b_gia,
...
/
create or replace procedure FBH_XEG_PHIb(b_tp number,
    dk_ma pht_type.a_var,dk_ma_ct pht_type.a_var,dk_lkeP pht_type.a_var,dk_lkeM pht_type.a_var,dk_cap pht_type.a_num,
    dk_kieu pht_type.a_var,dk_tien in out pht_type.a_num,dk_phi in out pht_type.a_num,b_loi out varchar2)
AS
    b_i1 number; b_cap number:=1; b_phi number; b_tien number;
begin
-- viet anh - Tinh bac cao
b_loi:='loi:Loi xu ly FBH_XEG_PHIb:loi';
for b_lp in 1..dk_ma.count loop
...
/
create or replace procedure FBH_XEG_PHIt(b_tp number,
...
begin
-- viet anh - Tinh bac cao
b_loi:='loi:Loi xu ly FBH_XEG_PHIt:loi';
b_cap:=b_cap-1;
while b_cap>0 loop
    for b_lp in 1..dk_ma.count loop
        if dk_cap(b_lp)<>b_cap then continue; end if;
